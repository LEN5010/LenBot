...
- 仍未做：除授权表单外没有别的“已登记对象引用”字段——扫过全部 11 个已声明插件，只有 `asoul_calendar` 的 `commands` 与 `avatar_paths` 是映射形状的进阶 JSON，按声明的表单子集无法表达，保持 JSON 编辑；面板改动仍缺一次真实人工核对。
...
- `src/len_bot/web/frontend` 执行 `npm run build` 成功（1.29s）；概览依赖卡只做了静态代码结论，未在真实实例上核对插件清单取值。**未**打开浏览器面板，未取得任何像素或人工页面核对，导航分组、群页分区、插件列表分区、字段级错误定位、授权表单与窄屏表现都只有静态代码结论。U06 要求的键盘进入／返回焦点与窄屏主要动作仍是待人工核对的交互项。
