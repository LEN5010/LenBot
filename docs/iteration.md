...
- **U04 常用控件**：表单按 Schema 逐个渲染——布尔、枚举、数字、文本、简单列表（可增删与上下移动的行，删空即提交空列表）、有声明属性的嵌套对象（workspace 的 `worker`／`gateway` 作为子分区编辑，切换分支时按 Schema 默认值初始化）以及其余对象／列表的高级 JSON；字段标题、说明和上下限都取自 Schema，前端不另写一份。提交前按 Schema 核对必填与 JSON 结构，服务端拒绝时按同一路径把错误落到对应控件。已登记对象引用下拉仍未做成通用控件，仍按文本填写。
- 仍未做：已登记对象引用（群、插件、主播、策略）的下拉仍需手填 ID；面板改动仍缺一次真实人工核对。
...
- `src/len_bot/web/frontend` 执行 `npm run build` 成功（1.26s）；**未**打开浏览器面板，未取得任何像素或人工页面核对，导航分组、群页分区、插件列表分区、字段级错误定位、授权表单与窄屏表现都只有静态代码结论。U06 要求的键盘进入／返回焦点与窄屏主要动作仍是待人工核对的交互项。
...
按依赖顺序实施 C12、C13、C15、C16、C17、C18、C19、C20（C14 视浏览器后端形态另定）。P1-U 的 U01／U02／U03／U05／U06 已在源码层面落地，剩余的 U04 常用控件（枚举中文名、简单列表行编辑、已登记对象引用下拉）尚未补齐；所有面板改动都需要一次真实人工核对（浏览器打开、键盘路径、窄屏）。
