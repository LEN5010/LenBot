...
本次前端与维护链路修复：信息工作筛选区改为可收缩横向布局，左上角品牌标记改为仓库内的 LenBot 极简 SVG；对话提示补充庆祝、吐槽和明确表情包请求时的检索、读图与适度发送规则。历史维护批次会为系统提示和工具 schema 预留输入空间，维护失败页面可读取对应 trace 的具体原因；运行配置样例的维护上下文上限调整为 32768。尚未在生产启动后观察新的维护重试、模型实际输出或 QQ 送达。
