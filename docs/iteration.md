...
## 2026-09-20：工具表变化的真实调用复核

本次获得有限真实模型调用授权。没有启动群消息收发主进程，没有执行响应中的工具，没有修改根配置、服务器配置或运行代码，没有重启服务。请求使用合成记录与现有工具定义，模型绑定、推理参数和输出上限读取根配置；只观察原始响应，不运行测试、断言或自动重试。临时请求与响应保存在本机临时目录，不加入仓库。

### 已观察：正文不变，工具表变化会失去原有缓存复用

沿根配置的对话入口顺序调用，703 条消息、约 139k 输入 token，和原业务请求同量级。A 有 25 个工具；B 仅移除 `refute_memory`、`supersede_memory`，消息正文、工具选择方式及其余参数不变。

| 调用 | 输入 token | 已报告缓存 token | 说明 |
|---|---:|---:|---|
| A1 | 139233 | 字段缺失 | 首次建立 |
| A2 | 139233 | 134969 | 原请求重复，96.94% |
| B1 | 138850 | 字段缺失 | 仅移除两个工具 |
| B2 | 138850 | 134976 | B 原样重复，97.21% |
| A3 | 139233 | 134969 | 恢复原工具表，原缓存仍可复用 |
| A4 | 139233 | 134969 | 仅修改尾部时间，仍为 96.94% |

六次均 HTTP 200、`finish_reason=tool_calls`，只记录响应，不执行工具。工具表变化与尾部时间变化的结果不同，因此不能再用消息目录稳定排除应用侧请求变化。`cognition/proposals.py` 的 `definitions()` 随工作、认识等可见状态增删工具，`terminal_definition()` 也会随本轮引用改变参数结构；后者的独立影响尚未实测。

### 历史五次异常不再是同一类

对原有 29 个轮次按首次模型调用时间排序，排除每群窗口内首次及窗口头跨边界后：

| 工具集合与上轮相比 | 缓存字段 | 次数 | 两次首次调用的开始时间间隔 |
|---|---|---:|---|
...
两次工具变化分别是群 `126300994` 移除 `revise_work` / `cancel_work`，以及群 `1102823315` 移除 `refute_memory` / `supersede_memory`；窗口头均未移动。后一群再次加回工具时恢复正数缓存。工具名称相同仍不等于完整 Schema 相同，历史正文与完整 Schema 未保存，不能逐次证明历史根因。

三次长间隔符合下面观察到的短时留存现象。此前“上游缓存固定保留 60 分钟”没有经过本链路验证，不能据此排除时间因素。此前基于较小负载宣称已经排除容量淘汰，也不能推广为所有负载的结论。

### 时间对照与单独的连接故障

A4 返回后等待 360 秒，从本机向原入口发送完全相同请求，13.334 秒后得到 `ReadError: [Errno 54] Connection reset by peer`。随后只读核对网关账单和转发器访问日志，均未找到该笔请求，故不计作缓存失效，没有自动重试。

为单独隔离本机至入口的传输影响，后续改从 lbot 本机访问同一个网关的 `127.0.0.1:3000/v1/chat/completions`，仍用根配置的凭据、绑定和相同请求正文，没有直连或更换上游通道：

| 调用 | 条件 | 输入 token | 缓存记录 |
|---|---|---:|---|
| A7 | 距 A4 返回约 510 秒 | 139233 | 响应字段缺失，网关计数 0 |
| A8 | A7 后立即重复 | 139233 | 134969，96.94% |
| A9 | 保持服务器本机路径，A8 返回后等待 310 秒 | 139233 | 网关计数 0；原始响应未取回 |
| A10 | A9 账单完成后约 97 秒，相同路径短间隔重复 | 139233 | 134969，96.94% |

A9 的 SSH 输出中断，原文为 `Read from remote host 106.53.150.204: Connection reset by peer` / `client_loop: send disconnect: Broken pipe`。只读回查确认请求已在服务器完成：访问日志 HTTP 200，网关账单 `created_at=1789840559`、`use_time=4`、`prompt_tokens=139233`、`completion_tokens=26`、`cache_tokens=0`。此行依据服务端日志，不伪称已经取得原始响应；原计划的立即重复未执行，A10 是之后单独发起的短间隔重复。

观察期间的服务端访问日志与通道 4 账单中，长空档内没有其他模型调用。相同入口和请求也能在数分钟空档后失去缓存复用，说明“必须有其他群灌入大量 token 才会冷”不是必要条件。当前证据支持约五分钟级的时间相关失效，不足以宣称精确固定 TTL 或排除上游不可见的淘汰行为。

本批共完成 10 次真实模型调用，另有 1 次未在服务端记录的传输失败；没有继续压测。可操作的代码侧问题是动态工具集合与 Schema 的缓存代价；稳定它们时仍须保留场景权限、引用合法性及提交校验，不能为了缓存放宽执行边界。运行代码尚未修改，通道 6 的间歇失败不在本次核验范围内。

### 额外观察：网关扣额没有缓存折扣

通道 4 本批账单的 `cache_ratio=1`。A1 缓存计数为 0、A2 为 134969，但二者 `quota` 都是 5225138；B1 / B2 也同为 5210775。即本次网关额度扣减没有因命中缓存降低，不能把命中率提高直接写成当前网关费用下降；这不是上游实际账单的证明。没有修改计费参数。
