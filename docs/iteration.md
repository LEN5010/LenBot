...
- `src/len_bot/web/frontend` 执行 `npm run build` 成功（1.31s）；**未**打开浏览器面板，未取得任何像素或人工页面核对，导航分组、群页分区、插件列表分区、字段级错误定位、授权表单与窄屏表现都只有静态代码结论。U06 要求的键盘进入／返回焦点与窄屏主要动作仍是待人工核对的交互项。
