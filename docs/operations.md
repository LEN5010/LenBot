...
某个群的历史摘要长期不再增长时，先在日志里找 `History maintenance blocked in <群>`：一次供应商失败或进程中断留下的未完成批次会挡住该群之后的全部摘要，不会自动重试。到群历史页找到那条 failed/pending 批次并重试；重试成功后维护会沿原前沿连续补齐积压。不要用删库或 Reset 绕过它，也不要在原批次仍未处理时手工改写覆盖范围。
