...
次数字段可以填 `null`，含义是“这一维度不参与停止判定”，此时该循环必须另有期限或累计 token 作为停止条件：对话轮次的次数与 `conversation_window_seconds` 不能同时为空，配置解析会直接拒绝这种没有任何停止条件的组合；历史维护的模型次数保持必填，因为该循环本来没有期限维度。工作的停止条件由运行账本给出——绝对期限按 `job_max_seconds` 自该工作首次执行起累计（排队未开始不计入，恢复不重置），累计 token 上限取该工作创建时的预占额，已用量包含压缩、技能维护与插件子 Agent 的调用。额度只够终结一次时，运行时只提供终结工具，让工作在边界处提交已有结果与未完成项，而不是被截断；事件完成后页面“执行预算”表把 `null` 显示为“不设限（由其他维度停止）”，不会显示成空白或 0。
