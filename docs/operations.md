...
“额度策略”按名称保存单工作、账号日和可选场景日上限。账务日按 time.timezone，未配置则按 UTC；创建准入的账号日账跨群聚合。留空策略不再套用 10M/30M，各维度为不设上限；根文件里写出的数字才是限制。具名系统策略与授权 Grant 的选择尚不一致，见 FX05。保存 resources 会重新应用预算配置；已有工作按自己的快照执行，结束后恢复也使用原上限，不会按新默认策略补造。
