...
能力页按真实插件 ID 展示状态。常用任务走「启用向导」：本群使用 Python、公开研究与指定群分享、增加群播报来源。向导先预览将写入的字段，再在原配置锁内一次保存；不要求填写 grant_id 或内部主体。向导判断「是否已授予」用的就是运行时那一次判定，含 enabled 与到期：已过有效期的授予会被提议补发并在预览里注明，不会显示成已可用。研究向导不会自动发起付费模型或群发送。替换模型密钥、插件秘密或 OneBot 令牌时，若另一页已先改过同一凭据，保存会冲突并保留草稿；选择「保留我的改动」会用最新修订重建基线后再提交，选择「采用现值」放弃本页密钥改动。
