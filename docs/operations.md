...
延期记录沿任务详情查看原 action、original_due_at、delivery_phase 和实际回执。同批同请求的前序消息失败或未知时，后序会保存拒绝回执；前序仍未尝试时，后序继续等待原 Scheduler。无关请求不受该依赖限制。旧闲聊过期，工作成果不重跑，直播重取原场次。不要将 queued 当作已发送；attempted 无可靠回执、旧领取不明或 Gateway 终止未确认均需人工核对，不自动重发/重开。当前已无服务监听时，不为了页面核对擅自启动生产。
...
限额、连接、超时、上游内部错误和进程重启留下的批次，以及被中断留下的 `pending` 批次，由下一次维护自动转回 pending 续跑，日志记 `History maintenance resuming blocked batch in <群>`；每个批次每进程只自动恢复一次，所以恢复后再次失败的批次不会反复重跑。某个群的历史摘要长期不再增长时，在日志里找 `History maintenance blocked in <群>`：落在这里的是其余原因，以及已经自动恢复过一次又失败的批次，它们会挡住该群之后的全部摘要。到群历史页找到那条 failed/pending 批次并重试；重试成功后维护会沿原前沿连续补齐积压。重试点一次就够：该群正在对话时不再拒绝，后台会等这一轮结束再跑，连点得到的「该群的历史维护正在进行」说明它确实在跑，不是失败。不要用删库或 Reset 绕过它，也不要在原批次仍未处理时手工改写覆盖范围。摘要停更还会连带影响成本：原话窗口的起点要靠已完成摘要才能钉在固定位置，前沿被挡住时窗口逐轮滑动，该群每轮对话的请求前缀都无法复用供应商缓存。

历史摘要已完成但显示“认识候选待核对”时，打开对应维护回执查看候选、来源和版本变化。原候选没有写入认识账本，不能直接当作现有事实；核对当前原话和认识后再通过原认识操作处理。不要重试已完成摘要或用新版本号强行采用旧候选。

能力授予的到期控件按本机时区支持秒精度；未改到期字段时保留服务器原始 epoch（含更细精度）。白名单和授予分别按是否修改提交，保存白名单不会重新签发授予。已有授予版本冲突返回错误并保留页面草稿，需核对最新保存值。

从 Agent → 工具能力选择群和已有参与者，分别查看本群开放、当前主体授权、执行后端和最近事实。总览中的能力卡使用同一读取入口；“有历史记录”只表示有工具返回、执行或平台回执。点击原工作或回执继续核对实际工具、原请求与结果；未运行的能力显示“未验证”。系统 → 插件与开发保留连接参数与生命周期排查。

配置保存提示“配置已被其他操作修改”时，本次草稿没有保存，后端不会整节覆盖新值。先保留需要的草稿内容，再核对最新保存值和返回的冲突字段；刷新按钮保留未保存草稿及它的原基线，不会悄悄把冲突当作已解决。OneBot 令牌通过保留／替换／清除选择操作；留空的新值不能替代明确操作。旧版直接提交配置对象的调用方需更新为 baseline/values 合同，插件入口分别携带公开配置基线或原启用值。

人格、人工表达样例、参与方式与睡眠时间从 Agent 入口编辑；原设置页深链自动定位同一个新编辑器。模型页面分别显示保存的职责与当前运行绑定，应用失败时不会显示成保存失败；先查看差异与错误，不重复购买能力检查来代替配置核对。
