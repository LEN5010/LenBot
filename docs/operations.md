...
首次启用 `gscore_adapter` 前确认 Core 协议、端点和 bot 身份；启用 `workspace` 前先确定唯一执行后端：`worker` 需确认运行时、镜像、无网络和工作目录权限，`gateway` 需另外部署网关进程（见下文），两者不能同时配置；启用 `browser_agent` 前填写域名白名单并核对执行隔离。当前浏览器是同进程能力，仅可在明确获准的功能使用范围内操作，不能以白名单检查替代独立隔离验收。边界详见 [执行边界](execution-boundaries.md)。
...
workspace 插件在根配置 `plugins.workspace.config` 里二选一：`worker`（LenBot 调宿主容器运行时）或 `gateway`（独立 Worker Gateway）。必须恰好一个非 null，网关失败不会回落宿主执行。样例保持 worker 且 `gateway: null`。把真实根配置改成 gateway、部署网关服务与镜像，才是生产切换；本手册在未取得离线证据前不把它列为已完成步骤。源码已按 FX06—FX10 收口启动/取消、工作区占用、HTTP 分类、worker GID 挂载和产物边界。
...
创建期预占与工作行原子提交，失败会回滚创建。业务结果完成时，若还有技能候选则保持原预占直至维护请求结束；取消在仍有在途调用时不先释放。模型页账户合计按全日聚合，明细最多 100 条；账户全天只在同一具名策略下运行时按该策略显示日上限与余量，混用或无具名策略则回落默认策略。不能单独批准追加额度。费用核对查看完整 model_calls，区分真实 usage、估算和未确认。
