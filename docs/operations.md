...
首次开放新功能时，先确定一个聊天群和一个仅播报群、QQ 回复白名单与真实成员身份，填写业务时间和各来源参数。若该群仍有旧 AstrBot 同类日程／直播入口，在本次迁移授权内先停用旧入口，再开放 LenBot 对应插件与类别；同一功能同一群只保留一个负责人，不监听旧 Bot 或自动抢占。全局暂停实发使用 Shadow，停用一个群则关闭“本群设置”的启用字段；聊天群/播报群快捷操作只填写实际字段，不保存另一份 mode。QQ 回复白名单只在系统设置维护，不强制每条回复，也不授予运营权限；恢复配置只修改运营明确选定的值，不从文档复制群号。
