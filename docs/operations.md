...
某个群的历史摘要长期不再增长时，先在日志里找 `History maintenance blocked in <群>`：一次供应商失败或进程中断留下的未完成批次会挡住该群之后的全部摘要，不会自动重试。到群历史页找到那条 failed/pending 批次并重试；重试成功后维护会沿原前沿连续补齐积压。重试点一次就够：该群正在对话时不再拒绝，后台会等这一轮结束再跑，连点得到的「该群的历史维护正在进行」说明它确实在跑，不是失败。不要用删库或 Reset 绕过它，也不要在原批次仍未处理时手工改写覆盖范围。摘要停更还会连带影响成本：原话窗口的起点要靠已完成摘要才能钉在固定位置，前沿被挡住时窗口逐轮滑动，该群每轮对话的请求前缀都无法复用供应商缓存。
