...
作者从 [公共导出入口](../src/len_bot/plugins/api.py)取得现有类型，以本文的工具、事件、表达、工作与提交合同为世代 2 的使用说明。以下边界不因增加字段而改变：身份、场景、预算、阅读资格和提交回执归宿主；直接访问 `_runtime`、`_host` 或其他内部模块不属于公共合同。现有内部依赖尚待逐条收窄，声明世代不表示这项工作已经完成。
...
公共导出包括既有 `Event`、`ResultPresentation` 和窄 `PluginWorkSnapshot`、`PluginWorkPreparation`，接口世代现为 2。返回 Event 是本地查询对象，不是可写持久句柄；修改对象不改原事件，直接构造 Event 不赋予发布权限。群报告服务只保存业务配置，原话、资料分页、产物和状态读取均经当前调用；固定原话快照与调用截点／宿主账号不一致时明确结束，不改用当前范围继续。工作准备、阅读采用、续页和是否需模型的回调不再取得完整 store/job，其他执行协作与错误类型等内部依赖仍待收窄，不能据此声称全部公共化或实现恶意代码隔离。
...
全局关机的 unload_all 会记录单插件卸载错误并继续清理其他独立插件，但不会重新建立失败实例。注销条目不证明远程执行已经停止或外部客户端成功关闭；如远端取消未知，沿原执行记录保留未知，不能改写为成功。已保存原话、观察、工作结果与发送回执不因注销而删除；等待和未发送提醒仍沿原事务进入待核对。
