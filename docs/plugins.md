...
公共导出增加既有 `Event` 和窄 `PluginWorkSnapshot`，接口世代仍为 1。返回 Event 是本地查询对象，不是可写持久句柄；修改对象不改原事件，直接构造 Event 不赋予发布权限。群报告的服务对象现在只保存业务配置，原话、资料分页、产物和状态读取均经当前调用；固定原话快照与调用截点／宿主账号不一致时明确结束，不改用当前范围继续。群报告的内部工作准备、阅读采用与续页回调仍待收窄，不能据此声称已完全脱离内核存储。
