...
完整目标、D01—D12 裁决、M01—M20 模块和 C00—C29 提交合同见[完整实施计划](LenBot_社会Agent_完整实施计划_7a4152d.md)（保护文件，正文不改）。本文不复制计划对象与未来拓扑。公共兴趣、心跳与睡眠、文件上传及点赞收藏尚未形成当前可用链路。执行后端由 workspace 配置在宿主 worker 与独立 Gateway 之间二选一，样例仍是宿主 worker，浏览器仍在本进程内；源码可按配置把 `run_python` 交给网关，不等于执行隔离已验收。
...
`resources.policies` 保存具名 ReservationPolicy。未配置时各维度默认为 null（不设 token 上限），不再隐式套用 10M/30M。有限日额度必须能在创建时算出有限单工作预占，否则拒绝该组合，而不是预占 0。账务日使用业务时区，未配置时按 UTC；账号检查跨群聚合，场景检查仅在策略设限时执行。预占绑定授权该工作能力的那条 Grant。
...
每次工作侧 ModelGateway 请求在写入调用行的同一事务里按最终输入估算和本次输出上限准入。压缩、技能维护和插件工作子调用走同一入口：预算不足时保留已有结果，不追加一次免费的总结调用；真实 usage 高于估算时记入差额并停止新调用，不截断已消费的用量。结算按 call_id 幂等，迟到的真实 usage 替换该次调用的保守估算而不重复计费。complete_job 在仍有技能候选时把预占标为 settling 而不是立刻按消费收口（该上限覆盖业务结果之后再运行的维护）；最后一次回执按该调用自己的记录重算日账。历史维护的模型次数仍必填。

“没有 token 维度”与“没有可核对的上限记录”必须区分：`budget_json` 里写出的 `null` 是前者，该工作由期限和消息上限结束；既没有预算快照也没有预占上限的旧记录是后者，准入按“本工作没有可核对的上限记录”拒绝继续，不按无限额度放行。
...
容器使用无网络、只读根、非 root、cap drop、no-new-privileges、pids/内存/CPU 和临时目录限制；未显式指定 `container_user` 时跟随启动进程 UID/GID，实际映射须由部署确认。宿主控制脚本与输入只读挂载，工作目录保存产物。同一工作的输入导出与执行串行；失败不回落宿主 Python 或任意 shell。

取消或超时按唯一容器名尝试 kill、回收客户端、移除并核对，结局为 confirmed_stopped、confirmed_absent 或 unconfirmed。未确认记录阻止后续复用，取消请求本身不算停止成功。父运行取消、插件停用、工具超时和容器超时是四件不同的事；`WorkspaceCancelled` 是取消信号，宿主继续传播以结束等待它的 Agent，不改成普通工具错误后继续工作。外层工具时限覆盖内层执行期限加一次清理窗口，严格大于它而不是与它竞争。

目录字节数和文件数在执行中轮询、退出后再核对，这是应用层限制而不是底层文件系统硬配额；超限产物保留元信息和 over_limit。列表、文本分页与二进制下载都走同一条只读路径，文件名是本工作目录下的相对路径，因此中文与空格文件名原样列出并原样读回（已用解释器核对本地列出与网关遍历给出同一集合）；面板下载用 RFC 5987 的 `filename*=UTF-8''…` 加上由真实文件名派生的 ASCII 回退，中文名不会在响应头编码时失败。产物清单被截断时返回截断原因，不把「按上限截断的清单」当成完整目录。宿主文件读取通过固定工作区描述符逐级 O_NOFOLLOW 打开，以 O_NONBLOCK 避免特殊文件阻塞，并对同一 fd 做 fstat；文本按范围读取，二进制读取受字节上限约束。图片导出须成功登记媒体才成为附件，普通文件沿已授权工作面板下载，均不自动发群。

**产物与发送分开。** `export_workspace_artifact` 只登记媒体并返回附件引用，不发送；普通文件留在工作目录，只能由授权面板按工作归属读取或下载。图片登记失败时返回 `partial` 与原因（文件仍可下载），不把它写成发送成功。

输入文件先全部解码校验到暂存目录，再整体替换进执行控制区；被拒绝的输入不留半成品。
...
`execution/protocol`、`client`、`journal` 及 `services/worker_gateway` 已有代码。工作必须已有类型化发起者，否则拒绝。宿主先写自己的 `execution_runs` 行，再把脚本文本与 `input_files` 交给网关；`input_assets` 只作来源登记，字节不由网关拉取——宿主已把真实字节放进 `input_files`，因此一次附件导入的 `input_assets` 就是本次实际读取的资产 id，两者同时出现是正常形态；只有 id 而没有 `input_files` 的请求按缺口拒绝，不会带着缺失输入起跑。单次执行期限取网关配置与该工作剩余期限的较小值，超时或结果未知只查询同一执行 ID。取消走网关并把终止回执 park 回原工作。

新执行前对同一工作区未终结行做一次对账：网关未见过的记失败，已终结的补记，不重跑；未确认终止的行也在复核范围内。列表、分页读取、导出和面板下载取该工作最新一次已确认（终态）执行登记的产物；只有非终态记录时说明当前快照不可读，不用空目录或“从未执行”代替未知。历史产物必须显式带该次执行的 identity，不隐式回退到另一次执行。宿主的 execution_runs 与网关日志是两侧各自的事实，不是已同步的副本。

Gateway 的 execution_id 用于重复提交核对，记录 job/revision、workspace、镜像/网络策略引用、状态、输出和终止事实；exited 不表示业务结果正确。状态更新和序列事件共用事务。短执行未确认 running 时仍记录进程结果；取消后不再启动；重启 sweep 经取消/停止边收口，单条失败不拖垮查询服务。占用含未知终止。HTTP 客户端把枚举字符串解析为内部状态，并区分已拒绝、身份冲突和结果未知。控制目录按 worker GID 授权读取脚本，权限准备要核对实际可访问性，不满足时按部署错误结束本次执行而不是记成脚本失败；产物复制到不可变存放后按描述符打开。目录字节/文件数在运行中检查。正式放行条件只在完整计划和当前任务维护。
...
ToolResult 的 coordinate_unit 区分 characters 与 records，displayed_range 使用 `[start,end)` 和该单位的 total；next_call 包含工具名与可直接使用的参数，只定位本地已保存正文或记录的后续范围。source_next_call 单独指向尚未取得的源端下一批，保留原筛选与排序；模型须先读完本次已取得资料，才能继续源端范围。source_truncated 与本地分页、只给定位、空记录、原图未装入和 GIF 首帧覆盖分别保留，不能从一个截断标志推定其他覆盖。已保存正文完整不表示源端全文已经取回。
...
群报告 2.0 的业务在 group_summary 目录内：入口按真实 request_source 的时间解析自然日，固定范围和快照；service 读取并按消息时间、rowid 排序；analysis 依据完整请求估算安排连续批次，再折叠合并候选；report 保存类型化数据；render 仅用 Pillow 和本地字体生成 PNG。媒体标记只表示协议内容，没有图片像素就不描述画面。统计从完整固定范围计算，话题引用具体消息，参与者从这些来源的账号确定，金句由程序按原话片段提取。批次复用要求当前群、插件版本、原消息序列、业务时区和有效分析要求一致，且全部引用上下文仍在当前范围；焦点、额外目标或附加要求改变后重新选择可复用批次。复用的分析不进入当前工作的已读计数，也不成为长期事实。报告 complete 只表示本范围保存的文本均已有成功分析，未确认的讨论仍单独列出；预算不足以分析全范围但可形成已有部分时保存 partial。read_group_report 只按本群精确范围和工作版本读取保存的成品，不调用模型或发送，旧 1.1 参数与分页工作不自动转换。
...
插件生产者、Host、Toolkit、工作与维护读取使用 ToolResult；外部协议在生产者解析，持久观察从已知 JSON 结构读回，不使用任意字符串成功包装、包含判断或文本猜状态。工具执行失败由工具边界形成错误结果，供应商失败由 Gateway、事务失败由提交处负责。工具错误的 content、error_code、error_stage、tool_name、tool_call_id、字段 loc/type/message 及 HTTP 状态使用同一份 ToolResult；错误详情不保存原始 input、凭据或二进制。提前返回的参数错误、插件提案失败和核心提案异常也沿现有观察保存入口取得唯一 result_id；已有观察不重复保存，观察事件不触发新的社会认知。原始观察、ObservationPage 与读取授权保持不同职责，每页 evidence_span 直接由真实 result_id、coordinate_unit 和 displayed_range 派生，原始正文不因显示页或错误纠正而变化。

工具定义保存用途、别名、关键词与参数模型。Schema 从同一模型生成，Host 或核心入口严格解析一次后向 handler 传递类型化参数，null、默认值、时间和列表边界不再各自解释。注册时检查插件与工具重名，并从实际核心定义取得保留名，冲突报告双方来源，不静默覆盖处理器。发现只在当前场景、角色、插件状态与对象条件允许的候选集中，按名称／别名精确命中、关键词覆盖和描述片段排序；结果提供名称、用途与参数提示，无匹配返回 no_results 和当前可尝试类别，完整 Schema 在下一次请求展开，目录容量使用现有 tool_discovery_limit。别名不是执行入口，实际调用前仍复核可用性；请求期间停用插件会返回明确不可用观察。

find_person 按当前场景与截点内的账号/群名片/昵称和有效 reported 称呼查找身份，返回 U 与原话定位，不对 U 字符串做全文检索。短计算、网页与 B 站公开读取同时可用于 conversation/work，原网络边界不变。内置与本地插件都由各自目录的 PluginSpec 描述，Host 从实际注册生成工具清单，加载失败释放资源但保留发现元数据与错误；目录和公共入口见[插件开发](plugins.md)。历史、工具观察和媒体的场景隔离在 SQL／存储层执行，global-safe 素材由运营显式发布，现存网络策略限制可访问目标，工具发现与分页不改变这些权限。

ICS 仅用配置来源，保留源 UID、时间、取消与更新时间，未支持的源结构明确失败。动态服务只使用原站实际接口，详情只读已取得且仍有效的源记录，源续页携带原成员、筛选、排序和页量；公共缓存不按群重复采集，进入 Agent 的观察仍属于调用群，到期刷新失败不延长旧数据有效期，随机二创调用不按相同参数合并。直播订阅读取工具返回当前群订阅、公告开关和独立的样本状态；群内没有修改根配置的管理员能力，订阅/退订沿已登录面板的原场景保存入口，查询不写配置，也不把时间提醒伪装成订阅。采集只有一个公共循环，客户端以 `LenBot/0.1` 标识自身并继续禁用环境代理继承；首次样本建立基线，之后以真实 `(room_id, started_at)` 识别新场次，每群通知使用源场次与 scene 组成的事件身份并由现有事件唯一键去重，采集错误不视作下播。窄公告只装人格、明确主播/标题/链接/场次与公告参数，使用插件根配置 announcement_model_role 指向的既有绑定和 AgentLoop，不装整群历史或 pending wakes。

GSUID Core 适配器 `gscore_adapter` 默认停用，可选。它只在场景配置明确允许时把精确的 `/gs <命令>` 转成独立 Core 的 `MessageReceive` 帧，再把目标为已配置群的 `MessageSend` 转回 LenBot 插件事件；`MessageSend.echo` 只在 Core 请求时用于 `recall_message_id` 回执，不代表 Core 请求已完成。普通群聊不上报给 Core。适配器不提供直接操作 OneBot、数据库或发送队列的接口，网络隔离仍需部署保证；未知消息段保留在观察中，但不假定为可发送内容。连接身份 `core_bot_id`、上行平台身份 `platform_bot_id` 和 `bot_self_id` 分开配置。它没有搬运 AstrBot 插件运行时，也不会自动安装游戏插件；协议细节见 [SOURCE.md](../src/len_bot/plugins/builtin/gscore_adapter/SOURCE.md)。
...
媒体读取失败保留异常类型、执行阶段、HTTP 状态和去除凭据与查询参数的来源地址；httpx 请求日志也只保留方法、主机路径和状态。实际请求仍使用原地址与配置，不据 HTTP 状态猜测过期或风控，也不改变来源、重试或请求权限（单凭 400 或 403 不能断言链接过期或风控）。

MediaService 读取获准文件并解码、缩放，不计算内容校验和。新文件使用随机文件名；已登记资产继续使用原 path，不重新编号、下载或拆开历史共享文件，同一资产复用已有文件。发送已登记获准资产不要求当前像素；资产的启用与场景范围仍在提案事务和实际发送前核验。视觉分析仍以实际装入像素为依据。当前像素集合从实际消息块同步，压缩或恢复不会把历史读图永久占作当前窗口；换出图片仍保留来源与覆盖记录，缺失、未装入、截断和只覆盖首帧各自明确。视频和音频通过同一媒体资产、临时文件和发送准备路径，按独立文件字节上限处理；取消时清理临时文件，下载完成不代表已提交发送。内部 `audio` 发送到 OneBot 11 时映射为 `record`，下载工具成功结果以已验证资产的 MIME 为准，HTTP 响应头只作为来源观测。

运营素材和搜索候选还从本群有限 MESSAGE_SENT 窗口投影 last_sent_at、recent_send_count 和 used_in_last_reply；只按真实 sent 的登记资产计数，不存额外计数账本。目录只提供定位、说明与标签，发送使用获准原图；无人调用的目录联系表渲染已退出。明确选定或要求重复的已登记获准素材可直接提出发送；分析画面、断言图中文字或人物、依据视觉内容选图仍须读取实际像素；运营明确标为表情包的素材使用 QQ 自定义表情类型。

ActionQueue 在实际发送前复查素材范围、工作版本、当前群及输出类别，并使用媒体准备回调；准备失败进入原发送失败记录，不继续发送原消息，插件不能通过另一条发送中间件改写行动。全局 Shadow 单独维持观察语义，独立实发名单退出。同群保序、跨群可并发。Scheduler 是持久认领的单写者：睡眠与延期交付不得存放在内存 ActionQueue 中，也不新增第二套投递队列。同批失败依赖按 origin_event_id 与实际 job／履约／确认事项 ID 记录，只停止同一请求同一事项的后续消息，独立请求继续原有队序。回执保留 origin_event_id、requester_qq_uid、job_id、job_revision、batch 和 action 身份；真实 MESSAGE_SENT 才确认送达、履约与聊天 OpenLoop 激活，命令和公告不建立社交关注，Shadow 和模拟不产生这些事实。
...
RuntimeQueryService 按保存的事件、episode、job、action 和 result ID 组合公开投影，不以时间相近猜因果。额度页账户合计按全日预占/待收口/已结算聚合，明细仍最多 100 条；群筛选时另给场景小计，不再把截断明细当成账号余额。账户全天只在同一具名策略下运行时按该策略显示日上限与余量，混用多个策略时说明需要选定范围，无具名策略按默认策略说明并把用过的策略引用名单独列出，不显示由默认策略推导出的假余额。消息详情区分已读来源、处理来源与当前 pending；工作页分开显示请求原话、创建确认、当前版本、结果交付及实际采用范围；同轮其他请求保留各自归属。资料详情区分本地正文续读和“源端下一批，仅位置未取得”，页面只读已保存资料，不执行源端参数。原话安全显示，媒体走鉴权接口，磁盘路径、凭据和私有续接不外露。插件配置投影按 Schema 路径逐层去除凭据字段，只回可显示值与“该路径是否已配置”，令牌不以占位值回写。

写操作经现有事件／提案边界，保留具体失败，不自动重试；刷新不恢复工作或重发。迟到响应不覆盖新对象，未保存草稿和旧读取时间明确显示。技能 skipped 在面板显示正常原因，不显示为维护失败。

保存类页面把结果拆成四句可核对的话：草稿是否已写入根配置、运行时是否已应用、是否需要插件重装或服务重启、以及“配置已保存但应用失败”。保存失败保留草稿并定位到服务端指出的字段，字段旁与该路径显示同一句说明；字段错误在本机先做必填与 JSON 结构检查时同样先定位再提交。状态文字不依赖颜色单独表达，重建失败与保存失败分开显示。概览的依赖清单只读已保存记录来回答“还缺什么、去哪里填”：它不探测外部服务，也不随刷新打开任何能力，任何一项的成败都只反映当时的根配置与运行状态。
...
语义检索绑定位于 `models.retrieval`，与 conversation/work/maintenance 路由分开，默认为空；每个群还必须在场景设置中明确开启，未开启时不会把该群文本送往检索提供方。Embedding 与 rerank 请求各自按 `model_calls` 的 retrieval purpose 计量；rerank 必须填写已确认的协议，未确认时不会请求。索引只读取已经通过认识事务提交的 statement，保存在可删除重建的派生表中。查询仍先按 scene、主体、状态和时间范围回原账本过滤，语义候选不能授予原话证据或修订权限；索引失败不回滚认识提交。索引覆盖量只统计当前认识修订，重建清理按当时的原表状态执行，不删除重建期间新增的合法索引。历史摘要检索保留词面与语义命中的并集，不越过当前读取截点，摘要中的批次身份与查询观察的资料引用分开。
