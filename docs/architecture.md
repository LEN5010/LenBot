...
群总结在原 task.payload 保存工作种类、请求来源、summary_range 和 summary_coverage。总结 Schema 使用含时区的 ISO 8601 字符串，插件入口只转换一次为 AwareDatetime；Ledger 与 GroupSummaryRange 使用带时区对象，不再次猜格式。绝对范围与快照由提案固定，修订沿已有 revision；统计与分页复用同一 scene、时间半开区间和 rowid 上界条件。默认只读本群非 Bot 的真实人类消息，保留日程命令和评论。分页位置是存储顺序，引用来自原事件；资料取回不等于模型已读，展示范围决定实际覆盖。模型主动终结但尚未读完匹配记录时为 partial；reason 根据实际剩余预算区分主动结束与达到模型/工具/时间边界，硬预算阻止终结时为 interrupted；不新增调度器、模型流水线、群史库或自动技能维护。显式继续时，总结插件对已保存的原资料范围核验后，在工作目录提供当前 revision 的源续页位置。旧工具调用参数仍原样保留，提交旧游标会明确拒绝，不偷偷改参执行；已取得正文仍先按原 result_id 续读。
