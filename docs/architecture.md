...
直播只有一个公共采集循环，客户端以 `LenBot/0.1` 标识自身并继续禁用环境代理继承。首次样本建立基线，之后以真实 `(room_id, started_at)` 识别新场次。每群通知使用源场次与 scene 组成的事件身份，由现有事件唯一键在进入模型前去重；生成状态与失败在 trace，提交与发送状态沿原事件和 action 回执，无第二套投递库。窄公告只装人格、明确主播/标题/链接/场次与公告参数，仍使用 conversation 绑定和 AgentLoop；不装整群历史或 pending wakes。
