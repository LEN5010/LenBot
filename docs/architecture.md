...
专用工作在原 task.payload 保存 PluginOrigin、work_parameters 与 work_progress。PluginSpec.work 提供参数和进度类型及本地业务函数；JobStore 继续负责原事务、版本、真实阅读范围和结果提交，运行器不按插件名选择流程。插件钩子提供指令，声明的工具集合在上下文整理前生效。预算和模型绑定不另存第二份，工具中发起的专用 Agent 仍收取父工作账户的实际调用与时间。

群总结的带时区范围、快照、覆盖和续页规则归其插件目录。绝对范围在提案中固定，修订沿原 revision；统计与分页复用同一群、时间半开区间和 rowid 上界。只读本群非 Bot 的真实人类消息，保留命令和评论；取回资料不等于模型已读，实际展示范围决定覆盖。尚未完整读过匹配原话时，插件在结果事务中保留 partial 与未读说明；硬预算中断仍为 interrupted，不创建技能候选。显式继续时按原资料范围提供当前版本源续页位置，旧调用参数原样保留，旧游标明确拒绝，不偷偷改参执行。
