...
| `history_batch.id`、`source_event_ids`、`generation_version` | 历史维护批次保留明确纳入的来源事件 ID、原文覆盖范围、版本和完成状态；事件只读关联据原来源列表反查批次 | 来源参加批次不等于摘要已完成、来源仍可用、摘要进入模型请求或原话已读；范围重叠不能代替身份关系 |
...
`answer_basis` 的原话位置和资料页范围只说明本条答复所声明并实际核对的依据，不由 `source`／`covers` 自动推导。只读关联查询沿上述已存身份扩展事件、历史批次、调用、工作、行动与回执，不按时间或文本相似匹配；旧记录缺关系时显示未记录，不补造 ID。当前会话段 ID 只定位活动上下文，不替代 episode、工作或发送身份。
...
等待只在真实消息送达的原事务中激活。入场匹配仍要求未过期等待、真实发送来源、目标回复者与明确引用或唯一提及关系；匹配只是恢复候选，不代表已经认领。

EventStore 在原事件事务中按 loop_id、场景、真实发送来源、active 状态与期限条件认领。成功认领和收到的原话、媒体登记、Session 更新仍一起提交或回滚。如果候选在入场后已过期、被消费或改变，条件更新没有命中时不再连同真实原话一起拒绝：原话及原候选包继续保存，metadata.conversation_resume_error 记录原失败说明；旧等待不重新开放，不改 TTL，也不虚构恢复成功。

运行中收到这一标记会记录原 observation 的 not_started／wait_claim_rejected 并结束，不派发插件、不进入普通聊天或其他工作事件处理。后续普通对话调度与插件在途输入不重新采用它的恢复候选；提交时也不把这项已经失败的恢复当成必须吸收的新输入，避免无限否定别的答复。原话仍可按现有范围检索，标记不代表已读、已回答或已处理，不消费其他待处理来源。

这里只区分失效的业务认领与原始输入保存。数据库、媒体登记或 Session 写入失败仍沿原事务回滚并抛出，不改参数重试；保存成功后的诊断写入失败也不回滚已经保存的原话。尚未确认的运行分支不通过制造竞态验证，旧历史事件不批量补标记。

### 工作修订、撤销与成果采用

工作 checkpoint 与 complete_job 仍在原写锁和短事务内核对 job_id、场景、revision 及 processing 状态。取消／修订改变原工作版本，旧执行不能用旧版本覆盖当前结果；已完成的读取和实际费用沿原记录保留，不当作旧结论已被采用。

普通 finish_work 与专用插件工作都通过同一个 commit_result。它将既有 require_current_access 作为必需内部回调传给 complete_job，在证据和候选整理之后、completed／partial 成果写入之前复核当前插件／场景／主体资格。此处复用原执行入口的权限逻辑，不新建 grant、权限模型或重新选择账目策略。

权限拒绝沿原 BaseException 回滚整个成果事务，包含本事务暂存的进度、方法候选或公共兴趣采用；此前已保存的观察和实际计费不回滚。原工作运行器继续用现有 current_access_denied 分支保存 interrupted 说明及已取得资料；这一类中断收尾不要求恢复被撤销的执行权，也不自动继续模型调用、采用成功成果或发送。工作已取消／改版时，原版本条件仍优先阻止收尾覆盖新状态。

成果及状态采用还在原写事务中复核本次实际资料引用：从本群保存观察回读，认识查询按修订／到期、历史摘要按批次及原话来源检查。回调在插件最终整理和候选暂存之后、最终结果写入之前执行；拒绝沿原成果／工具参数错误传播并回滚本事务候选，随后仍有原当前执行资格核对。普通外部页面不因此被自动联网复验；认识与摘要的具体规则见[上下文合同](context.md#工作成果与状态采用时的资料复核)。

失败、中断或取消结果中的旧工作状态只供审计，不再要求它满足新完成步骤的证明规则，避免旧状态不可用时无法记录退出。显式新状态和 completed／partial 成果仍按原证据规则及当前来源核对；失败保留不代表重新采用旧结论。

这是成果采用点的当前资格核对，不撤回已经发生的模型请求或既往已提交事实；结果准备、交付提交和发送仍有各自原边界，不能以 result_ready 或存有图片宣称已送达。

## 配置与持续数据

普通对话当前段的原话窗口引用和完整原生交换由 SceneActor 在原执行租约内保存到 scene_sessions.state_json.conversation_segment；下一轮原话从原事件回读，原生交换在同一模型绑定下按资料身份、页范围和宿主最小回执重建。它不提升观察截点、不消费唤醒、不记成已读或提交。开启／换段与引用更新使用同一原存储事务，失败不修改内存段或自动换来源；成功终结、拒绝、取消和提交后发布失败各自的保存结果见[原生交换合同](context.md#当前段的原生交换)，覆盖及重启行为见[原话窗口合同](context.md#当前段的原话窗口引用)。

对话等待共享执行槽位时尚未建立处理 episode，因此另以原 Stimulus 身份保存 conversation_wait 轨迹，关联明确的 source_event_ids；它不生成虚构轮次或模型调用，也不改变待处理来源。槽位取得、等待取消和失败都保留实际等待区间；等待失败记录若也失败，记录日志并继续传播原错误，不自动重试业务。save_trace 的写入／提交受原写锁保护，异常或取消回滚后继续抛出，避免等待取消的诊断写入留下未完成事务。
...
| 已保存的事实 | 本机如何参与 | 不能据此推断 |
|---|---|---|
| 真实提及、回复 Bot、私聊，或已匹配的等待答复 | 直接搭话理由进入短合并车道，来源保留独立身份 | certain 不是必须回复，更不绕过场景、睡眠、额度和原执行资格 |
| 名称或关键词命中 | 按原冷却取得弱观察机会，引用文字不算新命中 | 弱机会不是确定请求，也不是关键词强制回复 |
| 短时观察期或已有对话进行中 | 新原话获得观察机会；在途补充仍沿同一轮原入口 | 观察期不等于必须发言，也不自动再开一套决策流程 |
| 真实送达且回应对象与请求／工作关系匹配 | 只为匹配对象续期有限关注；明确 release 的对象不续期 | 提交提案、模拟或失败回执都不是已确认互动 |
| 普通周期消息 | 第一条待观察输入持有截止，后续消息只能提前最大截止 | 群中持续有消息不意味着可以把同批等待无限往后推 |
| 显式 silent／incomplete 处理结果 | 分别保留理由或未完成范围；silent 与实际消息／操作冲突时拒绝 | 没有发送记录本身不能证明模型选择了沉默 |

已提交但尚无回执的回应对象，只在原有限时间内作为 in_flight_follow_up 线索，不写成真实送达后的持续关注。focused_participants 在处理后续事件时按到期时间清理；observing_until 的延长须来自真实直接输入，或本轮新处理原话的明确观察操作，不能由定时器、Bot 自己的消息或同一旧来源无限续期。

核对“提及后没有回复”时，先看原事件是否有入口理由，再看观察／等待记录是否因场景、睡眠、限额或执行失败没有开始；已进入请求时，再区分实际原话阅读、来源的 silent／incomplete／replied 等处理和最终发送回执。只有来源处理记录能说明主动沉默的理由，只有真实回执能说明送达；请求是否本来就需要回答仍由实际原话和人工复核确定，不从提及标记替人判断。四条车道的最大等待是本机合并边界，不是模型响应或平台送达的实时承诺。
...
当前循环能完成的短查询、计算和必要续读直接使用已开放工具；read_tool_result 读取同群保存资料的下一页，不创建后台工作。只有较长执行、跨轮进度或工作专用能力才显式 start_work。一般社交不强制搜索，资料不足则保留具体缺口，不能因没有查询结果而随意改写对象或答案。

| 交给模型的语义选择 | 宿主从已有事实确定的关系 |
|---|---|
| 回应哪条来源、实际对谁说、是否展示引用或期待回答 | 解析已读 M／U；展示引用、回应对象与请求者不互相猜测 |
| 读取什么、是否续页、是否委托较长工作 | 按本入口目录、原范围和预算执行，不因注册或分页另建工作 |
| 创建、修订、取消、继续或引用成果 | 从 request_source、原工作及 ack_ref／operation_ref／delivery_ref／work_ref 派生请求者、身份和修订 |

普通 start_work 在当前未提交 Ledger 中，对完全相同的请求原话、目标、约束、来源／资料列表和复用成果快照返回原 proposal_ref／ack_ref，不再追加一份同样的创建提案。插件工作除原插件身份和专用参数外，也比较本次约束与来源／资料；引用先解析，变化输入不被旧目标匹配吞掉。列表顺序保持，不作相似内容推断。

复用只针对暂存提案，不改变真实调用／回执及原预算，不自动撤销或修改不同输入的旧提案。需要替换时先 discard_proposal；已提交后暂存列表清空，须通过真实工作引用处理现有事项，不能靠旧 ack_ref 取得继续或交付资格。不同人或不同请求原话不自动合并；重复确认、实际创建及发送仍受原 respond、事务与回执合同约束。
...
| 用户要做什么 | 现有路径 | 保留的边界 |
|---|---|---|
| 修改目标或条件 | revise_work，原工作增加 revision | 原请求者、资料、绑定、累计费用和创建时预算保留；旧结果不代表新目标成功 |
| 取消未终结工作 | cancel_work，原工作增加 revision 并取消 | 不采用旧版本结果；已经发生的外部动作不能据此宣称撤回 |
| 继续原未完成范围 | resume_work，原 ID 的新 revision | 不改目标，不重新获得全部预算；期限和已用量仍计算 |
| 对已读成果另做整理／导出 | start_work + reuse_work_ref，新的真实委托 | 固定所选原结果版本，只传资料与整理输入，不继承阅读、额度或上传资格 |
| 再引用／发送已有成果 | 读取原结果后沿原消息或资产交付入口 | 不为重发重新研究；未知回执及重复文件上传仍按原规则拒绝自动重放 |

完整参考业务沿[当前群增量报告](plugin-examples.md#三后台工作与产物当前群增量报告)：真实委托经原暂存与提交进入后台工作，插件保存资料／进度并形成 PreparedWorkDelivery，完成事件回到场景后经 Actor/Gate 和发送队列交付。该路径有源码，不代表已有本次同版现场回执；普通文件的生成、下载、上传区别另见[文件资产与交付](#普通文件资产与交付提案)。

插件成品交付的调用、工具箱与 Mailbox 使用同一个原 job/revision 和 work-delivery 轮次，initiator 取自原工作；缺身份仍为空，不从处理器补借身份。它保留原场景、来源、读取截点和 conversation 角色，不以交付入口重开后台执行。回调前的资格、提交事务的版本、实际尝试前的资格和最终回执分别核对。
...
历史维护的原文片段位置另带 source_context：原事件类型、说话人、模拟标记以及发送记录的既有送达分类／来源模式。它在新批次与原批次读回时从事件投影，不写进 history_source_text，也不改变已保存的字符坐标或完整覆盖身份。完整片段的正文已含事件类型和说话人，发给模型的位置记录只写 event_id、complete 和正文没有的字段（simulated 仅为 true 时写出，发送记录的送达分类与来源模式照写）；分段片段仍带字符坐标和完整 source_context。

历史批次按扣除认识读取预留后的输入容量切分：每次允许的 query_memory 预留 2048 估算 token（最多占输入容量一半），关闭工具或只有一步时不预留。面板重试沿用同一容量核对。维护终结和原提交事务均拒绝以模拟记录产生当前 review_items；认识仍经过同一写入校验。摘要可以如实描述模拟记录，但不是把模拟互动变成真实经历，也不因此自动重做旧摘要。
...
事件详情提供只读诊断预览与下载。`event_diagnostics` 复用原同场景事件关联查询和各类上限，仅按固定字段投影事件／提交／调用／行动／工作／工具资料及直接来源历史批次的身份、状态、范围与用量；格式 2 增加批次 ID、版本、范围和即时来源可用性，不复制摘要正文。事件详情在原六阶段之外另列直接来源批次，不将维护关系当作回复依据。诊断不导出正文、人格、工具参数、错误原文、请求快照、配置或媒体。仍含场景与业务编号，不视作匿名资料。接口继续使用原 Cookie 登录，响应禁止缓存；生成不写数据库、不新增阅读资格、不执行模型或平台。读取起止与截断类别随文件提供，跨查询不承诺原子快照，同轮关联不当作本条独占因果。
...
| 路径 | 何时取得机会 | 不承诺什么 |
|---|---|---|
| Scheduler 心跳 | 当前真实槽，原系统主体、占用、容量、启用与睡眠条件允许 | 没有种子可零调用结束；有种子也不保证产生有用成果 |
| 公共研究 | 原公共工具子集、匿名来源及原工作预算准入 | 不继承私群历史、成员、凭据和私有技能，不决定发布群 |
| 公共兴趣分享 | 有效候选通过当前目标群的话题、权限、睡眠与配额检查 | 候选不是必发通知，群语境可选择沉默；未知发送不重放 |
| 直播邀请 | 订阅对象的实际新场次先保存为业务事件，再沿原插件入口处理 | 首次采样不补刷旧开播；场次失效或订阅撤销不发送旧邀请 |

心跳的触发检查不是最终采用资格。后台创建工作在原事务内，读完占用和活动工作数量后，再按当前根配置和同一检查时钟核对槽位、开关、睡眠、为人类保留的位置与公共研究能力；拒绝不创建原占用／工作关联。原拒绝／失败说明保留，不自动换槽、改种子或重新购买调用。已经建立的工作仍沿其原预算和终止边界，而不是由这个启动检查重新获得额度。

主动业务必须显式配置并取得所属主体和目标范围授权。选择参考业务、保存配置、实际运行、候选提交和真实送达是不同步骤；本文不启用任何业务。完整业务参考选择与当前启用情况只在当前任务记录，真实开启或发送另行授权。
...
候选事件在插件发出与 Actor 接入时按描述符的 `candidate` 类型解析；出站准备从该已保存事件读取 `interest_id`／`revision`，再由原 `publication_for` 回读当前兴趣与来源关系，不在内核再次导入业务插件的候选模型。出站来源／版本、群授权、配额及实际送达仍由原 Gate 与发送事务核对；这只去掉一处反向类型依赖，不表示兴趣分享已从内核全部拆出。
