...
同一调用还提供已核验来源、当前群固定截点原话、所属插件版本资料分页、专用工作只读快照与场景媒体记录可用性。宿主绑定场景、插件身份及操作类型；工作快照只投影身份、版本、状态、请求者和解析后的业务参数副本，不返回预算、检查点或可写工作对象。群报告服务不再持有完整 EventStore；工作准备／采用回调与其他内部集成仍保持原合同，尚未全部公共化。
