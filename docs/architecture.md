...
请求消息按 persona、近期窗口、关联/取回原话、易变事实排序；窗口只在本次已装配摘要完整覆盖待裁事件时按全局 rowid 锚定。核心提案、respond 和对话原话续读定义不随待处理目标增删；具体引用不进入 Schema。插件发现、角色/资格变化和原 hook 的合法子集仍可能改变目录，固定核心结构不是缓存命中保证。具体容量、锚定、图片和工作压缩机制统一见[上下文装配](context.md)。
