...
材料格式 v2 增加固定对话合同及核心提案工具的声明修订号与当次快照；最终内容不再符合声明时明确缺档，不用当前源码补旧调用。页面同时读取 v1 的位置清单与 v2 的固定组件，显示工具定义留存数量，并按需展开具体片段和定义。人格、插件动态指令与未接入工具仍不保存完整内容；快照范围与维护方式见[上下文装配](context.md#7-缓存与诊断边界)。这不改变调用登记、准入、计量、原话阅读或执行权限。
