...
小时限额到顶时不再对 @ 自动发送固定说明。被搭话提高的是读取优先级，不是"必须解释沉默"的义务，而那条提示不经模型，聊真人嘉然的群会反复收到。入口拒绝改为写一条 `observation` trace（`status='not_started'`、`reason='hourly_limit'`），剩余额度显示在群配置页。
...
提醒目录另提供按原交付条件计算的 can_deliver；只有 processing/result_ready、尚无 delivery_action_id 且需要结果的旧 query 已有结果时，T 引用才进入 delivery_ref 枚举。本轮已控制的提醒不再提供旧交付引用。respond 在提交前重新读取任务并拒绝未就绪交付，数据库仍以原事务条件复核并认领发送，状态竞争不绕过最终校验。pending、review_required、awaiting_delivery 等状态不会仅因进入目录而获得交付资格；系统调度槽不进入普通提醒目录。
...
历史维护沿 ReflectionEngine、LLMReflector 和原 history 存储处理新增原始范围。生成前保存本群认识的主体、类别、版本与状态快照；提交时在原事务内核对候选目标及同主体同类别的记录。无关认识修订不阻断摘要，相关记录变化则不采用该候选，将完整提案、前后版本和原因存入同一 REFLECTION_RECORDED 回执；摘要、实际采用的认识、未采用候选和覆盖一起提交。此为 A02 对原“认识冲突令整批失败”合同的定向调整，不自动改写候选版本或重新购买模型调用。群历史页独立显示待核对候选，摘要完成不表示所有认识已采用。对话期间维护在 Actor 队列外等待；存储层仍核对当前 Actor 状态、来源范围、关键原话与证据。提交后索引/回调失败不改写完成事实。供应商失败或进程中断留下的批次按它自己的状态分流：`commit_history_batch` 把摘要与 `completed` 写在同一个事务里，所以非 completed 的批次可证明没有写过摘要，重做它产生不出同一段对话的第二份说法，而被拒的请求也没有花掉调用。据此，`RESUMABLE_HISTORY_ERRORS`（限额、连接、超时、上游内部错误、进程重启）以及 `pending`，由 `begin_history_batch` 就地转回 pending 继续，等价于替运营者执行了那一次重试；按批次 id 记在进程内，每个批次每进程只自动恢复一次，再失败即回到下面的阻塞。其余原因仍须显式处理，且这个阻塞是**完全的**：`begin_history_batch` 见到这样的非 completed 批次就返回 None，该群从此不再生成摘要，长期记忆停止生长，直到有人在面板重试。面板重试在该群对话进行时不再被拒绝：它占住该群的维护位，等这一轮结束再跑。忙群里两次对话之间的空隙可能永远等不到，而定时器那条路只会再次拒绝未完成批次，重试若在此处交还给定时器就等于被静默丢掉。因此被挡住时按"每个新的阻塞点一次"记 WARNING，写明群、批次、错误类型与被卡的 rowid 区间，不再静默停摆。
...
**跳过本槽与是否还有下一槽是两件事。** 小时限额的判断移入插件 `run_slot()` 内部（记 `skipped_hourly_limit`），续排留在同一个 `finally` 里：限额只让当前槽正常结束，仍然启用的插件照常获得下一槽；只有停用才停止续排（`ensure_next()` 自身核对 `manifest.enabled`）。错过的分享不补发，也不另设后台自愈循环。此前 Runtime 入口在限额时直接取消任务，绕过了续排，单群部署会就此停摆到重启或改配置。
