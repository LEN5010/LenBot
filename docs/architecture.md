...
保存类页面把结果拆成四句可核对的话：草稿是否已写入根配置、运行时是否已应用、是否需要插件重装或服务重启、以及“配置已保存但应用失败”。保存失败保留草稿并定位到服务端指出的字段，字段旁与该路径显示同一句说明；字段错误在本机先做必填与 JSON 结构检查时同样先定位再提交。状态文字不依赖颜色单独表达，重建失败与保存失败分开显示。
