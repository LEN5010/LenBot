...
内建插件由一个显式目录注册表加载。ICS 仅用配置来源，保留源 UID、时间、取消与更新时间，未支持的源结构明确失败。动态服务只使用原站实际接口，详情只读已取得且仍有效的源记录，源续页携带原成员、筛选、排序和页量。公共缓存不按群重复采集，进入 Agent 的观察仍属于调用群；到期刷新失败不延长旧数据有效期，随机二创调用不按相同参数合并。
