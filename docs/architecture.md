...
阶段 2 已实现结构化工具观察、通用正文读取、资料分页和工具发现；阶段 4 已接入独立工作；阶段 5 已接入媒体与发送节奏；阶段 6 已接入真实回复观察与独立人工评阅；真实视觉与实群自然度仍待验收。既有真实模型五例显示纠错和依据不稳定。限制随实施更新，不因批准计划而视为解决。
...
MESSAGE_SENT 的真实 message_id 在同一 Event 事务建立 reply_observations。仅从该时刻向后观察五分钟或十五条人类消息，以先到者为准；引用自动关联，其余原话只记窗口上下文，不臆测为追问或满意。运营通过 REPLY_FEEDBACK_LABELLED 追加追问/纠正/态度及单独自然度评价。Shadow、模拟送达和回放输入不产生真实互动证据；无回应是未知。评价事件不推进社会观察截点，不能作为独立记忆证据，不自动改人格、任务或社会认识。

新版 diana-v3 预览展示具体样例，稳定追加排查、未知、纠正与结束四组；保留已有diana-v2样例的ID、人工修改和启停。应用前需验证预览令牌，启动不自动应用。认知在最终续接后可确认本轮已经读到的连续工具观察，不能越过未读人类输入推进截点。
