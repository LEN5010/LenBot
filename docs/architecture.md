...
目录字节数和文件数在执行中轮询、退出后再核对，这是应用层限制而不是底层文件系统硬配额；超限产物保留元信息和 over_limit。宿主文件读取通过固定工作区描述符逐级 O_NOFOLLOW 打开，以 O_NONBLOCK 避免特殊文件阻塞，并对同一 fd 做 fstat；文本按范围读取，二进制读取受字节上限约束。图片导出须成功登记媒体才成为附件，普通文件沿已授权工作面板下载，均不自动发群。

输入文件先全部解码校验到暂存目录，再整体替换进执行控制区；被拒绝的输入不留半成品。

### Gateway

`execution/protocol`、`client`、`journal` 及 `services/worker_gateway` 已有代码。工作必须已有类型化发起者，否则拒绝。宿主先写自己的 `execution_runs` 行，再把脚本文本与 `input_files` 交给网关；`input_assets` 只作来源登记，字节不由网关拉取。单次执行期限取网关配置与该工作剩余期限的较小值，超时或结果未知只查询同一执行 ID。取消走网关并把终止回执 park 回原工作。
