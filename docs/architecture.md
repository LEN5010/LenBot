...
[完整改造计划](LenBot_群聊体验与可靠执行_完整改造计划_20260920.md) 规定待实现目标，本文只记录现有源码合同。核心工具目录、终结定义、答复级 `answer_basis` 数据链，以及群工作台与单条依据界面已分批修改源码；范围与缺口见当前任务，实际页面、真实协议、资料回答及缓存效果仍待确认。心跳、睡眠、公开出口、文件上传、账号动作与 Core 均保留独立配置及授权条件；部署是否就绪和真实链路是否确认只看当前任务中的有日期记录，不从历史计划继承。
