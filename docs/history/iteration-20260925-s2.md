...
S2-04 剩余分支与 S2-05 的会话段失效传递逐项读源码核对，结论写入[上下文·会话段在新输入、重启、取消与撤销时的边界](../context.md#会话段在新输入重启取消与撤销时的边界)。全部是源码核对结论，没有故障注入、制造竞态或运行实证。

本单元唯一的代码改动：交接区块原先对所有交接的等待登记可结束的 L 编号，比运行事实（只登记本轮相关人物的等待）范围更大；改为只有等待对象属于本轮相关人物时才登记，其余只给位置。

核对中确认但未改的点：
- 重建遇到上表以外的异常不转为换段，按原错误结束并记录；同一群会在同一处反复失败，需经授权离线移除活动段字段（运行手册已补处置说明）。这符合“不在解析异常里自动替换段”的设计约束，但会造成该群对话停摆直至人工处理。
- 被取消租约排队中的段保存由 Actor 以冲突拒绝，调用方看到的是冲突而非取消；沿用原行为。
- 供应商已接收的旧请求无法撤回，失效只作用于之后的新请求。

## 原收尾现场清单（未执行）

以下从 9 月 25 日收尾记录移入，保留原观察点，不代表本轮已执行。
