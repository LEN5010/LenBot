...
from len_bot.execution.workspace import RunPythonRequest, WorkspaceRequest, WorkspaceWorker
...
class WorkspaceService:
    ...
    async def _export_inputs(self, scope: WorkspaceScope, result_ids: list[str], scene_id: str) -> list[dict]:
        ...
        directory = self.worker.directory(scope.workspace_id) / 'input'
        ...
        for index, result_id in enumerate(dict.fromkeys(result_ids), 1):
            ...
            path.write_text(observation.content, encoding='utf-8')
            manifest.append({'result_id': result_id, 'path': f'input/result_{index}.txt',
    ...
    async def run_python(self, call, request: RunPythonInput) -> dict:
        ...
        result = await self.worker.run_python(RunPythonRequest(workspace_id=scope.workspace_id, script=request.script))
    ...
    async def read_file(self, call, request: WorkspaceFileInput) -> dict:
        ...
        return {'scene_id': scope.scene_id, 'job_id': scope.job_id, 'path': request.path,
            'content': content[request.offset:end], 'offset': request.offset, 'next_offset': end if end < len(content) else None,
    ...
    async def read_for_job(self, scene_id: str, job_id: str, path: str, offset: int, limit: int) -> dict | None:
        ...
        return {'scene_id': scene_id, 'job_id': job_id, 'path': path, 'content': content[offset:end],
