...
class StatusCardRenderer:
    """A status card for a source that did not return usable data.

    This is not a degraded schedule: it states that the schedule is unavailable
    and never claims an empty day.  It shares the configured card tokens and
    ...
    def render(self, title: str, lines: list[str], *, status_label: str) -> bytes:
        width = self.config.image_width
        scale = width / 1080
        unit = lambda pixels: round(pixels * scale)
        font_path = self.resource_directory / self.config.font_path
        fonts = {key: ImageFont.truetype(str(font_path), unit(size)) for key, size in
                 (("title", 48), ("status", 29), ("body", 27), ("footer", 19))}
        outer, inner = unit(28), unit(55)
        measure = ImageDraw.Draw(Image.new("RGB", (width, 1)))
        body_lines = [line for paragraph in lines
                      for line in wrap_lines(measure, paragraph, fonts["body"], width - inner * 2)]
        footer_lines = [line for paragraph in ("爱驼推送 / Generated by LEN · LenBot",)
                        for line in wrap_lines(measure, paragraph, fonts["footer"], width - inner * 2)]
        height = unit(260) + len(body_lines) * unit(40) + unit(40) + len(footer_lines) * unit(28) + unit(90)
        image = Image.new("RGB", (width, height), THEME.canvas)
        draw = ImageDraw.Draw(image)
        draw.ellipse((unit(-120), unit(-80), unit(420), unit(300)), fill="#efd4c2")
        draw.ellipse((width - unit(320), unit(-40), width + unit(80), unit(280)), fill="#dce8df")
        card(draw, (outer, outer, width - outer, height - outer), fill=THEME.card, outline=THEME.border,
            radius=unit(THEME.radius_outer), width=unit(2))
        draw.rounded_rectangle((outer, outer, width - outer, unit(238)), radius=unit(THEME.radius_outer), fill=THEME.soft)
        draw.text((inner, unit(62)), "A-SOUL LIVE", font=fonts["status"], fill=THEME.accent_deep)
        draw.text((inner, unit(110)), title, font=fonts["title"], fill=THEME.ink)
        top = unit(260)
        for line in body_lines:
            draw.text((inner, top), line, font=fonts["body"], fill=THEME.ink)
            top += unit(40)
        top += unit(16)
        draw.text((inner, top), status_label, font=fonts["status"], fill=THEME.accent_deep)
        top += unit(56)
        for line in footer_lines:
            draw.text((inner, top), line, font=fonts["footer"], fill=THEME.muted)
            top += unit(28)
        output = io.BytesIO()
        image.save(output, format="PNG")
        return output.getvalue()
