...
const anchors={}
const nestedRefs={}
const setAnchor=key=>element=>{
  if (element) anchors[key]=element
}
const setNestedRef=key=>element=>{
  if (element) nestedRefs[key]=element
}
...
async function focus(target) {
  ...
  if (rest.length&&nested) {
    await nested.focus(rest.join('.'));
    return
  }
...
const setRow=(key,value,index,row)=>{
  ...
  <div class="config-fields">
    <template v-for="group in groups" :key="'group:'+group.title">
      <div class="exclusive-group">
        ...
        >
          <v-btn v-for="key in group.fields" :key="key" :value="key" :disabled="disabled">
    ...
    <template v-for="field in fields" :key="field.key">
      <template v-if="visible(field.key)">
        <v-text-field
          :disabled="disabled"
          v-if="secrets.includes(path(field.key))"
          :ref="setAnchor(field.key)"
          :model-value="modelValue[field.key]"
          @update:model-value="value=>update(field.key,value)"
          :label="label(field)"
          type="password"
          autocomplete="new-password"
          :placeholder="configSet[path(field.key)]?'已保存，留空保留':'尚未配置'"
          :hint="field.schema.description"
          :error="hasProblem(field.key)"
          :error-messages="messages(field.key)"
          persistent-hint
        />
        <v-text-field
          :disabled="disabled"
          v-else-if="field.schema.const!==undefined"
          :model-value="field.schema.const"
          :label="label(field)"
          readonly
        />
        <v-select
          ...
          :model-value="modelValue[field.key]"
          @update:model-value="value=>update(field.key,value)"
          :items="enumItems(field,{nullable:field.nullable})"
          :label="label(field)"
          :hint="field.schema.description"
          :error="hasProblem(field.key)"
          :error-messages="messages(field.key)"
        ...
        <v-select
          ...
          :model-value="modelValue[field.key]"
          @update:model-value="value=>update(field.key,value)"
          :items="[{title:'是',value:true},{title:'否',value:false},...(field.nullable?[{title:'未指定',value:null}]:[])]"
          :label="label(field)"
          :hint="field.schema.description"
          :error="hasProblem(field.key)"
          :error-messages="messages(field.key)"
        ...
        <v-text-field
          :disabled="disabled"
          v-else-if="['number','integer'].includes(field.schema.type)"
          :ref="setAnchor(field.key)"
          :model-value="modelValue[field.key]"
          @update:model-value="value=>update(field.key,value===''?null:Number(value))"
          :label="label(field)"
          type="number"
        ...
        <section v-else-if="field.nested" class="nested-field">
          ...
          <PluginConfigFields
            ...
            :ref="setNestedRef(field.key)"
            :model-value="modelValue[field.key]"
            :schema="field.schema"
            :secrets="nestedSecrets(field)"
            :config-set="nestedConfigSet(field)"
            :problems="problems"
            :prefix="path(field.key)"
        ...
        <section v-else-if="field.list" class="list-field">
          ...
          <div v-if="choicesFor(field)" class="list-choices">
            <v-checkbox
              :disabled="disabled"
              v-for="choice in choicesFor(field)"
              :key="choice"
              :model-value="rows(field.key,modelValue[field.key]).includes(choice)"
              :label="choiceTitle(field,choice)"
              hide-details
              density="compact"
          ...
          <template v-else>
            <div
              v-for="(row,index) in rows(field.key,modelValue[field.key])"
              :key="index"
              class="list-row"
            >
              <v-select
                :disabled="disabled"
                v-if="field.schema.items?.enum"
                :model-value="row"
                :items="field.schema.items.enum"
                :label="`第 ${index+1} 项`"
                hide-details
                @update:model-value="value=>setRow(field.key,modelValue[field.key],index,value)"
              />
              <v-text-field
                :disabled="disabled"
                v-else-if="['number','integer'].includes(field.schema.items?.type)"
                :model-value="row"
                :label="`第 ${index+1} 项`"
                type="number"
                hide-details
                @update:model-value="value=>setRow(field.key,modelValue[field.key],index,value===''?null:Number(value))"
              />
              <v-text-field
                :disabled="disabled"
                v-else
              ...
              <div class="list-actions">
                <v-btn
                  ...
                  @click="moveRow(field.key,modelValue[field.key],index,-1)"
                >上移</v-btn>
                <v-btn
                  size="small"
                  variant="text"
                  :disabled="disabled||index===rows(field.key,modelValue[field.key]).length-1"
                  @click="moveRow(field.key,modelValue[field.key],index,1)"
                >下移</v-btn>
                <v-btn
                  size="small"
                  variant="text"
                  color="error"
                  @click="removeRow(field.key,modelValue[field.key],index)"
                  :disabled="disabled"
                >移除</v-btn>
              </div>
            </div>
            ...
            <v-btn
              size="small"
              variant="tonal"
              @click="addRow(field.key,modelValue[field.key],field)"
              :disabled="disabled"
            >添加一项</v-btn>
          </template>
        ...
        <v-textarea
          :disabled="disabled"
          v-else-if="field.json"
          :ref="setAnchor(field.key)"
          class="compound-field"
          :model-value="modelValue[field.key]"
          @update:model-value="value=>update(field.key,value)"
          :label="label(field)+' · JSON'"
          rows="4"
          auto-grow
          :hint="field.schema.description||'按下方插件 Schema 填写对象或列表；保存时校验结构。'"
          :error="hasProblem(field.key)"
          :error-messages="messages(field.key)"
          persistent-hint
        />
