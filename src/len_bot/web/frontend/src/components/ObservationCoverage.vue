...
<template>
  <section v-if="details" class="coverage-details" aria-label="浏览器与媒体实际取得范围">
    ...
    <template v-else-if="details.kind==='browser_pixels'">
      <h4>已取得的网页图片资产</h4>
      <p>页面 {{ details.page_ref }} · {{ details.area==='full_page' ? '整页区域' : '视口区域' }} · 请求核对的 DOM 版本 {{ details.requested_snapshot_revision ?? '未指定或旧记录未保存' }}
      </p>
      ...
      <ul v-if="details.frames.length">
        <li v-for="frame in details.frames" :key="frame.asset_id">
          <EntityLink
            ...
            :id="frame.asset_id"
            :scene-id="sceneId"
            :label="`查看 ${frame.source_time_ms} ms 的采样帧`"
          />
          <span>
    ...
    <template v-else-if="details.kind==='audio_transcript'">
      ...
      <p class="muted-copy">音轨区间不是每个词都已被识别的证明；具体分段时间与正文见保存资料，不自动转写未覆盖部分。</p>
    </template>
  </section>
</template>
<style scoped>
