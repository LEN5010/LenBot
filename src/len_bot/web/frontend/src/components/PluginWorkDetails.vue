...
<template>
  <section v-if="job.plugin_origin || job.legacy_payload">
    ...
    <v-alert v-if="job.plugin_issue" type="warning" variant="tonal" class="my-4">
      {{ job.plugin_issue }}
    </v-alert>
    <template
      v-for="(schema,field) in {work_parameters:job.work_parameters_schema,work_progress:job.work_progress_schema}"
      :key="field"
    >
      ...
      <dl v-if="job[field]" class="business-fields">
        <template v-for="(value,key) in job[field]" :key="key">
          <dt>{{ label(schema,key) }}</dt>
          <dd>
            <EntityLink
              v-if="value && ['tool-result-id','media-id'].includes(resourceFormat(schema,key))"
              :type="resourceFormat(schema,key)==='media-id'?'media':'result'"
              :id="value"
              :scene-id="job.scene_id"
              :label="label(schema,key)"
            />
            <div v-else-if="resourceFormat(schema,key)==='tool-result-list'" class="resource-links">
              <EntityLink
                ...
                :label="`资料 ${index+1}`"
              />
              <span v-if="!value.length">尚无</span>
            </div>
            <template v-else>{{ display(value,schema,key) }}</template>
            <small v-if="description(schema,key)">{{ description(schema,key) }}</small>
          </dd>
        </template>
      </dl>
    </template>
...
.business-fields{display:grid;grid-template-columns:160px minmax(0,1fr);gap:10px 18px;margin:14px 0 22px}
.business-fields dt{color:rgb(var(--v-theme-on-surface-variant))}
.business-fields dd{margin:0;white-space:pre-wrap;overflow-wrap:anywhere}
.business-fields small{display:block;margin-top:4px;color:rgb(var(--v-theme-on-surface-variant))}
@media(max-width:650px){
  .business-fields{grid-template-columns:1fr;gap:4px}
  .business-fields dd{margin-bottom:10px}
}
</style>
