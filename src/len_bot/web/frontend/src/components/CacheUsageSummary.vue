  ...
  <section class="cache-summary" aria-label="缓存命中与统计覆盖">
    ...
    <template v-if="cache">
      <dl>
        ...
        <div>
          <dt>调用覆盖率</dt>
          <dd>
            {{ percent(cache.call_coverage) }}（{{ count(cache.cache_reported_calls) }} / {{ count(cache.calls) }}）</dd>
        </div>
    ...
    <template v-if="phases?.length">
      ...
      <dl v-for="phase in phases" :key="phase.phase">
        <div>
          <dt>{{ {first:'绑定内首次准备',followup:'绑定内后续准备',unknown:'顺序未记录'}[phase.phase] }}</dt>
          <dd>{{ count(phase.calls) }} 次调用</dd>
        </div>
        <div>
          <dt>平均输入 token（仅有效报告）</dt>
          <dd>{{ count(phase.mean_input_tokens) }}（{{ count(phase.known_input_calls) }} 次）</dd>
        </div>
        <div>
          <dt>缓存命中率（按输入量加权）</dt>
          <dd>{{ percent(phase.cache_hit_rate) }}（{{ count(phase.cache_reported_calls) }} 次有报告）</dd>
        </div>
      </dl>
...
<style scoped>
.cache-summary{margin:18px 0;padding:16px;border:1px solid var(--line);border-radius:8px;min-width:0}
.cache-summary h3{font-size:14px;margin:0 0 12px}
.cache-summary dl{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;font-size:12px}
.cache-summary dt,.cache-summary p{color:var(--muted)}
.cache-summary dd{font-size:14px;margin:6px 0 0;overflow-wrap:anywhere}
.cache-summary p{font-size:12px;line-height:1.7;margin:12px 0 0}
@media(max-width:700px){
  .cache-summary dl{grid-template-columns:minmax(0,1fr)}
}
</style>
