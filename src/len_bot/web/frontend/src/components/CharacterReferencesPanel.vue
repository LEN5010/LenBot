...
async function load({ accept = () => true } = {}) {
  ...
  loading.value = true;
  error.value = '';
  conflicts.beginRead('references')
  ...
  try {
    ...
    if (fresh()) {
      ...
      conflicts.readFailed('references', problem)
    }
  } finally {
...
const assetLink = asset => ({
  ...
  query:{
    ...route.query,
    list_scene:route.query.list_scene || route.query.scene || 'global-safe',
    scene:asset.scope,
    id:asset.id
  }
})
...
<template>
  <v-card class="pa-4" id="character-references">
    <div class="reference-heading">
      <div>
        <h2 class="text-h6">人物与服装参考</h2>
        <p class="muted">
          {{ snapshot ? `已保存 ${snapshot.configured.length} 项` : '尚未读取' }} · 平时提供定位，辨认时按需读图，独立于固定表情目录。</p>
      </div>
      <v-btn
        variant="text"
        :aria-expanded="expanded"
        aria-controls="character-reference-form"
        @click="expanded=!expanded"
      >
        {{ expanded ? '收起' : '管理绑定' }}{{ dirty ? ' · 有未保存修改' : '' }}
      </v-btn>
    </div>
    ...
    <div v-show="expanded" id="character-reference-form" class="reference-form mt-4">
      ...
      <v-alert v-if="uncertain" type="warning" variant="tonal">
        ...
        <template v-if="uncertain.current">
          ...
          <v-btn variant="text" :disabled="loading||saving" @click="adoptUncertain">采用当前值重新编辑</v-btn>
        </template>
      </v-alert>
      <ConfigConflictBanner
        :conflict="conflict?.problem"
        :current="conflict?.snapshot"
        :path-label="conflict?.problem.path.join(' → ')"
        :read-at="conflict?.readAt"
      ...
      >
        <template #current>
          <ResourceViewer :content="conflict?.snapshot?.saved" title="当前已保存的绑定列表" />
        </template>
      </ConfigConflictBanner>
      <v-form v-if="baseline" :disabled="locked" class="reference-form" @submit.prevent="save">
        <v-card v-for="(row,index) in rows" :key="index" variant="outlined" class="pa-3">
          <div class="reference-fields">
            <v-text-field
              v-model="row.character_key"
              label="人物键"
              hint="小写字母开头，可含数字、短横线与下划线"
              persistent-hint
              maxlength="32"
            />
          ...
          <div v-if="assetFor(row)" class="reference-asset mt-3">
            <div class="reference-preview">
              <MediaPreview :asset="assetFor(row)" :scene-id="assetFor(row).scope" />
            </div>
            <div>
        ...
        <v-btn
          ...
          color="primary"
          :loading="saving"
          :disabled="locked||!!error||!dirty||!valid"
        >保存参考绑定</v-btn>
      </v-form>
      <template v-if="snapshot">
        <div
          v-for="item in snapshot.configured.filter(item=>item.issue)"
          :key="item.asset_id"
          class="text-warning"
...
<style scoped>
.reference-heading,.reference-actions{display:flex;gap:12px;align-items:center;justify-content:space-between;flex-wrap:wrap}
.reference-actions{justify-content:flex-start}
.reference-form{display:grid;gap:16px}
.reference-form>.v-btn{justify-self:start}
...
@media(max-width:600px){
  ...
  .reference-heading>.v-btn{margin-left:auto}
}
</style>
