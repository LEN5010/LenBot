  ...
  <section class="trace-timings" aria-label="已保存的阶段耗时">
    ...
    <article v-for="run in timings?.runs || []" :key="run.index" class="timing-run">
      ...
      <dl class="timing-facts">
        <div v-if="run.cognition_slot_wait_ms !== null && run.cognition_slot_wait_ms !== undefined">
          <dt>对话执行槽位等待</dt>
          <dd>{{ duration(run.cognition_slot_wait_ms) }}</dd>
        </div>
        <div v-if="run.work_slot_wait_ms !== null && run.work_slot_wait_ms !== undefined">
          <dt>工作执行槽位等待</dt>
          <dd>{{ duration(run.work_slot_wait_ms) }}</dd>
        </div>
        <div v-if="run.work_selection_ms !== null && run.work_selection_ms !== undefined">
          <dt>本群工作目录读取与选择</dt>
          <dd>{{ duration(run.work_selection_ms) }}</dd>
        </div>
        <div
          v-if="run.maintenance_slot_wait_ms !== null && run.maintenance_slot_wait_ms !== undefined"
        >
          <dt>方法维护槽位等待</dt>
          <dd>{{ duration(run.maintenance_slot_wait_ms) }}</dd>
        </div>
        <div v-if="run.agent_lock_wait_ms !== null && run.agent_lock_wait_ms !== undefined">
          <dt>插件子调用串行锁等待</dt>
          <dd>{{ duration(run.agent_lock_wait_ms) }}</dd>
        </div>
        <div
          v-for="(wait,index) in run.publication_waits"
          :key="`${wait.commit_event_id || 'missing'}:${index}`"
        >
          <dt>第 {{ index + 1 }} 次发布串行锁申请</dt>
          <dd>
            {{ duration(wait.elapsed_ms) }} · {{ publicationWaitStates[wait.state] || wait.state }}<EntityLink
              v-if="wait.commit_event_id"
              type="event"
              :id="wait.commit_event_id"
              :scene-id="sceneId"
              label="查看对应提交"
            />
          </dd>
      ...
      <details v-if="run.steps.length">
        ...
        <article v-for="(step, index) in run.steps" :key="index" class="timing-step">
          ...
          <ul v-if="step.tools.length">
            <li v-for="(tool, toolIndex) in step.tools" :key="toolIndex">
              {{ tool.name || '工具名未记录' }} · 执行 {{ duration(tool.execution_ms) }}<span v-if="tool.id"> · {{ tool.id }}</span>
            </li>
          </ul>
        </article>
      </details>
...
<style scoped>
.trace-timings{min-width:0;margin:16px 0}
.trace-timings h4{font-size:13px;margin:0 0 8px}
.trace-timings h5{font-size:12px;margin:0}
.timing-note{font-size:12px;color:var(--muted);line-height:1.7;overflow-wrap:anywhere}
.timing-run{border-top:1px solid var(--line);padding:14px 0}
.timing-facts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;font-size:12px;margin:12px 0}
.timing-facts dt{color:var(--muted)}
.timing-facts dd{margin:4px 0 0;overflow-wrap:anywhere}
.trace-timings summary{font-size:12px;cursor:pointer}
.timing-step{border-top:1px solid var(--line);padding:12px 0;margin-top:10px;font-size:12px}
.timing-step strong{display:block;margin-bottom:6px}
.timing-step ul{padding-left:18px;line-height:1.8;overflow-wrap:anywhere}
.timing-step li span{color:var(--muted)}
@media(max-width:600px){
  .timing-facts{grid-template-columns:minmax(0,1fr)}
}
</style>
