  ...
  <section ref="container" class="scene-inspector" :role="modal ? 'dialog' : 'region'" :aria-modal="modal && active ? 'true' : undefined" tabindex="-1" aria-label="消息关联检查" @keydown="trapFocus" @keydown.esc.stop.prevent="$emit('close')">
    ...
    <div class="inspector-body">
      ...
      <template v-if="event">
        ...
        <article v-for="turn in readTurns" :key="turn.id" class="relation-record"><div class="inspector-badges"><StatusBadge domain="attention" status="read" /><StatusBadge v-if="handledStatus(turn)" domain="attention" :status="handledStatus(turn)" /></div><p v-if="!handledStatus(turn)" class="muted-copy">本次提交没有为这条原话记录处理结果；已提供观察不等于请求已完成。</p><p class="preserve-lines">本轮整体决定：{{ turn.payload.outcome?.decision_reason || '未记录' }} <StatusBadge v-if="turn.payload.outcome?.disposition" domain="attention" :status="disposition(turn.payload.outcome.disposition)" /></p><SourceOutcomes :items="turn.payload.source_outcomes" :scene-id="event.scene_id" /><EntityLink type="event" :id="turn.id" :scene-id="event.scene_id" label="查看已提交的本轮结果" /></article>
