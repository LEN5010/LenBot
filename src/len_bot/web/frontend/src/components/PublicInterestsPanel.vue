...
async function load() {
  ...
  if (loadedSelection.value !== target) { detail.value = null; data.value = null; readAt.value = null }
  ...
  try {
    ...
    loadedSelection.value = target; readAt.value = value.read_at
...
<template>
  <section class="interests">
    ...
    <v-alert v-if="error" type="error" variant="tonal">{{ error }}<span v-if="readAt"> · 保留上次读取 {{ fmtTime(readAt) }} 的内容</span></v-alert><v-progress-linear v-if="loading" indeterminate />
    ...
    <template v-if="detail">
      <v-card class="pa-4"><h3>{{ detail.topic }}</h3><p>{{ types[detail.record_type] }} · v{{ detail.revision }} · {{ statuses[detail.status] || detail.status }}{{ detail.expired ? ' · 当前已过期' : '' }}</p><p class="copy">{{ detail.statement }}</p><p>记录的发布时间 {{ fmtTime(detail.published_at) }}（不是群发送时间） · 观察 {{ fmtTime(detail.observed_at) }} · 有效至 {{ detail.valid_until === null ? '未设定' : fmtTime(detail.valid_until) }}</p><p v-if="readAt">详情读取于 {{ fmtTime(readAt) }}</p><v-alert :type="detail.public_sources_confirmed ? 'info' : 'warning'" variant="tonal">{{ detail.public_sources_confirmed ? '来源链具有匿名公共资料证明。' : '来源链未全部确认，不能作为已验证公共资料使用。' }} 来源关联不代表后续调用已呈现正文或图像，也不单独证明结论正确。</v-alert><ResourceViewer title="采用时的具体证据范围" :content="detail.evidence_spans" /></v-card>
      <h3>原始资料来源</h3><v-card v-for="source in detail.sources" :key="source.id" class="pa-4"><EntityLink type="result" :id="source.id" :scene-id="source.scene_id" /><EntityLink v-if="source.event_id" type="event" :id="source.event_id" :scene-id="source.scene_id" label="取得资料的事件" /><ObservationDetails :observation="source.result" :scene-id="source.scene_id" /></v-card>
      ...
      <v-card v-for="publication in detail.publications || []" :key="publication.scene_id + ':' + publication.action_id" class="pa-4 publication">
        <div class="actions"><EntityLink type="scene" :id="publication.scene_id" :scene-id="publication.scene_id" /><StatusBadge domain="delivery" :status="publication.status" /><span>当时采用版本 {{ publication.interest_revision == null ? '未记录' : `v${publication.interest_revision}` }}</span></div>
        <code>{{ publication.action_id }}</code><p>终态回执时间 {{ fmtTime(publication.receipt_at) }}<span v-if="publication.status==='sent' && publication.message_id"> · 平台消息 ID {{ publication.message_id }}</span></p>
        <p v-if="publication.error" class="copy">{{ publication.error }}</p>
        <div class="publication-links"><EntityLink v-for="eventId in publication.attempt_event_ids" :key="eventId" type="event" :id="eventId" :scene-id="publication.scene_id" label="查看原发送尝试" /><EntityLink v-for="eventId in publication.receipt_event_ids" :key="eventId" type="event" :id="eventId" :scene-id="publication.scene_id" label="查看原回执" /></div>
      </v-card>
      <p v-if="!detail.publications?.length">没有已保存的发布尝试或回执；这不代表没有候选或没有被考虑过。</p>
      <h3>最近修订（最多 50 条）</h3><p v-if="detail.more_changes">仍有更早修订，可按下列来源场景进入运行记录查看。</p><v-card v-for="change in detail.changes" :key="change.id" class="pa-4"><EntityLink type="event" :id="change.id" :scene-id="change.scene_id" :label="fmtTime(change.timestamp)" /><EntityLink v-if="change.payload.job_id" type="job" :id="change.payload.job_id" :scene-id="change.scene_id" label="采用此修订的工作" /><ResourceViewer title="修订内容与原因" :content="change.payload" /></v-card>
    </template>
  </section>
</template>

<style scoped>
.publication{min-width:0}.publication code{overflow-wrap:anywhere}.publication-links{display:flex;gap:8px 16px;flex-wrap:wrap}
