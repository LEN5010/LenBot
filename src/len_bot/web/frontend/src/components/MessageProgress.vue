...
<template>
  <section v-if="applicable" class="message-progress" aria-label="消息处理阶段">
      ...
      <li v-for="(item,index) in progress.steps" :key="item.name">
        ...
        <div class="step-body">
          <div class="step-heading">
            <h4>{{ item.name }}</h4>
            <v-chip size="x-small" variant="tonal" :color="states[item.state].color">
              {{ states[item.state].text }}
            </v-chip>
          </div>
          <strong class="step-summary">{{ item.summary }}</strong>
          <p v-if="item.detail">{{ item.detail }}</p>
          <EntityLink
            v-if="index===2 && progress.handlingTurn"
            type="event"
            :id="progress.handlingTurn.event_id"
            :scene-id="event.scene_id"
            label="查看对应处理提交"
          />
          <template v-if="index===2 && progress.outcome">
            <p
              v-for="(unfinished, unfinishedIndex) in progress.outcome.unfinished || []"
              :key="unfinishedIndex"
            >未完成：{{ unfinished }}
            </p>
          ...
          <template v-if="index===3">
            <div v-for="job in progress.jobs" :key="job.id" class="progress-work">
              ...
              <p>工作当前版本 v{{ job.revision }}，不是答复当时采用版本。</p>
            </div>
            <details v-if="!progress.isReceipt && progress.actions.length">
              <summary>查看对应表达的依据（{{ progress.actions.length }}）</summary>
              <article v-for="(action,position) in progress.actions" :key="action.id">
                <p>关联表达 {{ position+1 }}</p>
                <AnswerBasisDetails :basis="action.answer_basis" :scene-id="event.scene_id" />
                <EntityLink
                  v-if="action.commit_event_id"
                  type="event"
                  :id="action.commit_event_id"
                  :scene-id="event.scene_id"
                  label="查看本条表达提交"
                />
              </article>
          ...
          <details
            v-if="index===5 && progress.actions.some(action=>action.receipt_event_ids?.length)"
          >
            <summary>查看对应回执</summary>
            <div
              v-for="(action,position) in progress.actions"
              :key="action.id"
          ...
          <template v-if="index===5 && progress.deliveryProblems.length">
            ...
            <article
              ...
              :key="receipt.id"
              class="delivery-problem"
            >
              <div class="step-heading">
                <StatusBadge domain="delivery" :status="receipt.delivery_status" />
                <StatusBadge v-if="receipt.simulated" domain="delivery" status="simulated" />
                <span>{{ fmtTime(receipt.timestamp) }}</span>
              </div>
              <p>{{ receipt.payload.error }}</p>
              <EntityLink
                type="event"
                :id="receipt.id"
                :scene-id="event.scene_id"
    ...
    <p
      v-else-if="['GROUP_MESSAGE_RECEIVED','PRIVATE_MESSAGE_RECEIVED'].includes(event.event_type) && relations"
      class="progress-note"
    ...
    <section class="progress-calls" aria-label="关联处理与发送耗时">
      ...
      <article v-for="receipt in progress.deliveryReceipts" :key="receipt.id" class="progress-call">
        <div class="step-heading">
          <StatusBadge domain="delivery" :status="receipt.delivery_status" />
          <StatusBadge v-if="receipt.simulated" domain="delivery" status="simulated" />
          <time>{{ fmtTime(receipt.timestamp) }}</time>
        </div>
        <dl class="call-metrics">
          ...
          <div>
            <dt>原记录起点至本次回执</dt>
            <dd>{{ formatDurationMs(receipt.payload.event_to_delivery_ms) }}</dd>
          </div>
        </dl>
    ...
    <section class="progress-calls" aria-label="关联轮次的模型调用">
      ...
      <article v-for="call in progress.calls" :key="call.id" class="progress-call">
        <div class="step-heading">
          ...
          <time>{{ fmtTime(call.started_at) }}</time>
        </div>
        ...
        <dl class="call-metrics">
          ...
          <div>
            <dt>其中缓存输入 tokens</dt>
            <dd>{{ number(call.usage?.prompt_tokens_details?.cached_tokens) }}</dd>
          </div>
          <div><dt>本地估算输入 tokens</dt><dd>{{ number(call.estimate?.input_tokens) }}</dd></div>
          <div v-if="call.usage?.type==='duration'">
            <dt>供应商音频用量</dt>
            <dd>{{ number(call.usage.seconds) }} 秒</dd>
          </div>
        </dl>
        ...
        <details>
          ...
          <div
            ...
            :key="`${request.trace_id}:${index}`"
            class="progress-links"
          >
            <p class="progress-note">消息位置 {{ number(request.message_count) }} · 标记省略 {{ number(request.omitted_message_count) }} · 图像资产 {{ number(request.pixel_asset_count) }} · 工具定义 {{ number(request.tool_count) }}
            </p>
            <EntityLink
              type="trace"
              :id="request.trace_id"
        ...
        <div class="progress-links">
          <EntityLink type="call" :id="call.id" :scene-id="event.scene_id" label="查看调用与登记材料" />
          <EntityLink
            type="episode"
            :id="call.episode_id"
            :scene-id="event.scene_id"
    ...
    <details v-if="progress.problems.length" class="progress-problems">
      ...
      <article v-for="problem in progress.problems" :key="problem.id">
        ...
        <p v-if="problem.error">{{ failureLabel(problem.error_type) }}：{{ problem.error }}</p>
        <p v-if="problem.publication_error">发布步骤 {{ problem.publication_phase || '未记录' }}：{{ problem.publication_error }}
        </p>
        <EntityLink type="trace" :id="problem.id" :scene-id="event.scene_id" label="查看这次轮次的问题与调用" />
      </article>
    </details>
  </section>
</template>
...
<style scoped>
.message-progress{min-width:0;margin:18px 0}
.message-progress h3{font-size:15px;margin:0 0 10px}
.progress-note{font-size:12px;color:var(--muted);line-height:1.7;margin:8px 0}
.progress-steps{list-style:none;padding:0;margin:16px 0}
.progress-steps>li{display:flex;gap:12px;padding:0 0 18px;min-width:0}
.step-number{display:grid;place-items:center;flex:none;width:24px;height:24px;border:1px solid var(--line);border-radius:50%;font-size:12px;color:var(--muted)}
.step-body{min-width:0;flex:1}
.step-heading{display:flex;gap:8px;flex-wrap:wrap;align-items:center}
.step-heading h4{font-size:12px;margin:0;color:var(--muted)}
.step-summary{display:block;font-size:13px;line-height:1.7;margin-top:6px;overflow-wrap:anywhere}
.step-body p{font-size:12px;color:var(--muted);line-height:1.7;white-space:pre-wrap;overflow-wrap:anywhere;margin:6px 0}
.step-body :deep(.entity-link){font-size:12px}
.message-progress summary{cursor:pointer;font-size:12px;line-height:1.7}
.message-progress details{margin-top:10px}
.progress-work,.progress-problems article{border-top:1px solid var(--line);padding:12px 0;margin-top:10px;font-size:12px;min-width:0}
.progress-work .step-heading{margin-top:8px}
.progress-links{display:grid;gap:8px;margin:10px 0;min-width:0}
.progress-problems p{white-space:pre-wrap;overflow-wrap:anywhere;font-size:12px;line-height:1.7}
.progress-problems strong{font-size:12px}
.delivery-problem{border-left:2px solid var(--line);padding-left:12px;margin:10px 0;font-size:12px}
.delivery-problem .step-heading>span{color:var(--muted)}
.progress-batches{margin:0 0 18px;border-top:1px solid var(--line);padding-top:12px;font-size:12px}
.progress-batches ul{padding-left:18px}
.progress-batches li{margin:10px 0;line-height:1.7;overflow-wrap:anywhere}
.progress-batches code{overflow-wrap:anywhere}
.progress-batches li p{margin:4px 0;color:var(--muted)}
.progress-calls{border-top:1px solid var(--line);padding-top:16px}
.progress-calls h4{font-size:13px;margin:0}
.progress-calls h4 span,.progress-call time{color:var(--muted);font-weight:400}
.progress-call{border-top:1px solid var(--line);padding:14px 0;font-size:12px;min-width:0}
.call-metrics{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin:12px 0}
.call-metrics dt{color:var(--muted);font-size:11px}
.call-metrics dd{margin:4px 0 0;overflow-wrap:anywhere}
.call-error{color:rgb(var(--v-theme-error));white-space:pre-wrap;overflow-wrap:anywhere}
.progress-call .progress-note{overflow-wrap:anywhere}
.progress-call :deep(.entity-link){font-size:12px}
@media(max-width:600px){
  .call-metrics{grid-template-columns:minmax(0,1fr)}
}
</style>
