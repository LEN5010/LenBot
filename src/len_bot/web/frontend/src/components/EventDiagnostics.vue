...
function download() {
  ...
  link.href = url; link.download = 'lenbot-event-diagnostics.json'
  link.click()
...
<template>
  <section class="event-diagnostics" aria-label="只读诊断材料">
    ...
    <template v-if="record">
      ...
      <details><summary>预览导出 JSON</summary><ResourceViewer title="诊断材料（无正文）" :content="record" /></details>
...
<style scoped>
.event-diagnostics{border-top:1px solid var(--line);padding:16px 0;margin-top:16px;min-width:0}.event-diagnostics h3{font-size:14px;margin:0 0 8px}.event-diagnostics p{font-size:12px;line-height:1.7;color:var(--muted);overflow-wrap:anywhere}.event-diagnostics summary{font-size:12px;cursor:pointer}.event-diagnostics details{margin:12px 0}
</style>
