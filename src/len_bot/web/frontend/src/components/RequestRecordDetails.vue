  ...
  <section class="request-record" aria-label="调用登记时的请求材料">
    ...
    <template v-else-if="supported">
      ...
      <dl class="request-facts">
        ...
        <div>
          <dt>图像块</dt>
          <dd>{{ images.length }}；{{ images.filter(image => !image.asset_id).length }} 个缺少资产定位</dd>
        </div>
      ...
      <template v-if="[2, 3, 4, 5].includes(record.format_version)">
        ...
        <details
          ...
          :key="`${component.messageIndex}:${component.component_id}`"
        >
          <summary>位置 {{ component.messageIndex + 1 }} · {{ component.component_id }} · 修订 {{ component.revision }} · {{ component.status === 'retained' ? '已留存' : '声明后内容有变化，未留存' }}
          </summary>
      ...
      <div class="request-table-wrap">
        <table>
          ...
          <thead>
            <tr>
              <th scope="col">位置 / 角色</th>
              <th scope="col">类别</th>
              <th scope="col">来源与范围</th>
              <th scope="col">省略 / 图像</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="message in messages.slice(0,visibleCount)" :key="message.index">
              <th scope="row">{{ message.index + 1 }} · {{ message.role }}</th>
              <td>{{ message.section || '未记录类别' }}</td>
              <td>
                <EntityLink
                  v-if="message.event_id"
                  type="event"
                  :id="message.event_id"
                  :scene-id="sceneId"
                  label="查看原始事件"
                />
                <span v-else>未记录直接事件来源</span>
                <p v-if="message.text_range">原文字符 [{{ message.text_range.start }}, {{ message.text_range.end }}) / {{ message.text_range.total }}
                </p>
                ...
                <ul v-if="message.tool_presentations?.length">
                  <li v-for="(page,pageIndex) in message.tool_presentations" :key="pageIndex">
                    <EntityLink
                      type="result"
                      :id="page.result_id"
                      :scene-id="sceneId"
                      ...
                      label="查看本次保留的资料范围"
                    />
                    <p>[{{ page.start }}, {{ page.end }}) / {{ page.total }} · {{ page.coordinate_unit === 'records' ? '记录坐标' : '字符坐标' }}<span v-if="page.evidence_ref"> · {{ page.evidence_ref }}</span>
                    </p>
                  </li>
                </ul>
                ...
                >
                  <li v-for="item in message.result_locators" :key="item.ref">
                    {{ item.ref }} · {{ item.tool || '原工具未记录' }} · <EntityLink
                      v-if="item.result_id"
                      type="result"
                      :id="item.result_id"
                      :scene-id="sceneId"
                ...
                >
                  <li v-for="item in message.summary_refs" :key="item.batch_id">
              批次 <code>{{ item.batch_id }}</code> · 版本 {{ item.generation_version }} · 覆盖 [{{ item.range[0] }}:{{ item.range[1] }}, {{ item.range[2] }}:{{ item.range[3] }})
            </li>
                </ul>
              </td>
              <td>
                {{ message.omitted === null ? '省略状态未记录' : message.omitted ? '标记省略' : '未标记省略' }}<p v-if="message.omitted">{{ message.omission_reason || '省略原因未单独记录' }}</p>
                <p>图像块 {{ message.images.length }}</p>
                <details v-if="message.images.length" class="request-images">
                  <summary>消息 {{ message.index + 1 }} 的图像定位</summary>
                  <ul>
                    <li v-for="image in message.images" :key="image.part_index">
      ...
      >继续显示 25 个位置（已显示 {{ visibleCount }} / {{ messages.length }}）</v-btn>
      <details :open="rawOpen" @toggle="rawOpen=$event.target.open">
        <summary>完整定位字段</summary>
        <ResourceViewer v-if="rawOpen" title="该调用已保存的材料记录" :content="record" />
      </details>
    </template>
...
<style scoped>
.request-record{min-width:0}
.request-record h3{font-size:15px;margin:0 0 12px}
.request-note{font-size:12px;color:var(--muted);line-height:1.7;overflow-wrap:anywhere}
.request-facts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;font-size:12px}
.request-facts dt{color:var(--muted)}
.request-facts dd{margin:4px 0 0;overflow-wrap:anywhere}
.request-record summary{cursor:pointer;font-size:12px;line-height:1.7}
.request-record details{margin:12px 0}
.request-tools{font-size:12px;line-height:1.7;overflow-wrap:anywhere}
.request-table-wrap{overflow-x:auto;margin-top:12px}
.request-record table{width:100%;border-collapse:collapse;font-size:12px;text-align:left}
.request-record caption{text-align:left;color:var(--muted);padding:8px 0}
.request-record th,.request-record td{padding:10px 8px;border-bottom:1px solid var(--line);vertical-align:top;overflow-wrap:anywhere;min-width:90px}
.request-record td p{margin:5px 0 0;color:var(--muted)}
.request-record th{font-weight:500}
.request-record :deep(.entity-link){max-width:180px;font-size:12px}
@media(max-width:600px){
  .request-facts{grid-template-columns:minmax(0,1fr)}
}
.request-images summary{padding:2px 0}
.request-images ul{margin:8px 0;padding-left:16px}
.request-images li+li{margin-top:8px}
.request-images code{white-space:normal;overflow-wrap:anywhere}
</style>
