...
<template>
  <v-card class="memory-index">
    <v-card-text>
      ...
      <v-alert v-if="readError" type="error" variant="tonal" class="my-3">索引状态读取失败：{{ readError }}<div v-if="readAt">保留读取于 {{ fmtTime(readAt) }} 的状态；这不代表认识列表读取失败。</div>
      </v-alert>
      ...
      <template v-if="state">
        ...
        <details v-if="state.last_error" class="index-record">
          <summary>最近保存的索引错误（历史记录，不代表当前仍失败）</summary>
