...
const readGuard = useRequestGuard(selection),
  factsGuard = useRequestGuard(selection),
  operationGuard = useRequestGuard(selection)
const focusGuard = useRequestGuard(selection),
  pluginPanels = ref({}),
  pluginRows = new Map(),
  pluginFields = new Map()
...
async function focusPlugin(id, path) {
  const fresh=focusGuard();
  pluginPanels.value[id]=0
  await nextTick()
  if(!fresh())return
...
function adoptRecord(source) {
  ...
  sendFile.value=principals;
  sendFileOriginal.value=JSON.stringify(principals)
  extraUid.value='';
  pluginProblems.value={}
}
function clearConflict() {
  conflict.value=null;
  conflictCurrent.value=null;
  conflictReadError.value='';
  conflictReadAt.value=null
}
...
function inheritAttention() {
  draft.value.attention = null
}
const sticker = computed({
  get: () => draft.value?.expression?.sticker_preference || 'inherit',
  set: value => {
    draft.value.expression = value==='inherit' ? null : {sticker_preference: value}
  },
})
...
async function load({ accept = () => true } = {}) {
  ...
  if(saveOutcome.value){
    outcomeRecord.value=null;
  ...
  try {
    ...
    if(conflict.value){
      conflictCurrent.value=data;
      conflictReadAt.value=readAt.value
    }
    else if(saveOutcome.value){
      outcomeRecord.value=data;
      outcomeReadAt.value=readAt.value
    }
    else if (!dirty.value) adoptRecord(data)
  } catch(e) {
    if(fresh()){
      error.value=e.message;
      if(conflict.value)conflictReadError.value=e.message
    }
  }
  finally {
    if (fresh()) {
      loading.value=false;
      emit('loaded', {sceneId:id,ok:!error.value})
    }
  }
}
async function loadRuntimeFacts({ accept = () => true } = {}) {
  ...
  try {
    ...
    if (fresh()) factsError.value = problem.message
  }
  finally {
    if (fresh()) factsLoading.value = false
  }
}
function refresh() {
  if(!saving.value)return Promise.all([load(), loadRuntimeFacts()])
}
...
function keepMine() {
  ...
  try {
    const source=conflictCurrent.value,
      savedDraft=makeDraft(source.settings,source),
      principals=filePrincipals(source)
    const previous=JSON.parse(original.value),
      next=rebaseConfigDraft(previous,draft.value,savedDraft)
...
function takeCurrent() {
  ...
  try{
    ...
    outcomeReadAt.value=null;
    error.value='';
    message.value='已采用本次读取的保存值，没有再次保存、重试运行应用或启用。'
  }catch(e){
    error.value=e.message
  }
}
  ...
  <div class="scene-settings">
    ...
    <template v-else>
      ...
      <div v-if="record" class="status-row mb-4">
        <v-chip
          size="small"
          :color="record.joined?'success':record.joined===false?'warning':'default'"
        >
          {{ record.joined?'已加入':record.joined===false?'待确认加入':'加入状态未知' }}
        </v-chip>
        <v-chip size="small" :color="record.configured?'primary':'default'">
          {{ record.configured?'已保存':'未配置' }}
        </v-chip>
        <v-chip v-if="dirty" size="small" color="warning">有未保存草稿</v-chip>
        <span class="muted-copy">群号 {{ sceneId.replace('group:','') }}</span>
      </div>
      <v-alert v-if="record?.discovery?.error" type="warning" variant="tonal" class="mb-4">群列表读取失败，保留已知列表：{{ record.discovery.error }}<span v-if="record.discovery.sampled_at"> · 样本 {{ fmtTime(record.discovery.sampled_at) }}
        </span>
      </v-alert>
      <ConfigConflictBanner
        ...
        :path-label="conflict?.path?.join('.') || '本群设置或文件授予'"
        :busy="saving||loading||!!saveOutcome"
        :read-error="conflictReadError"
        :read-at="conflictReadAt"
        @keep="keepMine"
        @take="takeCurrent"
        @reload="refresh"
      />
      <v-alert v-if="error" type="error" variant="tonal" class="mb-4">
        {{ error }}<div v-if="readAt">上次读取 {{ fmtTime(readAt) }}</div>
      </v-alert>
      <v-alert v-if="pluginErrors.length" type="error" variant="tonal" class="mb-4">
        <p>点击具体字段定位并修正；修正前没有提交保存。</p>
        <ul class="field-errors">
          <li v-for="item in pluginErrors" :key="item.id+item.key">
            <button type="button" :disabled="saving" @click="focusPlugin(item.id,item.key)">
              {{ pluginById[item.id]?.name || item.id }} · {{ item.message }}
            </button>
          </li>
        </ul>
      </v-alert>
        ...
        <v-expansion-panel title="当前运行、能力缺项与最近结果">
          <v-expansion-panel-text>
            ...
            <v-btn
              variant="text"
              class="mt-3"
              :to="withReturn(route,{name:'capabilities',query:{scene:sceneId}})"
            >选择本群发起者，核对其使用资格</v-btn>
          </v-expansion-panel-text>
        </v-expansion-panel>
      </v-expansion-panels>
      <template v-if="record">
        <v-alert type="info" variant="tonal" class="mb-4">
          {{ dirty?'未保存草稿：':'已保存规则：' }}{{ draftEffect }}
        </v-alert>
        <v-btn
          v-if="!draft&&record.settings===null"
          color="primary"
          variant="tonal"
          :disabled="saving||loading||!!saveOutcome"
          @click="beginConfiguration"
        >为本群填写设置</v-btn>
        <v-form v-if="draft" :disabled="saving||!!saveOutcome" @submit.prevent="save">
          <section class="config-block">
            <h4>本群参与</h4>
            <div class="settings-actions mb-3">
              <v-btn variant="tonal" :disabled="saving||!!saveOutcome" @click="setMode('chat')">填为聊天群</v-btn>
              <v-btn variant="tonal" :disabled="saving||!!saveOutcome" @click="setMode('listen')">填为跟读群</v-btn>
              <v-btn
                variant="tonal"
                :disabled="saving||!!saveOutcome"
            ...
            <div class="settings-grid">
              ...
              <v-switch
                v-model="draft.listen"
                :disabled="draft.chat"
                label="不聊天时仍跟读本群"
                color="primary"
                :hint="draft.chat?'聊天已包含跟读':'不回话，但持续总结成历史与记忆'"
                persistent-hint
              />
              ...
              <AdvancedSection title="仅本群覆盖旁听参数" note="会让本群偏离统一聊天参数">
                ...
                <div class="settings-actions">
                  <v-btn
                    ...
                    :disabled="saving||!!saveOutcome"
                    @click="applyRaise"
                  >按已保存值开启并将观察间隔减半</v-btn>
                  <v-btn
                    size="small"
                    variant="text"
                    :disabled="saving||!!saveOutcome"
                    @click="inheritAttention"
                  >恢复继承全局</v-btn>
                </div>
                <div v-if="draft.attention" class="form-grid mt-3">
                  <v-select
                    v-model="draft.attention.observation_enabled"
                    label="本群普通周期观察"
                    :items="[{title:'继承全局',value:null},{title:'开启',value:true},{title:'关闭',value:false}]"
                  />
                  <v-text-field
                    v-model.number="draft.attention.observation_interval_seconds"
                    type="number"
                    min="0.1"
                    step="0.1"
                    label="本群观察间隔（秒）"
                  />
                  <v-text-field
                    v-model.number="draft.attention.keyword_cooldown_seconds"
                    type="number"
                    min="0"
                    step="1"
                    label="名称与关键词提速冷却（秒）"
                ...
                <v-alert
                  v-if="record.attention.raise_two_steps.enables_observation"
                  type="warning"
                  variant="tonal"
                >当前普通周期观察关闭，保存 +2 将开启。真实搭话和短时观察独立生效。</v-alert>
                <p
                  v-for="note in record.attention.raise_two_steps.notes"
                  :key="note"
                  class="muted-copy"
                >
                  {{ note }}
            ...
            <v-select
              v-model="sticker"
              label="表情倾向"
              :items="[{title:'继承自然',value:'inherit'},{title:'自然',value:'natural'},{title:'稍多',value:'slightly_more'}]"
            />
          ...
          <section class="config-block">
            ...
            <div v-for="card in pluginSections" :key="card.id" class="plugin-setting">
              ...
              <div
                ...
                :key="plugin.id"
                :ref="node=>setPluginRef(pluginRows,plugin.id,node)"
                class="plugin-row"
              >
                <div class="settings-heading">
                  <div>
                  ...
                  <v-switch
                    v-if="draft.plugins[plugin.id]"
                    v-model="draft.plugins[plugin.id].enabled"
                    :disabled="saving||!!saveOutcome||(!plugin.configured&&!draft.plugins[plugin.id].enabled)"
                    label="在本群启用"
                    color="primary"
                    hide-details
                  />
                  <v-btn
                    ...
                    variant="tonal"
                    size="small"
                    :disabled="saving||!!saveOutcome||!readyToAdd(plugin)"
                    @click="setPluginEnabled(plugin,true)"
                  >
                    {{ readyToAdd(plugin)?'添加到本群草稿并启用':plugin.missing?'当前目录无此实现':'需先填全局参数' }}
                  </v-btn>
                </div>
                <RouterLink
                  v-if="pluginById[plugin.id]"
                  :to="withReturn(route,{name:'plugins',query:{id:plugin.id}})"
                >查看 {{ plugin.name }} 的全局配置与装载状态</RouterLink>
                <template v-if="draft.plugins[plugin.id]">
                  <p
                    v-if="!Object.keys(plugin.scene_config_schema.properties||{}).length"
                    class="muted-copy"
                  >没有额外群参数。</p>
                  <v-expansion-panels v-else v-model="pluginPanels[plugin.id]" class="mt-2">
                    <v-expansion-panel title="本群参数">
                      <v-expansion-panel-text>
                        <PluginConfigFields
                          :ref="node=>setPluginRef(pluginFields,plugin.id,node)"
                          :disabled="saving||!!saveOutcome"
                          v-model="draft.plugins[plugin.id].config"
                          :schema="plugin.scene_config_schema"
                          :problems="pluginProblems[plugin.id]||[]"
                        />
                      </v-expansion-panel-text>
                    </v-expansion-panel>
                  </v-expansion-panels>
          ...
          <div class="save-bar">
            ...
            <div class="settings-actions">
              <v-btn
                variant="text"
                :disabled="saving||!!saveOutcome||loading||!dirty||(!!conflict&&!conflictCurrent)"
                @click="takeCurrent"
              >取消本次修改</v-btn>
              <v-btn
                type="submit"
                color="primary"
                :loading="saving"
                :disabled="saving||!!saveOutcome||!!conflict||!dirty"
              >保存本群设置</v-btn>
            </div>
          </div>
        </v-form>
...
.settings-heading,.settings-actions{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}
...
.field-errors{padding-left:20px;margin-top:8px}
.field-errors button{text-align:left;text-decoration:underline;overflow-wrap:anywhere;padding:4px 0}
.field-errors button:focus-visible{outline:2px solid currentColor;outline-offset:3px}
