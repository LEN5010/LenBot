...
<template>
  <v-navigation-drawer v-model="drawer" :permanent="!mobile" :temporary="mobile" :width="224" aria-label="主导航" class="app-navigation">
    <div class="app-brand"><span class="app-mark">L</span><div><strong>LenBot</strong><span>运行管理中心</span></div><v-btn v-if="mobile" :icon="mdiClose" variant="text" aria-label="关闭导航" @click="drawer=false" /></div>
  ...
  <v-app-bar flat :height="64" class="app-toolbar">
    ...
    <v-menu location="bottom end" :close-on-content-click="false">
      ...
      <v-card width="340" max-width="calc(100vw - 32px)"><v-card-title>当前状态</v-card-title><v-card-text class="status-details">
        ...
        <template v-if="app.status"><p>运行时：{{ app.status.running?'已启动':'已停止' }}</p><p>OneBot：{{ app.status.onebot.connected?'已连接':'未连接' }}</p><p>发送方式：{{ app.status.shadow_mode?'Shadow · 仅记录候选':'实发 · 按保存名单' }}</p><p class="muted">采样于 {{ fmtTime(app.status.sampled_at) }}（北京时间）</p></template>
...
<style scoped>
.app-navigation{border-right:1px solid var(--line)}.app-brand{display:flex;gap:12px;align-items:center;padding:24px 20px 16px}.app-brand strong{display:block;letter-spacing:-.03em;font-size:21px}.app-brand span:not(.app-mark){font-size:12px;color:var(--muted)}.app-mark{width:38px;height:38px;border-radius:10px;display:grid;place-items:center;background:#2563eb;color:white;font-size:23px;font-weight:750}.nav-groups{padding:4px 12px 12px}.nav-section{font-size:11px;letter-spacing:.08em;color:var(--muted);margin:16px 12px 2px;font-weight:600}.nav-groups :deep(.v-list){padding-top:4px;padding-bottom:0}.nav-groups :deep(.v-list-item){min-height:42px;border-radius:8px;margin-bottom:3px}.nav-groups :deep(.v-list-item__prepend > .v-icon){margin-inline-end:14px;font-size:20px;opacity:.85}.nav-groups :deep(.v-list-item-title){font-size:14px}.navigation-footer{padding:12px;border-top:1px solid var(--line)}.app-toolbar{border-bottom:1px solid var(--line)}.toolbar-title{font-size:14px;font-weight:600}.toolbar-status{font-size:12px;color:var(--muted)}.status-indicator{width:6px;height:6px;border-radius:50%;background:#bd8340;display:inline-block;margin-right:8px}.status-indicator.healthy{background:#16845c}.mode-chip{margin-inline:16px 24px}.toolbar-divider{height:20px;align-self:center;margin-left:8px}.status-details p{margin:0 0 12px;line-height:1.6}.status-details{display:grid;gap:4px}
