...
<template>
  ...
  <v-app-bar flat :height="64" class="app-toolbar">
    ...
    <v-menu location="bottom end" :close-on-content-click="false">
      <template #activator="{props}">
        <v-btn
          ...
          aria-label="查看当前运行状态"
        >
          <span class="status-indicator" :class="{healthy:!app.error && app.status?.running}"></span>{{ statusText }}
        </v-btn>
      </template>
      <v-card width="340" max-width="calc(100vw - 32px)">
        <v-card-title>当前状态</v-card-title>
        <v-card-text class="status-details">
          <v-alert v-if="app.error" type="error" variant="tonal">刷新失败：{{ app.error }}。下方为上次读取结果。</v-alert>
          <template v-if="app.status">
            <p>运行时：{{ app.status.running?'已启动':'已停止' }}</p>
        ...
        <v-card-actions>
          <v-btn :prepend-icon="mdiRefresh" :loading="app.loading" @click="refreshStatus">刷新状态</v-btn>
        </v-card-actions>
      </v-card>
    </v-menu>
    ...
    <v-chip
      ...
      class="mode-chip"
    >
      {{ app.status.shadow_mode?'Shadow':'按群规则发送' }}
    </v-chip>
  </v-app-bar>
  <v-main tag="div">
    <main class="app-page" id="main-content">
      <v-alert v-if="error" type="error" variant="tonal" class="mb-4">{{ error }}</v-alert>
      <nav v-if="section?.items.length" class="section-navigation" aria-label="当前区域">
        <v-btn
          v-for="[label,to] in section.items"
          :key="label"
          :to="to"
          :active="isSectionItem(to)"
          :aria-current="isSectionItem(to)?'page':undefined"
          :variant="isSectionItem(to)?'tonal':'text'"
          size="small"
        >
          {{ label }}
        </v-btn>
      </nav>
      <v-btn v-if="origin" :to="origin" :prepend-icon="mdiArrowLeft" variant="text" class="mb-4">
        {{ originLabel }}
      </v-btn>
      <slot />
    </main>
...
<style scoped>
.section-navigation{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:24px;padding-bottom:12px;border-bottom:1px solid var(--line)}
...
.nav-section{font-size:11px;letter-spacing:.08em;color:var(--muted);margin:16px 12px 2px;font-weight:600}
