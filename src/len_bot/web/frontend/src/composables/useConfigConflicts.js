...
export function useConfigConflicts() {
  ...
  function mark(domain, problem) {
    ...
    entries[domain] = { problem: { message: problem.message, path: Array.isArray(problem.details?.path) ? problem.details.path : [] }, snapshot: null, readAt: null, readError: '' }
    return true
  }
  function beginRead(domain) {
    const entry = entries[domain]
    if (entry) { entry.snapshot = null; entry.readAt = null; entry.readError = '' }
  }
  function capture(domain, snapshot) {
    const entry = entries[domain]
    if (!entry) return false
    entry.snapshot = snapshot; entry.readAt = Date.now()/1000; entry.readError = ''
    return true
  }
  function readFailed(domain, problem) {
    if (entries[domain]) entries[domain].readError = problem.message
  }
  function clear(domain) { delete entries[domain] }
