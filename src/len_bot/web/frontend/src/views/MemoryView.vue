...
const statuses = [{ title: '当前有效', value: 'current' }, { title: '已到期但未撤销', value: 'expired' }, { title: '已替代', value: 'superseded' }, { title: '已撤销', value: 'refuted' }, { title: '全部原始状态', value: '' }, { title: '原状态有效（含到期）', value: 'active' }]
...
function expired(item, at = Date.now() / 1000) { return item.expires_at !== null && item.expires_at <= at }
...
function detailRoute(item) { return { name: 'memories', query: { ...listQuery(), list_scene: route.query.list_scene === undefined ? scalar(route.query.scene) : scalar(route.query.list_scene), id: item.id, scene: item.scope } } }
function open(item) { router.push(detailRoute(item)) }
function close() { router.push({ name: 'memories', query: listQuery() }) }
function applyFilters() { router.push({ name: 'memories', query: { ...clean({ return_to: route.query.return_to, scene: filters.value.scene, subject: (filters.value.subject || '').trim(), kind: filters.value.kind, query: (filters.value.query || '').trim() }), status: filters.value.status, page: 1 } }) }
...
async function loadList() {
  ...
  const params = new URLSearchParams(clean({ scope: scalar(route.query.scene), subject: scalar(route.query.subject), status: validity ? 'active' : selection, validity, kind: scalar(route.query.kind), query: scalar(route.query.query), page: page.value, page_size: 30 }))
  try {
    const result = await api('/api/cockpit/memories?' + params)
    if (!fresh() || id.value) return
    rows.value = result.items; total.value = result.total; pageSize.value = result.page_size; listLoaded.value = true; readAt.value = result.sampled_at
...
watch(() => [route.query.tab, route.query.id, route.query.scene, route.query.subject, route.query.status, route.query.kind, route.query.query, route.query.page], () => {
  filters.value = { scene: scalar(route.query.scene), subject: scalar(route.query.subject), status: route.query.status === undefined ? 'current' : scalar(route.query.status), kind: scalar(route.query.kind), query: scalar(route.query.query) }
...
<template>
  <section class="memories-page">
    <v-tabs :model-value="route.query.tab === 'interests' ? 'interests' : 'social'" class="mb-5" @update:model-value="value => router.push({name:'memories',query:{return_to:route.query.return_to,scene:route.query.scene,tab:value}})"><v-tab value="social">认识与记忆</v-tab><v-tab value="interests">公共兴趣</v-tab></v-tabs>
    ...
    <template v-if="!id">
      ...
      <v-card v-if="listLoaded && !rows.length && !listError" class="empty-state"><v-card-text>没有符合筛选条件的认识；这不表示没有原话或历史版本，可切换有效性筛选继续查看。</v-card-text></v-card><v-pagination v-if="pages > 1" :model-value="page" :length="pages" :total-visible="5" class="section-gap" @update:model-value="changePage" />
      <MemoryIndexPanel v-if="scalar(route.query.scene)" ref="indexPanel" :scene-id="scalar(route.query.scene)" />
        ...
        <v-card class="section-gap">
          ...
          <v-card-text>
            ...
            <div v-for="item in currentVersions" :key="item.id" class="current-version">
              <strong>{{ item.id === memory.id ? '所选记录仍有效' : '当前有效记录' }} · 版本 {{ item.revision }}</strong>
              <RouterLink :to="detailRoute(item)" class="record-title">{{ item.statement }}</RouterLink>
            </div>
            <p v-if="!currentVersions.length" class="auxiliary">此链在读取时没有仍有效且未到期的记录；原话、旧陈述及撤销依据保留，未寻找或推断其他独立认识。</p>
            <div class="link-list section-gap"><EntityLink v-if="memory.created_event_id" type="event" :id="memory.created_event_id" :scene-id="memory.scope" label="查看创建提交" /><EntityLink v-if="memory.revision_event_id" type="event" :id="memory.revision_event_id" :scene-id="memory.scope" label="查看最近修订提交" /></div>
          </v-card-text>
        </v-card>
...
<style scoped>
.current-version{display:grid;gap:8px;margin:16px 0;min-width:0;font-size:13px}.current-version>.record-title{white-space:pre-wrap;overflow-wrap:anywhere}
