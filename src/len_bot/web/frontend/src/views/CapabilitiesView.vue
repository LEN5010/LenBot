...
const loadedSelection=ref(null),
  participantScene=ref(null),
  participantReadAt=ref(null),
  participantLoading=ref(false)
const guard=useRequestGuard(()=>`${route.name}:${selection.value}`),
  participantGuard=useRequestGuard(()=>`${route.name}:${scene.value}`)
...
async function load(){
  const fresh=guard(), target=selection.value;
  loading.value=true;
  error.value=''
  ...
  if(invalidSelection.value){
    data.value=null;
    error.value='场景和请求者须各有一个明确参数，不能使用重复参数；未改为查询全部范围。';
    loading.value=false;
    return
  }
  try{
...
async function loadParticipants(){
  ...
  if(participantScene.value!==target){
    participants.value=[];
    participantReadAt.value=null
  }
  if(!target)return
  participantLoading.value=true
  try{
    const result=await api(`/api/cockpit/scenes/${encodeURIComponent(target)}`)
    if(!fresh())return
    ...
    participantScene.value=target;
    participantReadAt.value=Date.now()/1000
  }catch(e){
    if(fresh())participantError.value=e.message
  }
  finally{
    if(fresh())participantLoading.value=false
  }
}
...
function selectScene(scene){
  router.replace({
    name:'capabilities',
    query:{scene:scene||undefined,return_to:route.query.return_to}
  })
}
function selectRequester(requester){
  router.replace({name:'capabilities',query:{...route.query,requester:requester||undefined}})
}
...
<template>
  <div class="page-stack">
    ...
    <div class="filters">
      <ScopeSelect :model-value="scene" label="查看哪个群的能力" @update:model-value="selectScene" />
      <v-select
        :model-value="requester||null"
        :items="participants"
        label="从已保存成员选择请求者（可选）"
        clearable
        :loading="participantLoading"
        :disabled="!scene||participantLoading"
        :error-messages="participantError"
        @update:model-value="selectRequester"
      />
    </div>
    ...
    <template v-if="data">
      <p class="muted">{{ data.evidence_note }} · 读取于 {{ fmtTime(data.sampled_at) }}</p>
      <v-alert v-if="data.requires_restart" type="info" variant="tonal">另有已保存配置等待手动重启；每项能力的装载状态分别列出。</v-alert>
...
.filters>.v-select{max-width:360px;min-width:230px}
</style>
