...
function jobStatus(status) {
  return { pending: '等待执行', claimed: '已认领', processing: '正在处理', result_ready: '结果待回应', awaiting_delivery: '等待交付', completed: '已送达', partial: '部分完成', failed: '失败', cancelled: '已取消', review_required: '等待核对', interrupted: '已中断' }[status] || status
}
function executionStatus(status) {
  return { pending: '尚未开始', running: '执行中', completed: '执行完成', partial: '部分完成', failed: '执行失败', interrupted: '执行中断', cancelled: '执行取消', unknown: '待核对' }[status] || status
}
