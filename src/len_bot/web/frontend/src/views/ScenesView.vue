...
const primaryTabs = [{ value: 'messages', label: '消息' }, { value: 'settings', label: '配置' }]
// Attention, history, participants, jobs and receipts answer "why did this
// happen"; they stay reachable at their existing URLs but no longer compete
// with 消息 and 配置 for the first click.
const diagnosticTabs = [{ value: 'attention', label: '注意力' }, { value: 'history', label: '摘要覆盖' }, { value: 'participants', label: '参与者' }, { value: 'preferences', label: '互动偏好' }, { value: 'jobs', label: '工作' }, { value: 'deliveries', label: '发送记录' }]
    ...
    <div class="scene-workspace" :class="{ 'has-scene': sceneId, 'has-inspector': eventId, 'wide-workspace': wide }">
      <v-card v-show="!mobile || !sceneId" class="scene-directory">
        ...
        <v-list class="scene-list" lines="three" aria-label="场景目录">
          <v-list-item v-for="scene in scenes" :key="scene.scene_id" :to="{ name: 'scene', params: { sceneId: scene.scene_id }, query: { ...(route.query.query ? {query:route.query.query} : {}), ...(!scene.has_history ? {tab:'settings'} : {}) } }" :active="scene.scene_id === sceneId" color="primary" class="scene-list-row">
            <div class="scene-list-content"><strong>{{ scene.display_name }}</strong><span class="scene-id">{{ scene.scene_id }}</span><time>{{ scene.has_history?'最近活动 '+fmtTime(scene.last_event_at):scene.settings?'已配置，尚无原话记录':'已加入，尚未配置' }}</time><span v-if="scene.scene_type==='group'" class="scene-id">{{ scene.joined===true?'已加入':scene.joined===false?'加入待确认':'' }} {{ !scene.settings?'未配置':!scene.settings.enabled?'已停用':scene.settings.chat?'普通聊天已开放':'仅白名单聊天／命令与公告' }}</span><div class="scene-counts"><span>待处理 {{ scene.pending_wake_count }}</span><span>工作 {{ scene.active_job_count }}</span></div></div>
          </v-list-item>
        </v-list>
      ...
      <div v-if="sceneId" v-show="!mobile || !eventId" class="scene-main">
        ...
        <template v-if="detail||tab==='settings'||detailMissing">
          <v-card class="scene-heading"><v-card-text><h2>{{ detail?.session.display_name||sceneId }}</h2><div class="scene-heading-meta"><EntityLink type="scene" :id="sceneId" :scene-id="sceneId" :label="sceneId" /><span v-if="detail">{{ Object.keys(detail.session.participants).length }} 位已记录成员</span><span v-if="detailReadAt">读取于 {{ fmtTime(detailReadAt) }}</span></div></v-card-text><v-tabs :model-value="tabNames.includes(tab)?tab:(detail?'messages':'settings')" color="primary" show-arrows @update:model-value="setTab"><v-tab v-for="item in primaryTabs" :key="item.value" v-show="item.value!=='settings'||sceneId.startsWith('group:')" :value="item.value">{{ item.label }}</v-tab></v-tabs>
            <div v-if="sceneId.startsWith('group:')" class="diagnostic-tabs"><span class="muted">诊断</span><v-btn v-for="item in diagnosticTabs" :key="item.value" size="small" variant="text" :color="tab===item.value?'primary':undefined" @click="setTab(item.value)">{{ item.label }}</v-btn></div>
          </v-card>
          ...
          <v-card v-else class="scene-tab-card">
            <v-card-text class="scene-detail-body">
              ...
              <template v-if="tab === 'attention'">
                ...
                <p>短时观察截止：{{ detail.session.observing_until ? fmtTime(detail.session.observing_until) : '未开启' }}；有新输入才执行，截止不表示正在调用模型。</p><p>待观察或待处理来源 {{ aux?.total ?? detail.session.pending_wake_count }} 项</p><article v-for="wake in aux?.items || []" :key="wake.event_id" class="detail-record"><div class="record-meta"><v-chip variant="tonal" size="small">{{ wake.certain ? '直接搭话或运行来源' : '观察机会' }}</v-chip><span>{{ wake.actor_id || '运行事件' }}</span><span>位置 {{ wake.rowid }}</span></div><p>{{ wake.reasons.map(attentionReason).join(' · ') || '未记录原因' }}</p><EntityLink type="event" :id="wake.event_id" :scene-id="sceneId" label="查看来源原话与关联" /></article><p v-if="aux && !aux.items.length" class="empty-copy">没有尚待处理的唤醒来源。</p>
