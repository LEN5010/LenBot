...
<template>
  <div class="page-stack settings-view">
    ...
    <v-card v-if="tab==='runtime'&&runtimeText!==null" class="pa-5 form-card">
      <h2>运行参数</h2>
      <p class="muted my-3">编辑预算、并发、媒体和维护等运行参数；填写完整 JSON 对象后保存。</p>
      <v-alert v-if="runtimeRestart" type="info" variant="tonal" class="mb-4">本节配置保存后需手动重启服务生效。</v-alert>
...
<style scoped>
.preset-field{margin-bottom:12px}.runtime-json :deep(textarea){font-family:monospace;font-size:13px;line-height:1.6}
