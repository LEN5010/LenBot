...
import { purposeOptions,purposeLabel,eventLabel,traceLabel } from '../domain/activity.js'
  ...
  <div class="page-stack activity-view">
    ...
    <template v-if="data">
      ...
      <v-card class="activity-list">
        ...
        <template v-else-if="tab==='turns'"><article v-for="item in data.items" :key="item.id" class="record-row turns-grid"><div class="record-main"><strong>{{ traceLabel(item.kind) }}</strong><p class="clamp-2">{{ item.summary }}</p></div><EntityLink type="scene" :id="item.scene_id" :copyable="false" /><time>{{ fmtTime(item.created_at) }}</time><v-btn variant="text" color="primary" size="small" @click="openItem(item)">查看过程</v-btn></article></template>
    ...
    <v-dialog :model-value="!!selectedId" :fullscreen="smAndDown" max-width="1000" scrollable @update:model-value="!$event && closeDetail()"><v-card><v-card-title class="detail-heading"><span>{{ tab==='calls'?'调用详情':tab==='turns'?'对话与执行过程':'事件原文与关联' }}</span><v-btn :icon="mdiClose" variant="text" aria-label="关闭详情" @click="closeDetail" /></v-card-title><v-card-text class="detail-body"><v-progress-linear v-if="detailLoading" indeterminate color="primary" /><v-alert v-if="detailError" type="error" variant="tonal">{{ detailError }}</v-alert>
      <template v-if="selected">
        ...
        <section class="relations-section"><h3>持久关联</h3><p class="muted">仅显示记录中明确保存的关联；资料和候选不是已经发送的群消息。</p><v-alert v-if="relationError" type="error" variant="tonal">关联读取失败：{{ relationError }}</v-alert><p v-if="!relations && !detailLoading && !relationError" class="muted">没有可用于定位的轮次、工作或事件关联。</p><template v-if="relations"><v-alert v-if="Object.values(relations.truncated).some(Boolean)" type="info" variant="tonal">关联较多，当前显示各类别的受限结果；可从对象详情继续读取。</v-alert><div class="relation-grid"><section><h4>原始事件</h4><div v-for="item in relations.events" :key="item.id" class="relation-row"><EntityLink type="event" :id="item.id" :scene-id="item.scene_id" :label="eventLabel(item.event_type)" :copyable="false" /><span class="clamp-2">{{ eventSummary(item) }}</span></div><p v-if="!relations.events.length" class="muted">未关联事件</p></section><section><h4>工作与调用</h4><div v-for="item in relations.jobs" :key="item.id" class="relation-row"><EntityLink type="job" :id="item.id" :scene-id="item.scene_id" :label="item.goal" :copyable="false" /></div><div v-for="item in relations.calls" :key="item.id" class="relation-row"><EntityLink type="call" :id="item.id" :scene-id="item.scene_id" :label="`${purposeLabel(item.purpose)} · ${item.model}`" :copyable="false" /></div><p v-if="!relations.jobs.length && !relations.calls.length" class="muted">未关联工作或调用</p></section><section><h4>资料</h4><div v-for="item in relations.tool_results" :key="item.id" class="relation-row"><EntityLink type="result" :id="item.id" :scene-id="item.scene_id" :label="`${item.tool_name} · ${item.content_length} 字符`" :copyable="false" /></div><p v-if="!relations.tool_results.length" class="muted">未关联工具资料</p></section><section><h4>行动与回执</h4><div v-for="item in relations.actions" :key="item.id" class="relation-row"><span class="entity-id">{{ item.id }}</span><v-chip v-if="item.simulated" size="small" label>模拟记录</v-chip><StatusBadge domain="delivery" :status="item.delivery_status" /><EntityLink v-for="id in item.receipt_event_ids" :key="id" type="event" :id="id" :scene-id="item.scene_id" label="读取回执" :copyable="false" /></div><p v-if="!relations.actions.length" class="muted">未关联行动或回执</p></section></div></template></section>
      </template>
    </v-card-text></v-card></v-dialog>
    <v-dialog :model-value="!!resourceId" :fullscreen="smAndDown" max-width="960" scrollable @update:model-value="!$event && navigate({result:undefined})"><v-card><v-card-title class="detail-heading"><span>原始工具资料</span><v-btn :icon="mdiClose" variant="text" aria-label="关闭原始资料" @click="navigate({result:undefined})" /></v-card-title><v-card-text><ResourceViewer title="资料正文" :content="resource?.content" :loading="resourceLoading" :error="resourceError"><p v-if="resource" class="muted">{{ resource.coverage }}</p><v-btn v-if="resource?.next_offset!==null && resource?.next_offset!==undefined" :loading="resourceLoading" variant="outlined" @click="loadResource(resource.next_offset,true)">继续读取正文</v-btn></ResourceViewer></v-card-text></v-card></v-dialog>
  </div>
</template>
<style scoped>
