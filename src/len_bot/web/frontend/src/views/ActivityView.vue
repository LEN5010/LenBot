...
const resourceKey=computed(()=>JSON.stringify([resourceId.value,scene.value,route.query.result_start,route.query.result_end,route.query.result_unit]))
...
const knownUsage=computed(()=>data.value?.totals.reduce((sum,row)=>({prompt:sum.prompt+row.prompt_tokens,output:sum.output+row.completion_tokens,unknown:sum.unknown+row.unknown_usage,cached:sum.cached+row.cached_tokens,reasoning:sum.reasoning+row.reasoning_tokens,audio:sum.audio+(row.audio_seconds || 0)}),{prompt:0,output:0,unknown:0,cached:0,reasoning:0,audio:0}))
...
function navigate(values){const query={...route.query,...values};if(Object.hasOwn(values,'result') && !values.result)for(const key of ['result_start','result_end','result_unit'])delete query[key];for(const key of Object.keys(query))if(query[key]===undefined || query[key]===null || query[key]==='')delete query[key];return router.push({name:'activity',query})}
...
async function loadDetail(){
  ...
  if(key!==detailKey){selected.value=null;relations.value=null;detailReadAt.value=null;relationReadAt.value=null;detailKey=key}
  detailError.value='';relationError.value=''
  if(!selectedId.value || tab.value==='logs'){detailLoading.value=false;return}
  ...
  try{
    ...
    selected.value=value;detailReadAt.value=Date.now()/1000
    ...
    try{const linked=await api('/api/cockpit/relations?'+queryString(relationArgs));if(own===detailSequence){relations.value=linked;relationReadAt.value=Date.now()/1000}}catch(e){if(own===detailSequence)relationError.value=e.message}
...
async function loadResource(offset=resourceRange.value.start,append=false){
  const own=++resourceSequence,range=resourceRange.value;resourceError.value='';resourceLoading.value=false
  if(!resourceId.value)return
  if(range.error){resourceError.value=range.error;return}
  if(!scene.value){resourceError.value='读取原始资料需要明确来源场景。';return}
  ...
  try{
    ...
    if(own===resourceSequence){resource.value=value;resourcePages.value=append?[...resourcePages.value,value]:[value];resourceReadAt.value=Date.now()/1000}
  }catch(e){if(own===resourceSequence)resourceError.value=e.status===404?'该场景中没有此资料。':e.message}finally{if(own===resourceSequence)resourceLoading.value=false}
}
function retryResource(){return resource.value?.next_offset!=null ? loadResource(resource.value.next_offset,true) : loadResource()}
...
watch(resourceKey,()=>{resource.value=null;resourcePages.value=[];resourceReadAt.value=null;loadResource()},{immediate:true})
...
<template>
  <div class="page-stack activity-view">
      ...
      <template v-if="selected">
        ...
        <details :key="`${tab}:${selected.id}`" class="relations-section" :open="tab!=='events'">
          ...
          <template v-if="relations">
            ...
            <section v-if="relations.turns?.length" class="turn-relations"><h4>已提交轮次的读取与处理</h4><article v-for="turn in relations.turns" :key="turn.event_id" class="turn-relation"><EntityLink type="event" :id="turn.event_id" :scene-id="selected.scene_id" label="查看此轮提交" /><p>实际读取 {{ turn.read_source_event_ids.length }} 条来源；{{ turn.handled_source_event_ids===null?'未单独记录处理来源':`明确处理 ${turn.handled_source_event_ids.length} 条来源` }}。</p><div class="detail-meta"><EntityLink v-for="id in turn.handled_source_event_ids || []" :key="id" type="event" :id="id" :scene-id="selected.scene_id" label="本轮处理的来源" /></div><SourceOutcomes :items="turn.source_outcomes" :scene-id="selected.scene_id" /><ProvidedReads :turn="turn" :scene-id="selected.scene_id" /><details v-if="turn.read_source_event_ids.length"><summary>本轮实际读取的来源</summary><div class="detail-meta"><EntityLink v-for="id in turn.read_source_event_ids" :key="id" type="event" :id="id" :scene-id="selected.scene_id" :label="id" /></div></details></article></section>
    ...
    <v-dialog :model-value="!!resourceId" :fullscreen="smAndDown" max-width="960" scrollable @update:model-value="!$event && navigate({result:undefined})"><v-card><v-card-title class="detail-heading"><span>{{ resourceRange.bounded?'引用范围内的保存资料':'原始工具资料' }}</span><v-btn :icon="mdiClose" variant="text" aria-label="关闭原始资料" @click="navigate({result:undefined})" /></v-card-title><v-card-text class="resource-body">
      ...
      <v-btn v-if="resourceRange.bounded || resourceRange.error" variant="text" size="small" @click="navigate({result_start:undefined,result_end:undefined,result_unit:undefined})">改为从头查看保存正文</v-btn>
      <v-progress-linear v-if="resourceLoading" indeterminate aria-label="正在读取保存资料" />
      <v-alert v-if="resourceError" type="error" variant="tonal">{{ resourceError }}<span v-if="resourcePages.length">；保留已取得的页面（{{ fmtTime(resourceReadAt) }}）。</span><div v-if="!resourceRange.error" class="mt-3"><v-btn variant="outlined" size="small" :loading="resourceLoading" @click="retryResource">重读未取得的资料</v-btn></div></v-alert>
      <ResourceViewer v-for="(value,index) in resourcePages" :key="`${value.coordinate_unit}:${value.displayed_range.start}`" :title="`第 ${index+1} 页 · [${value.displayed_range.start}, ${value.displayed_range.end}) ${resourceUnit(value.coordinate_unit)}`" :content="value.content" />
      <ObservationDetails v-if="resource" :observation="resource" :scene-id="scene" range-label="最近一页保存资料范围" />
      <v-btn v-if="resource?.next_offset!==null && resource?.next_offset!==undefined" :loading="resourceLoading" variant="outlined" @click="loadResource(resource.next_offset,true)">{{ resourceRange.bounded?'继续读取引用范围':'继续读取已保存正文' }}</v-btn>
      <p v-else-if="resource && resourceRange.bounded" class="muted">所选范围已读取完毕；不代表整份来源或未取得的附件已读。</p><p v-if="resourceReadAt" class="sample-time">读取于 {{ fmtTime(resourceReadAt) }}</p>
    </v-card-text></v-card></v-dialog>
  </div>
</template>
<style scoped>
.relations-section>summary{font-size:14px;font-weight:600;cursor:pointer;line-height:1.7}
.resource-body{display:grid;gap:16px;min-width:0}.resource-body>.muted{font-size:12px;line-height:1.7}.resource-body>.v-btn{justify-self:start;max-width:100%;white-space:normal}
