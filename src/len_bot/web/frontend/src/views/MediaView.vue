...
const baselineFields = ['id','scope','source_event_id','created_at','curated','description','tags','enabled','palette_order']
const assetBaseline = asset=>clone(Object.fromEntries(baselineFields.map(key=>[key,asset[key]])))
const editableAsset = asset=>({description:asset.description,tags:[...asset.tags],enabled:asset.enabled,palette_order:asset.palette_order})
const draftValues = () => ({description:draft.value.description,tags:[...new Set(draft.value.tagsText.split(/\s+/).filter(Boolean))],enabled:draft.value.enabled,palette_order:draft.value.palette_order})
...
function savedReceipt(details, scope, id=null) {
  ...
  return {asset_id:details.asset_id,scope:details.scope,event_id:details.event_id,phase:details.phase}
}
...
const toAsset = asset => ({ name: 'media', query: { ...route.query, list_scene: listScope.value, scene: listScope.value, id: asset.id } })
function setDraft(asset) {
  const original = asset.curated ? assetBaseline(asset) : null
  const values = asset.curated ? { description: asset.description, tagsText: asset.tags.join(' '), enabled: asset.enabled, palette_order: asset.palette_order } : null
...
async function load() {
  ...
  try {
    ...
    if (!fresh()) return
    assets.value = result.items; total.value = result.total; uploadMaxBytes.value = result.upload_max_bytes; loaded.value = true; readAt.value = Date.now() / 1000
  } catch (e) { if (fresh()) error.value = e.message }
  finally { if (fresh()) loading.value = false }
}
async function loadPalette() {
  const fresh = paletteGuard()
  paletteLoading.value = true; paletteError.value = ''
  try {
    const value = await api('/api/media/palette?' + new URLSearchParams({ scene_id: listScope.value }))
    if (!fresh()) return
    palette.value = value.items; paletteLimit.value = value.limit; paletteLoaded.value = true; paletteReadAt.value = Date.now()/1000
  } catch (e) { if (fresh()) paletteError.value = e.message }
  finally { if (fresh()) paletteLoading.value = false }
}
async function loadDetail({accept=()=>true}={}) {
  const own=detailGuard(),fresh=()=>own()&&accept(),id=scalar(route.query.id),key=detailKey.value,scene=scalar(route.query.scene)||'global-safe'
  if (selectedKey.value !== key) { selected.value = null; draft.value = null; baseline.value = null; detailReadAt.value = null; selectedKey.value = null }
  detailError.value = ''
  conflicts.beginRead('media')
  if(saveReview.value){saveReview.value.current=null;saveReview.value.readAt=null}
  ...
  try {
    ...
    if (saveReview.value) {
      ...
      saveReview.value.current=asset;saveReview.value.readAt=Date.now()/1000
    }
    else if (readbackPending.value) {
      if(!sameAssetIdentity(asset,baseline.value))throw new Error('读回的素材身份或原来源已改变，不能用它确认原保存请求。')
      setDraft(asset);readbackPending.value=false
    }
    else if (!conflicts.capture('media',asset) && !dirty.value) setDraft(asset)
    selected.value=asset;selectedKey.value=key;detailReadAt.value=Date.now()/1000
  } catch (e) { if (fresh()) {detailError.value=e.message;conflicts.readFailed('media',e)} }
...
function adoptReviewedValues() {
  ...
  setDraft(current);saveReview.value=null;saveError.value=''
...
function resolveConflict(keep) {
  ...
  if(keep&&!paletteValid.value){saveError.value='请先将目录顺序修正为非负整数或留空，再保留修改。';return}
  if(keep&&(!asset.curated||['id','scope','source_event_id','created_at'].some(key=>asset[key]!==baseline.value[key]))) {
    saveError.value='原素材身份或来源已改变，须采用现值重新编辑，不能把旧草稿接到新素材。';return
  }
  const next=keep?rebaseConfigDraft(editableAsset(baseline.value),draftValues(),editableAsset(asset)):null
  setDraft(asset)
  if(keep)draft.value={description:next.description,tagsText:next.tags.join(' '),enabled:next.enabled,palette_order:next.palette_order}
  conflicts.clear('media');saveError.value='';detailMessage.value=keep?'已保留实际修改，其余采用现值；请核对后另行保存。':'已采用本次读取的保存值，没有提交修改。'
}
async function save() {
  ...
  detailGuard();detailLoading.value=false
  saving.value=true;saveError.value='';detailMessage.value='';savedNotice.value=null
  ...
  try {
    ...
    setDraft(result); selectedKey.value = key; detailReadAt.value = Date.now()/1000
    detailMessage.value='素材与固定目录已保存；目录是否进入本次限额请看当前目录读取。';await Promise.all([load(),loadPalette()])
  } catch (e) {
    if(!fresh())return
    const receipt=savedReceipt(e.details,scope,id)
    if(receipt) {readbackPending.value=true;savedNotice.value=receipt;saveError.value=e.message}
...
function openUpload() {
  ...
  uploadGuard();uploadForm.value={scope:nextScope,description:'',tags:'',file:null}
  uploadError.value='';uploadReceipt.value=null;uploadReview.value=null;uploadOpen.value=true
}
function closeUpload() {
  ...
  uploadOpen.value=false;uploadGuard();uploadForm.value={scope:listScope.value,description:'',tags:'',file:null}
...
async function upload() {
  ...
  const fresh=uploadGuard(),attempt={scope:uploadForm.value.scope,name:uploadForm.value.file.name,size:uploadForm.value.file.size}
  ...
  try {
    ...
    uploadReceipt.value={asset_id:asset.id,scope:asset.scope,event_id:asset.source_event_id,phase:'complete',name:attempt.name}
  } catch (e) {
    if(!fresh())return
    const receipt=savedReceipt(e.details,attempt.scope)
    if(receipt){uploadReceipt.value={...receipt,name:attempt.name};uploadError.value=e.message}
    else if(requestRefused(e))uploadError.value=e.message
    else {uploadReview.value=attempt;uploadError.value='未取得属于本次上传的有效确认，结果未知。接口信息：'+e.message}
  }
  finally { if (fresh()) uploading.value = false }
}
function viewUploaded() {
  if(!uploadReceipt.value||uploading.value)return
  return router.push({name:'media',query:{return_to:route.query.return_to,scene:uploadReceipt.value.scope,id:uploadReceipt.value.asset_id,page:1}})
}
async function inspectUploadScope() {
  ...
  uploadOpen.value=false;uploadGuard();uploadForm.value={scope,description:'',tags:'',file:null}
...
watch(detailKey, () => {
  saveGuard();detailGuard();saving.value=false;saveError.value='';detailMessage.value='';readbackPending.value=false;savedNotice.value=null;saveReview.value=null;conflicts.clear('media');loadDetail()
}, { immediate:true,flush:'sync' })
watch(() => route.fullPath, () => {
  uploadGuard()
  uploadOpen.value=false;uploading.value=false;uploadError.value='';uploadReceipt.value=null;uploadReview.value=null
...
<template>
  <div class="page-stack">
    <PageHeader title="媒体与素材" description="消息媒体、工具产物与运营图片。登记、预览和模型实际读取是不同的事实。">
      <v-btn variant="outlined" :loading="loading || paletteLoading" @click="refresh">刷新列表与目录</v-btn><v-btn color="primary" :disabled="saving||uploading" @click="openUpload">上传图片素材</v-btn>
    </PageHeader>
    ...
    <v-expansion-panels>
      <v-expansion-panel title="固定表情目录">
        <v-expansion-panel-text>
          <div class="list-summary mb-3"><p class="muted">当前配置最多 {{ paletteLimit ?? '未读取' }} 项，按运营顺序提供，独立于下方列表筛选。目录不证明某轮实际装配或使用。</p><v-btn variant="text" size="small" :loading="paletteLoading" @click="loadPalette">刷新目录</v-btn></div>
          <v-progress-linear v-if="paletteLoading" indeterminate aria-label="正在读取固定目录" />
          <v-alert v-if="paletteError" type="error" variant="tonal" class="mb-3">{{ paletteError }}<span v-if="paletteReadAt"> · 下方保留上次目录</span></v-alert>
          <p v-if="paletteReadAt" class="muted mb-3">目录读取于 {{ fmtTime(paletteReadAt) }}</p>
          <p v-if="paletteLoaded && !paletteLoading && !paletteError && !palette.length" class="muted">当前范围尚无固定目录素材。</p>
          <div class="palette-list"><RouterLink v-for="asset in palette" :key="asset.id" :to="toAsset(asset)" class="palette-item"><div class="palette-preview"><MediaPreview :asset="asset" :scene-id="listScope" /></div><span class="clamp-2">{{ asset.palette_order }} · {{ asset.description || asset.id }}</span></RouterLink></div>
        </v-expansion-panel-text>
      </v-expansion-panel>
    </v-expansion-panels>
    <div v-if="loaded" class="list-summary"><span>{{ total }} 项媒体</span><span v-if="readAt" class="muted">读取于 {{ fmtTime(readAt) }}</span></div>
    ...
    <v-dialog :model-value="!!route.query.id" max-width="920" scrollable :persistent="saving" @update:model-value="value => !value && closeDetail()">
      <v-card><v-card-title class="dialog-title">媒体详情<div><v-btn variant="text" :loading="detailLoading" :disabled="saving" @click="refreshDetail">刷新详情</v-btn><v-btn variant="text" :disabled="saving" @click="closeDetail">关闭</v-btn></div></v-card-title><v-card-text>
        <v-progress-linear v-if="detailLoading" indeterminate />
        <v-alert v-if="detailError" type="error" variant="tonal" class="mb-4">{{ detailError }}<span v-if="detailReadAt"> · 保留上次读取内容及当前表单</span></v-alert>
        <v-alert v-if="saveError" type="error" variant="tonal" class="mb-4">{{ saveError }}</v-alert>
        <v-alert v-if="detailMessage" type="info" variant="tonal" class="mb-4">{{ detailMessage }}</v-alert>
        <v-alert v-if="readbackPending" type="warning" variant="tonal" class="mb-4">素材已经入库，但当前表单尚未读回新值，暂不再次提交。请重读保存值；重读不重新登记事件。<v-btn variant="text" :disabled="saving" :loading="detailLoading" @click="refreshDetail">重读保存值</v-btn></v-alert>
        <v-alert v-if="saveReview" type="warning" variant="tonal" class="mb-4">
          ...
          <div class="chips mt-3"><v-btn variant="outlined" :disabled="saving" :loading="detailLoading" @click="refreshDetail">读取当前保存值</v-btn><v-btn variant="text" :disabled="saving||detailLoading||!saveReview.current" @click="adoptReviewedValues">放弃旧草稿，采用当前值重新编辑</v-btn></div>
          <ResourceViewer :content="{baseline:saveReview.baseline,submitted:{description:saveReview.description,tags:saveReview.tags,enabled:saveReview.enabled,palette_order:saveReview.palette_order}}" title="原请求与草稿" />
          <template v-if="saveReview.current"><p class="mt-3">当前值读取于 {{ fmtTime(saveReview.readAt) }}</p><ResourceViewer :content="assetBaseline(saveReview.current)" title="本次读取的当前值（不是原请求回执）" /></template>
        </v-alert>
        <p v-if="savedNotice" class="muted mb-3">原 {{ savedNotice.phase==='event'?'事件登记':'保存值读取' }} 阶段未正常结束；取得保存值不等于该阶段已恢复。<EntityLink v-if="savedNotice.event_id" type="event" :id="savedNotice.event_id" :scene-id="savedNotice.scope" label="核对本次变更事件（可能尚不可读）" /></p>
        <ConfigConflictBanner :conflict="currentConflict?.problem" :current="currentConflict?.snapshot" :path-label="currentConflict?.problem.path.join(' → ')" :read-at="currentConflict?.readAt" :read-error="currentConflict?.readError" :busy="saving||detailLoading||readbackPending" @keep="resolveConflict(true)" @take="resolveConflict(false)" @reload="refreshDetail">
          <template #current><ResourceViewer :content="currentConflict?.snapshot" title="本次读取的素材保存值" /></template>
    ...
    <v-dialog :model-value="uploadOpen" max-width="640" scrollable :persistent="uploading" @update:model-value="value => !value && closeUpload()">
      <v-card><v-card-title class="dialog-title">上传运营图片<v-btn variant="text" :disabled="uploading" @click="closeUpload">关闭</v-btn></v-card-title><v-card-text>
        <v-alert v-if="uploadError" type="error" variant="tonal" class="mb-4">{{ uploadError }}</v-alert>
        <v-alert v-if="uploadReceipt" :type="uploadReceipt.phase==='complete'?'success':'warning'" variant="tonal" class="mb-4">
          <p>{{ uploadReceipt.name }} 已登记为 {{ uploadReceipt.asset_id }}，范围 {{ uploadReceipt.scope }}。</p>
          <p class="mt-2">{{ uploadReceipt.phase==='complete'?'这不是向群上传或发送的回执。可进入详情查看原记录并设置固定目录。':'图片已入库，后续阶段尚未正常结束。请核对原素材，不要重新上传同一请求。' }}</p>
          <div class="chips mt-3"><v-btn variant="outlined" :disabled="uploading" @click="viewUploaded">查看已登记素材</v-btn><v-btn v-if="uploadReceipt.phase==='complete'" variant="text" :disabled="uploading" @click="openUpload">再登记一张图片</v-btn></div>
          <EntityLink v-if="uploadReceipt.event_id" type="event" :id="uploadReceipt.event_id" :scene-id="uploadReceipt.scope" label="核对登记事件" />
        </v-alert>
...
.detail-image{height:clamp(220px,42vh,450px);background:#f4f6f9;display:flex;margin-bottom:16px;min-width:0}.detail-meta{font-size:13px;align-items:center;margin-bottom:20px}.edit-form{display:grid;gap:12px}.edit-form>.v-btn{justify-self:start}.full-text{white-space:pre-wrap;overflow-wrap:anywhere;line-height:1.7}
.asset-link:focus-visible,.palette-item:focus-visible{outline:3px solid #2563eb;outline-offset:-3px}@media(max-width:1000px){.filters{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:500px){.filters{grid-template-columns:minmax(0,1fr)}.media-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.asset-copy{padding:10px}.chips{gap:4px}.detail-image{height:260px}}
</style>
