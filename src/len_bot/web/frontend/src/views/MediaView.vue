    ...
    <div class="media-grid">
      <v-card v-for="asset in assets" :key="asset.id" tag="article">
        <RouterLink :to="toAsset(asset)" class="asset-link">
          ...
          <div class="asset-copy"><strong class="clamp-2">{{ asset.description || (asset.curated ? '运营素材' : '聊天原图') }}</strong><p class="muted asset-scope">{{ asset.scope }}</p><div class="chips"><v-chip size="small" :color="asset.enabled ? 'success' : 'default'">{{ asset.enabled ? '已启用' : '已停用' }}</v-chip><v-chip v-if="asset.palette_order != null" size="small" color="primary">目录 {{ asset.palette_order }}</v-chip><v-chip v-if="asset.in_current_limit" size="small" color="success">本轮初始目录</v-chip><v-chip v-else-if="asset.in_initial_catalog" size="small">有序号，超出当前限额</v-chip><v-chip v-if="asset.curated && !asset.description_sufficient" size="small" color="warning">缺描述/标签</v-chip></div><p class="asset-tags clamp-2">{{ asset.tags.slice(0,3).join(' · ') }}<span v-if="asset.tags.length > 3"> · +{{ asset.tags.length - 3 }}</span></p></div>
        </RouterLink>
      </v-card>
    </div>
