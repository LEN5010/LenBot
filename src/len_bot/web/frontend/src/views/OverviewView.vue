...
const issueDefinitions=[
  {key:'review',title:'待核对工作',type:'job',path:'/api/cockpit/jobs?status=review_required&page_size=4',to:{name:'jobs',query:{status:'review_required'}}},
  {key:'jobs',title:'工作送达未知',type:'job',path:'/api/cockpit/jobs?status=delivery_unknown&page_size=4',to:{name:'jobs',query:{status:'delivery_unknown'}}},
  {key:'reminders',title:'提醒送达未知',type:'task',path:'/api/cockpit/tasks?kind=reminder&status=delivery_unknown&page_size=4',to:{name:'tasks',query:{tab:'reminders',status:'delivery_unknown'}}},
  {key:'deferred',title:'延期送达未知',type:'task',path:'/api/cockpit/tasks?kind=deferred&status=delivery_unknown&page_size=4',to:{name:'tasks',query:{tab:'deferred',status:'delivery_unknown'}}},
  {key:'calls',title:'已记录失败请求',type:'call',path:'/api/models/usage?status=failed&page_size=4',to:{name:'activity',query:{tab:'calls',status:'failed'}}},
]
...
async function load(){
  const fresh=statsGuard();statsLoading.value=true;error.value=''
  try{const result=await api('/api/overview/stats');if(fresh())data.value=result}
  catch(e){if(fresh())error.value=e.message}finally{if(fresh())statsLoading.value=false}
}
async function loadIssue(item){
  const fresh=issueGuards[item.key](), source=issues[item.key]
  source.loading=true;source.error=''
  try{const result=await api(item.path);if(fresh()){source.data=result;source.readAt=Date.now()/1000}}
  catch(e){if(fresh())source.error=e.message}finally{if(fresh())source.loading=false}
}
async function refresh(){await Promise.all([load(),loadPlugins(),refreshStatus(),...issueDefinitions.map(loadIssue)])}
...
async function loadPlugins(){
  const fresh=pluginGuard();pluginLoading.value=true;pluginError.value=''
  try{const result=await api('/api/overview/capabilities');if(fresh())capabilities.value=result}
  catch(e){if(fresh())pluginError.value=e.message}finally{if(fresh())pluginLoading.value=false}
}
...
<template>
  <div class="page-stack">
    ...
    <v-expansion-panels v-if="capabilities"><v-expansion-panel title="当前能力与验证记录"><v-expansion-panel-text><p class="muted mb-4">{{ capabilities.evidence_note }} · 能力样本 {{ fmtTime(capabilities.sampled_at) }}{{ pluginError ? '（刷新失败，保留旧结果）' : '' }}</p><CapabilityCards :data="capabilities" compact /><v-btn class="mt-4" variant="tonal" :to="{name:'capabilities'}">选择群与发起者，查看完整能力状态</v-btn></v-expansion-panel-text></v-expansion-panel></v-expansion-panels>
    <v-card><v-card-text>
      <div class="section-heading"><div><h2>需要留意</h2><p class="muted">各列表独立读取，每项最多展示四条。未知送达不自动重发；历史请求失败不等于当前仍有故障，普通沉默不计为异常。</p></div><v-chip v-if="issuesKnown&&!hasIssues" variant="tonal" size="small">本次各列表均无相关记录</v-chip></div>
      <div class="issues-grid">
        <section v-for="source in issueDefinitions" :key="source.key">
          <div class="issue-title"><strong>{{ source.title }}</strong><span>{{ issues[source.key].data?.total ?? '未读取' }}</span></div>
          <v-progress-linear v-if="issues[source.key].loading" indeterminate :aria-label="'正在读取' + source.title" />
          <v-alert v-if="issues[source.key].error" type="error" variant="tonal" density="compact">{{ issues[source.key].error }}<v-btn size="small" variant="text" :loading="issues[source.key].loading" @click="loadIssue(source)">重读此列表</v-btn></v-alert>
          <p v-if="issues[source.key].readAt" class="muted">{{ issues[source.key].error ? '保留上次读取' : '读取于' }} {{ fmtTime(issues[source.key].readAt) }}</p>
          <div v-for="item in issues[source.key].data?.items || []" :key="item.id" class="issue-item"><EntityLink :type="source.type" :id="item.id" :scene-id="item.scene_id" :task-kind="item.payload?.kind" :label="issueLabel(item,source.type)" :copyable="false" /></div>
          <p v-if="issues[source.key].data?.total===0&&!issues[source.key].error" class="muted">本次范围没有此类记录。</p>
          <p v-if="!issues[source.key].data&&!issues[source.key].loading&&!issues[source.key].error" class="muted">尚未取得此列表，不能判断数量。</p>
    ...
    <template v-if="data">
      <div class="connection-grid">
        ...
        <v-card class="connection-card"><v-card-text><div class="eyebrow">发送方式</div><div class="connection-value">{{ data.stats.shadow_mode?'Shadow 观察':'按各群规则发送' }}</div><p class="muted">{{ data.stats.shadow_mode?'表达只记录候选，不实际发送。':'通过 Gate 的表达按本群聊天、命令或公告资格投递。' }}</p><p v-if="app.status" class="muted">已启用 {{ app.status.scene_counts.enabled }} 个群，其中 {{ app.status.scene_counts.chat_enabled }} 个开放普通聊天。</p><v-btn variant="text" color="primary" size="small" :append-icon="mdiArrowRight" :to="{name:'settings',query:{tab:'delivery'}}">发送设置</v-btn></v-card-text></v-card>
