...
const tab = computed(() => ['waiting', 'system'].includes(route.query.tab) ? route.query.tab : 'reminders')
...
const editable = computed(() => detail.value && !isWork.value && !isSystem.value && ['pending', 'claimed', 'processing', 'result_ready', 'review_required'].includes(detail.value.status))
...
async function loadList() {
  ...
  const params = new URLSearchParams(clean({ scene_id: scalar(route.query.scene), status: routeStatus.value, kind: currentTab === 'system' ? 'system' : currentTab === 'reminders' ? 'reminder' : undefined, page: page.value, page_size: 30 }))
  try {
...
<template>
  <section class="tasks-page">
    <PageHeader :title="id ? (tab === 'waiting' ? '等待详情' : tab === 'system' ? '调度周期详情' : '提醒详情') : '提醒、等待与周期'" :description="'管理明确的提醒与等待。时间采用浏览器时区：' + zone + '。'">
      <v-btn v-if="id" variant="text" :prepend-icon="mdiArrowLeft" @click="close">返回{{ tab === 'waiting' ? '等待' : tab === 'system' ? '周期' : '提醒' }}列表</v-btn><v-btn variant="outlined" :prepend-icon="mdiRefresh" :loading="id ? detailLoading : loading" @click="refresh">刷新</v-btn>
    </PageHeader>
    <v-tabs :model-value="tab" color="primary" class="section-gap" @update:model-value="changeTab"><v-tab value="reminders">提醒</v-tab><v-tab value="waiting">等待回应</v-tab><v-tab value="system">心跳与兴趣分享</v-tab></v-tabs>
    <template v-if="!id">
      ...
      <v-card v-if="loaded && !rows.length && !listError" class="empty-state"><v-card-text>没有符合筛选条件的{{ tab === 'waiting' ? '等待记录' : tab === 'system' ? '调度周期' : '提醒' }}。</v-card-text></v-card><v-pagination v-if="pages > 1" :model-value="page" :length="pages" :total-visible="5" class="section-gap" @update:model-value="changePage" />
    </template>
    <template v-else>
