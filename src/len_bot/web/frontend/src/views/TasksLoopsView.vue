...
function fmtTime(timestamp) {
  return timestamp === null || timestamp === undefined ? '—' : Number.isFinite(new Date(timestamp * 1000).getTime()) ? new Date(timestamp * 1000).toLocaleString('zh-CN', { hour12: false }) : `${timestamp} Unix 秒（超出日期可表示范围）`
}
function absoluteTime(timestamp) {
  const value=new Date(timestamp*1000);
  return Number.isFinite(value.getTime()) ? value.toISOString() : `${timestamp} Unix 秒（超出日期可表示范围）`
}
...
function listQuery() {
  const { id: ignored, list_scene: originalScene, ...query } = route.query;
  if (originalScene !== undefined) query.scene = scalar(originalScene) || undefined;
  return query
}
function open(item) {
  router.push({ name: 'tasks', query: { ...listQuery(), list_scene: scalar(route.query.scene), id: item.id, scene: item.scene_id, tab: tab.value } })
}
function close() {
  router.push({ name: 'tasks', query: listQuery() })
}
function changeTab(value) {
  router.push({ name: 'tasks', query: { return_to: route.query.return_to, scene: scalar(route.query.scene) || undefined, tab: value, status: value === 'waiting' ? 'active' : '', page: 1 } })
}
function applyFilters() {
  router.push({ name: 'tasks', query: { ...clean({ return_to: route.query.return_to, scene: filters.value.scene }), tab: tab.value, status: filters.value.status, page: 1 } })
}
function changePage(value) {
  router.push({ name: 'tasks', query: { ...route.query, page: value } })
}
function triggerDescription(item) {
  return item.wake_event_type ? '事件条件：' + item.wake_event_type : '按预定时间触发'
}
function taskStatus(item) {
  return item.payload?.delivery_status === 'simulated' ? 'simulated' : item.status
}
...
function startEdit() {
  ...
  draft.value = { description: baseline.value.saved.description, time: baseline.value.time };
  editing.value = true;
  actionError.value = '';
  feedback.value = '';
...
function resolveConflict(keep) {
  ...
  if (keep && ['id','scene_id','created_at','source_event_id'].some(key=>current[key]!==before[key])) {
    actionError.value = '原提醒身份已改变，不能把旧草稿接到另一条提醒；请采用现值后重新编辑。';
    return
  }
  ...
  baseline.value={saved:current,time:inputTime(current.due_at)};
  draft.value=next;
  conflict.value=false;
  actionError.value=''
...
async function submitAction() {
  ...
  try {
    ...
    if (fresh()) {
      ...
      confirmation.value = null;
      if (error.status === 409 && error.details != null) {
        if(value.operation==='update')conflict.value=true;
        await loadDetail({accept:fresh})
      }
    }
...
<template>
  <section class="tasks-page">
    <PageHeader
      :title="id ? detailTitle : '提醒、等待与周期'"
      :description="'提醒、等待回应、原行动延期与系统周期分别查看。时间采用浏览器时区：' + zone + '。'"
    >
      <v-btn v-if="id" variant="text" :prepend-icon="mdiArrowLeft" @click="close">返回{{ tabLabel }}列表</v-btn>
      <v-btn
        variant="outlined"
    ...
    >
      <v-tab value="reminders">提醒</v-tab>
      <v-tab value="waiting">等待回应</v-tab>
      <v-tab value="deferred">延期交付</v-tab>
      <v-tab value="system">心跳与兴趣分享</v-tab>
    </v-tabs>
    <template v-if="!id">
      <v-card class="section-gap">
        <v-card-text>
          <v-form class="task-filters" @submit.prevent="applyFilters">
            <ScopeSelect v-model="filters.scene" clearable />
            <v-select v-model="filters.status" :items="statusOptions" label="状态" hide-details />
            <v-btn type="submit" color="primary">筛选</v-btn>
          </v-form>
          <p v-if="tab === 'reminders'" class="work-guide">本列表只显示提醒。<RouterLink
              :to="{ name: 'jobs', query: clean({ return_to: route.query.return_to, scene: scalar(route.query.scene) }) }"
            >查看信息工作</RouterLink>
          </p>
        </v-card-text>
      </v-card>
      <v-alert v-if="listError" type="error" variant="tonal" title="列表读取失败" class="section-gap">
        {{ listError }}<div v-if="readAt">保留上次读取结果：{{ fmtTime(readAt) }}</div>
      </v-alert>
      <v-progress-linear v-if="loading" indeterminate class="section-gap" aria-label="正在读取列表" />
      <div v-if="loaded" class="list-meta">
      ...
      <div class="task-list">
        <v-card v-for="item in rows" :key="item.id" tag="article" class="task-row">
          <div class="task-main">
            <RouterLink
              class="record-title two-lines"
              :to="{ name: 'tasks', query: { ...listQuery(), list_scene: scalar(route.query.scene), tab, id: item.id, scene: item.scene_id } }"
            >
              {{ tab === 'waiting' ? item.intent : item.description }}
            </RouterLink>
            <span v-if="tab === 'waiting'" class="target-name">等待对象：{{ item.target_actor_id }}
            </span>
            <span v-if="tab === 'deferred'" class="target-name">
              {{ deferredPhases[item.payload.delivery_phase] || '旧记录未保存延期阶段' }} · 原定 {{ fmtTime(item.payload.original_due_at) }}
            </span>
            <EntityLink
              v-if="tab === 'waiting'"
              type="event"
          ...
          <div class="task-time">
            <template v-if="tab === 'waiting'">
              ...
              <span>到期 {{ fmtTime(item.expires_at) }}</span>
            </template>
            <template v-else>
              <span>{{ tab === 'deferred' ? '下次释放时间' : '预定' }} {{ fmtTime(item.due_at) }}</span>
              <span class="auxiliary">{{ triggerDescription(item) }}</span>
            </template>
          </div>
          <div>
            <StatusBadge
              :domain="tab === 'waiting' ? 'waiting' : tab === 'system' ? 'schedule_slot' : 'task'"
              :status="tab === 'waiting' ? item.status : taskStatus(item)"
            />
          </div>
          <v-btn variant="tonal" @click="open(item)">查看详情</v-btn>
        </v-card>
      </div>
      <v-card v-if="loaded && !rows.length && !listError" class="empty-state">
        <v-card-text>没有符合筛选条件的{{ tabLabel }}记录。</v-card-text>
      </v-card>
      <v-pagination
        v-if="pages > 1"
        :model-value="page"
    ...
    <template v-else>
      <v-alert
        ...
        variant="tonal"
        :title="detailMissing ? '对象不存在或不属于此场景' : '详情读取失败'"
        class="section-gap"
      >
        {{ detailError }}<div v-if="detailReadAt">上次读取：{{ fmtTime(detailReadAt) }}</div>
      </v-alert>
      <v-skeleton-loader v-if="detailLoading && !detail" type="article" class="section-gap" />
      ...
      <v-card v-if="editing&&!detail" class="section-gap">
        <v-card-text>
          <p>当前提醒不可读取，原草稿保留，未套用到其他提醒。</p>
          <ResourceViewer title="未保存的提醒修改" :content="draft" />
          <v-btn variant="text" :disabled="saving" @click="cancelEdit">放弃提醒草稿</v-btn>
        </v-card-text>
      </v-card>
      <template v-if="detail">
        <v-card class="section-gap">
          <v-card-text>
            <h2 class="full-copy">{{ tab === 'waiting' ? detail.intent : detail.description }}</h2>
            <div class="identity-line">
              <span class="object-id">{{ detail.id }}</span>
              <EntityLink type="scene" :id="detail.scene_id" :scene-id="detail.scene_id" />
              <StatusBadge
                :domain="tab === 'waiting' ? 'waiting' : isWork ? 'job_delivery' : isSystem ? 'schedule_slot' : 'task'"
            ...
            <template v-if="tab === 'waiting'">
              ...
              <ResourceViewer
                ...
                title="挂起请求与原预算"
                :content="detail.resume_state"
              />
              <h3 class="section-title">来源消息</h3>
              <EntityLink type="event" :id="detail.source_event_id" :scene-id="detail.scene_id" />
              <div class="action-row">
                <v-btn
                  color="error"
                  variant="outlined"
                  :disabled="saving || detailLoading || !!detailError || !!readbackPending || !['active','review_required'].includes(detail.status)"
                  @click="askAction('resolve')"
                >人工结束等待</v-btn>
              </div>
            </template>
            <template v-else-if="isSystem">
              ...
              <EntityLink
                ...
                type="job"
                :id="detail.payload.job_id"
                :scene-id="detail.scene_id"
                label="关联研究工作"
              />
              <EntityLink
                v-if="detail.trigger_event_id"
                type="event"
                :id="detail.trigger_event_id"
                :scene-id="detail.scene_id"
                label="实际触发事件"
              />
              <ResourceViewer title="周期结果与跳过原因" :content="detail.payload" />
            ...
            <template v-else-if="isDeferred">
              ...
              <dl class="fact-list">
                ...
                <div>
                  <dt>延期阶段</dt>
                  <dd>{{ deferredPhases[detail.payload.delivery_phase] || '旧记录未保存' }}</dd>
                </div>
              </dl>
              <p class="breakable">原行动 {{ detail.payload.action_id }}</p>
              <p class="deferred-reason">{{ detail.payload.detail || '未保存延期原因' }}</p>
              <div v-if="deferredAction" class="link-list mt-4">
                <p>原请求者 QQ {{ deferredAction.requester_qq_uid || '未记录人类请求者' }} · 原表达模式 {{ deferredAction.origin_mode || '未记录' }}
                </p>
                <p>原回应对象：{{ deferredAction.response_actor_ids?.join('、') || '未记录' }}</p>
                <EntityLink
                  v-if="deferredAction.origin_event_id"
                  type="event"
                  :id="deferredAction.origin_event_id"
                  :scene-id="detail.scene_id"
                  label="原请求或触发事件"
                />
                <EntityLink
                  v-if="deferredAction.job_id"
                  type="job"
                  :id="deferredAction.job_id"
                  :scene-id="detail.scene_id"
                  :label="`回到原工作（行动依据 v${deferredAction.job_revision ?? '未记录'}）`"
                />
                <EntityLink
                  v-else-if="deferredAction.fulfils_task_id"
                  type="task"
                  :id="deferredAction.fulfils_task_id"
                  :scene-id="detail.scene_id"
                  label="回到原提醒"
                />
                <EntityLink
                  v-if="deferredAction.file_asset_id"
                  type="file"
                  :id="deferredAction.file_asset_id"
                  :job-id="deferredAction.job_id"
                  :scene-id="detail.scene_id"
                  label="查看文件资产与上传回执"
                />
              </div>
              <div class="identity-line">
                <span>保存的交付结局</span>
                <StatusBadge
                  v-if="detail.payload.delivery_status"
                  domain="delivery"
            ...
            <template v-else>
              <dl class="fact-list">
                <div><dt>预定时间</dt><dd>{{ fmtTime(detail.due_at) }}</dd></div>
                <div><dt>触发方式</dt><dd>{{ triggerDescription(detail) }}</dd></div>
                <div><dt>创建时间</dt><dd>{{ fmtTime(detail.created_at) }}</dd></div>
              </dl>
              <div class="action-row">
                <v-btn
                  variant="outlined"
                  :prepend-icon="mdiPencilOutline"
                  :disabled="!editable || saving || editing || detailLoading || !!detailError || !!readbackPending"
                  @click="startEdit"
                >修改提醒</v-btn>
                <v-btn
                  variant="outlined"
                  :prepend-icon="mdiClockFast"
                  :disabled="detail.status !== 'pending' || saving || editing || detailLoading || !!detailError || !!readbackPending"
                  @click="askAction('trigger_now')"
                >立即触发</v-btn>
                <v-btn
                  color="error"
                  variant="outlined"
          ...
          <v-card-text>
            <v-alert v-if="conflict" type="warning" variant="tonal" class="mb-4">
              ...
              <div class="action-row">
                <v-btn
                  variant="outlined"
                  :disabled="!editable||saving||detailLoading||!!detailError||!!readbackPending"
            ...
            <v-textarea
              ...
              rows="3"
              auto-grow
              :disabled="saving||!!readbackPending"
            />
            <v-text-field
              v-model="draft.time"
              type="datetime-local"
            ...
            <div class="action-row">
              <v-btn
                color="primary"
                :disabled="!editable || !dirty || !draft.description.trim() || !Number.isFinite(draftDue) || conflict || saving || detailLoading || !!detailError || !!readbackPending"
        ...
        <v-card v-if="tab !== 'waiting'" class="section-gap">
          <v-card-text class="detail-body">
            <ResourceViewer
              v-if="detail.payload.result !== undefined"
              title="完整结果"
              :content="detail.payload.result"
            />
            <v-alert v-if="detail.payload.error" type="warning" variant="tonal" class="mt-4">
              {{ detail.payload.error }}
            </v-alert>
            <h3 class="section-title">原始来源</h3>
            <div class="link-list">
              <EntityLink
                v-for="eventId in detail.payload.source_event_ids || []"
                :key="eventId"
                type="event"
                :id="eventId"
                :scene-id="detail.scene_id"
              />
              <EntityLink
                v-if="detail.payload.delivery_event_id"
                type="event"
                :id="detail.payload.delivery_event_id"
                :scene-id="detail.scene_id"
                label="查看发送回执"
              />
            </div>
            <v-expansion-panels variant="accordion" class="mt-4">
              <v-expansion-panel title="触发条件与完整任务记录">
                <v-expansion-panel-text>
                  <ResourceViewer title="触发匹配条件" :content="detail.wake_match" />
                  <ResourceViewer title="完整任务记录" :content="detail" />
                </v-expansion-panel-text>
              </v-expansion-panel>
            </v-expansion-panels>
          </v-card-text>
    ...
    >
      <v-card>
        <v-card-title class="dialog-title">{{ actionTitle }}</v-card-title>
        <v-card-text v-if="confirmation">
          <p class="full-copy">{{ confirmation.description }}</p>
          <p class="auxiliary">对象 {{ confirmation.id }} · {{ confirmation.scene }} · 原状态 {{ confirmation.baseline.status }}
          </p>
          <details class="my-3">
            <summary>核对本次选择的原值</summary>
            <ResourceViewer title="确认时选择的对象与触发条件" :content="confirmation.baseline" />
          </details>
        ...
        <v-card-actions class="dialog-actions">
          ...
          <v-btn
            :color="['cancel', 'resolve'].includes(confirmation?.operation) ? 'error' : 'primary'"
            :loading="saving"
            :disabled="saving || detailLoading || !!detailError || !!readbackPending"
            @click="submitAction"
          >确认{{ confirmation?.operation === 'update' ? '保存' : confirmation?.operation === 'trigger_now' ? '触发' : confirmation?.operation === 'cancel' ? '取消' : '结束' }}
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
<style scoped>
.breakable,.deferred-reason{overflow-wrap:anywhere}
.deferred-reason{white-space:pre-wrap;line-height:1.7;margin-top:12px}
.tasks-page{min-width:0}
.tasks-page>.page-header{margin-bottom:24px}
.tasks-page>.page-header+.v-tabs{margin-top:0}
.section-gap{margin-top:20px}
.task-filters{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) auto;gap:12px;align-items:center}
.work-guide{margin:8px 0 0;font-size:13px;color:rgb(var(--v-theme-on-surface-variant))}
.list-meta{display:flex;gap:12px;justify-content:space-between;flex-wrap:wrap;margin:16px 0;font-size:13px;color:rgb(var(--v-theme-on-surface-variant))}
.task-list{display:grid;gap:12px}
.task-row{padding:18px;display:grid;grid-template-columns:minmax(0,1fr) minmax(180px,220px) minmax(80px,110px) auto;gap:20px;align-items:center}
.task-main{display:grid;gap:8px;min-width:0}
.record-title{font-size:15px;font-weight:600;line-height:1.6;color:rgb(var(--v-theme-primary));text-decoration:none}
...
.task-time{display:grid;gap:8px;font-size:13px;line-height:1.5}
.target-name{font-size:13px;overflow-wrap:anywhere}
.auxiliary{color:rgb(var(--v-theme-on-surface-variant));font-size:13px;line-height:1.55}
.full-copy{white-space:pre-wrap;overflow-wrap:anywhere;line-height:1.6;font-size:19px;font-weight:600}
.identity-line,.action-row{display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin:14px 0}
.object-id{overflow-wrap:anywhere;font-size:13px}
.fact-list{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:20px;margin:24px 0}
.fact-list dt{font-size:13px;color:rgb(var(--v-theme-on-surface-variant));margin-bottom:6px}
.fact-list dd{margin:0;line-height:1.6;overflow-wrap:anywhere}
.section-title{font-size:17px;margin:20px 0 12px}
.link-list{display:grid;gap:10px}
.edit-card{max-width:900px}
.time-confirm{display:block;font-size:13px;color:rgb(var(--v-theme-on-surface-variant));overflow-wrap:anywhere;margin-top:8px}
...
@media(max-width:1150px){
  .task-row{grid-template-columns:minmax(0,1fr) auto;gap:16px}
  .task-time{grid-column:1;grid-row:2}
  .task-row>div:nth-child(3){grid-column:1;grid-row:3}
  .task-row>.v-btn{grid-column:2;grid-row:1/4}
}
@media(max-width:650px){
  .task-filters,.fact-list{grid-template-columns:1fr}
  .task-row{grid-template-columns:1fr;padding:16px}
  .task-time,.task-row>div:nth-child(3),.task-row>.v-btn{grid-column:auto;grid-row:auto}
  .task-row>.v-btn{justify-self:start}
  .identity-line{align-items:flex-start;flex-direction:column}
  .action-row>.v-btn{flex-grow:1}
  .full-copy{font-size:18px}
}
</style>
