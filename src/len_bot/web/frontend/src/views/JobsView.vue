...
const artifactDownloadUrl=path=>`/api/cockpit/jobs/${encodeURIComponent(job.value.id)}/workspace-artifact/download?` + new URLSearchParams({ scene_id: job.value.scene_id, path })
...
async function loadWorkspaceArtifact(path, offset = 0) {
  ...
  try { const value = await api(`/api/cockpit/jobs/${encodeURIComponent(job.value.id)}/workspace-artifact?` + new URLSearchParams({ scene_id: job.value.scene_id, path, offset }))
