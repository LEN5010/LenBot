    ...
    <template v-if="!jobId">
      ...
      <div class="work-list">
        <v-card v-for="item in rows" :key="item.id" tag="article" class="work-row">
          <div class="work-main"><RouterLink :to="{ name: 'job', params: { jobId: item.id }, query: { ...listQuery(), list_scene: scalar(route.query.scene), scene: item.scene_id, tab: 'result' } }" class="two-lines record-title">{{ item.goal }}</RouterLink><EntityLink type="scene" :id="item.scene_id" :scene-id="item.scene_id" /></div>
      ...
      <template v-if="job">
        ...
        <v-card class="section-gap">
          ...
          <v-card-text v-if="tab === 'result'" class="detail-body">
            ...
            <h3>当前要求</h3><ul v-if="job.constraints.length"><li v-for="(item, index) in job.constraints" :key="index">{{ item }}</li></ul><p v-else class="muted-copy">没有附加要求。</p><h3>原始请求与修订来源</h3><div class="link-list"><EntityLink v-for="id in job.source_event_ids" :key="id" type="event" :id="id" :scene-id="job.scene_id" /></div>
          </v-card-text>
          <v-card-text v-else-if="tab === 'progress'" class="detail-body">
            <template v-if="job.work_state"><div class="status-line"><h3>已保存进度 · 目标版本 {{ job.work_state.goal_revision }}</h3><v-chip v-if="job.work_state.goal_revision !== job.revision" color="warning" variant="tonal">旧目标进度，需重新核对</v-chip></div><h4>计划</h4><ol><li v-for="(item, index) in job.work_state.plan" :key="index">{{ item }}</li></ol><h4>已完成步骤与依据</h4><ul><li v-for="(step, index) in job.work_state.completed_steps" :key="index">{{ step.step }}<div class="action-row"><v-btn v-for="id in step.result_ids" :key="id" size="small" variant="text" @click="openResource(id)">原始资料 · {{ id.slice(0, 10) }}</v-btn></div></li></ul><h4>待解决</h4><ul><li v-for="(item, index) in job.work_state.unresolved" :key="index">{{ item }}</li></ul><p>下一步：{{ job.work_state.next_step || '尚未提供' }}</p></template><p v-else class="muted-copy">尚无已保存的进度。</p>
            ...
            <ResourceViewer v-if="route.query.resource" title="原始工具资料" :content="resourceText" :loading="resourceLoading" :error="resourceError" /><div v-if="resource" class="resource-meta"><p>取得时间 {{ fmtTime(resource.fetched_at) }} · 原状态 {{ resource.status }}</p><p v-for="(source, index) in resource.sources" :key="index">{{ source.title }}<span class="source-url">{{ source.url }}</span></p><v-btn v-if="resource.next_offset !== null" :loading="resourceLoading" :disabled="resourceLoading" variant="outlined" @click="loadResource(scalar(route.query.resource), resource.next_offset)">继续读取资料</v-btn></div>
