...
function run(trace) { return trace.payload?.conversation || trace.payload?.cognition || trace.payload || {} }
...
function kindLabel(kind) { return { conversation: '对话', conversation_error: '对话失败', agent_job: '信息工作', agent_job_error: '工作失败', history_maintenance: '历史维护', history_maintenance_error: '历史维护失败', work_compression: '工作压缩', skill_maintenance: '技能整理' }[kind] || kind }
...
function commitLabel(trace) { return trace.payload?.gate?.accepted === true || trace.kind === 'history_maintenance' ? '已提交' : commitRejected(trace) ? '已拒绝' : '暂无提交回执' }
function resultLabel(trace) {
  if (trace.kind === 'history_maintenance') return '摘要与认识已提交'
  if (trace.kind === 'history_maintenance_error') return '历史维护失败'
...
<template>
  <div class="trace-view">
    ...
    <form class="toolbar filter-bar" @submit.prevent="load"><input v-model="filters.scene_id" placeholder="场景，例如 group:123" /><select v-model="filters.kind"><option value="">全部记录</option><option value="conversation">对话</option><option value="conversation_error">对话失败</option><option value="agent_job">信息工作</option><option value="agent_job_error">工作失败</option><option value="history_maintenance">历史维护</option><option value="history_maintenance_error">历史维护失败</option><option value="work_compression">工作压缩</option><option value="skill_maintenance">技能整理</option></select><button>筛选</button></form>
    ...
    <section v-if="selected" class="panel detail-panel"><div class="panel-header"><div><div class="bento-badge">{{ selected.scene_id }} · {{ fmtTime(selected.created_at) }}</div><h2>{{ resultLabel(selected) }}</h2></div><button @click="selected = null">关闭</button></div>
      ...
      <h3>本轮唤醒来源</h3><div v-for="source in run(selected).wake_sources || []" :key="source.event_id" class="kv"><code>{{ source.event_id }}</code><span>{{ source.reasons?.map(attentionReason).join('、') }}</span></div><p v-if="!run(selected).wake_sources?.length" class="muted">此记录没有附唤醒来源</p>
