...
function steps(trace) { return run(trace).steps || [] }
...
function toolCount(trace) { return steps(trace).reduce((count, step) => count + (step.tool_calls?.length || 0), 0) }
...
function failures(trace) {
  const payload = trace.payload || {}, values = [payload.error, payload.failure_reason, run(trace).failure_reason]
  ...
  for (const step of steps(trace)) {
    ...
    for (const call of step.tool_calls || []) values.push(call.error)
  }
  return [...new Set(values.filter(Boolean).map(errorText))]
}
function resultLabel(trace) {
  if (trace.payload?.gate?.accepted === false) return '提交被拒绝'
  if (failures(trace).length && !trace.payload?.result) return '运行未完成'
...
function brief(trace) { return trace.payload?.result?.decision_reason || candidate(trace)?.note || trace.payload?.result?.summary || failures(trace)[0] || '未留下简短说明' }
...
<template>
  <div class="trace-view">
    ...
    <section v-if="selected" class="panel detail-panel"><div class="panel-header"><div><div class="bento-badge">{{ selected.scene_id }} · {{ fmtTime(selected.created_at) }}</div><h2>{{ resultLabel(selected) }}</h2></div><button @click="selected = null">关闭</button></div>
      ...
      <div class="detail-grid"><div><h3>输入与结果</h3><p class="detail-copy">{{ sourceText(selected) }}</p><p class="muted">{{ brief(selected) }}</p><div class="kv"><span class="k">读取截点</span><span class="v">{{ run(selected).read_cutoff ?? '—' }}</span></div><div class="kv"><span class="k">提交结果</span><span class="v">{{ selected.payload?.gate ? selected.payload.gate.accepted ? '已提交' : '已拒绝' : '暂无提交回执' }}</span></div></div><div><h3>表达提案</h3><article v-for="(message, index) in messages(selected)" :key="index" class="proposed-message"><span class="tag">第 {{ index + 1 }} 条</span><p>{{ messageText(message) }}</p><span v-if="message.reply_to" class="muted">引用消息 {{ message.reply_to }}</span></article><p v-if="!messages(selected).length" class="muted">没有表达提案</p><p class="muted">提案与实际送达分别记录，送达情况见场景回执。</p></div></div>
      ...
      <details v-if="candidate(selected)" class="detail-record"><summary>终结候选</summary><pre>{{ JSON.stringify(candidate(selected), null, 2) }}</pre></details>
