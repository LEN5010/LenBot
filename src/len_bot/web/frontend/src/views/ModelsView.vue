...
function adoptUnknownOutcome() {
  ...
  if(kind==='provider'){
    ...
    retrievalOpen.value=false;adoptRetrieval(data.value.retrieval)
  }else clearCatalog(kind.slice(kind.indexOf(':')+1))
  conflicts.clear(kind);saveOutcome.value=null;outcomeReadAt.value=null;error.value=''
...
async function saveRetrieval() {
  ...
  try { const result = await submitConfiguration('/api/models/retrieval',{method:'POST',body:JSON.stringify({baseline:retrievalBaseline.value,values:retrievalValues()})},progress); await readSaved(result, 'retrieval', fresh) }
  catch(e){ await saveFailure(e, 'retrieval', fresh, progress) } finally { if(fresh())busy.value='' }
}
...
async function saveProvider() {
  ...
  try {
    ...
    const result = await submitConfiguration('/api/models/providers', { method: 'POST', body: JSON.stringify({baseline:providerBaseline.value,values:body}) },progress)
...
async function deleteProvider(provider) {
  ...
  } catch (e) { await saveFailure(e, `delete:${provider.id}`, fresh, progress) }
  finally { if (fresh()) busy.value = '' }
}
...
async function saveModels(provider) {
  ...
  const progress={submitted:false,providerId:provider.id}
  try {
    const result = await submitConfiguration(`/api/models/providers/${encodeURIComponent(provider.id)}/models`, { method: 'POST', body: JSON.stringify({baseline:JSON.parse(modelOriginal.value[provider.id]),values:{models:modelNames(selectedModels.value[provider.id])}}) },progress)
    await readSaved(result, `models:${provider.id}`, fresh)
  } catch (e) { await saveFailure(e, `models:${provider.id}`, fresh, progress) }
  finally { if (fresh()) busy.value = '' }
}
...
async function saveRouting() {
  ...
  try {
    ...
    const result = await submitConfiguration('/api/models/routing', { method: 'POST', body: JSON.stringify({baseline:routingBaseline.value,values:routing}) },progress)
    await readSaved(result, 'routing', fresh)
  } catch (e) { await saveFailure(e, 'routing', fresh, progress) }
  finally { if (fresh()) busy.value = '' }
}
...
<template>
  <div class="page-stack">
    <PageHeader title="模型设置" description="三个职责显式配置，每次运行固定提供商、模型和推理强度。"><v-btn variant="outlined" :loading="loading||reservationLoading" :disabled="!!busy" @click="refresh">刷新</v-btn><v-btn v-if="tab==='roles'" color="primary" :disabled="!loaded||loading||!!busy||writeHeld" @click="editRouting">编辑职责配置</v-btn><v-btn v-else color="primary" :disabled="!loaded||loading||!!busy||writeHeld" @click="editProvider()">添加供应商</v-btn></PageHeader>
    ...
    <v-alert v-if="saveOutcome" type="warning" variant="tonal" class="mb-4">
      ...
      <v-btn variant="text" :disabled="!!busy||loading||outcomeReadAt===null" @click="adoptUnknownOutcome">采用当前值继续操作</v-btn>
    </v-alert>
    ...
    <v-card v-if="loaded" class="pa-4 retrieval-card"><div class="role-title"><h2>语义检索</h2><div class="actions"><v-chip size="small" :color="data.retrieval?.embedding ? 'primary' : 'default'">{{ data.retrieval?.embedding ? '已绑定' : '未启用' }}</v-chip><v-btn variant="outlined" size="small" :disabled="!!busy||writeHeld" @click="editRetrieval">编辑</v-btn></div></div><p class="muted">只在 Agent 主动查询认识时使用；索引失败不会回滚已提交认识。Embedding 与 rerank 请求单独计量。</p><p v-if="data.retrieval?.embedding" class="auxiliary">Embedding：{{ data.retrieval.embedding.provider_id }} · {{ data.retrieval.embedding.model }}<span v-if="data.retrieval.embedding.dimension"> · {{ data.retrieval.embedding.dimension }} 维</span></p><p v-if="data.retrieval?.rerank" class="auxiliary">Rerank：{{ data.retrieval.rerank.provider_id }} · {{ data.retrieval.rerank.model }}</p></v-card>
    ...
    <template v-else>
      ...
      <v-card v-for="id in orphanCatalogs" :key="`missing:${id}`" class="pa-5 provider-card">
        ...
        <v-btn class="mt-3" variant="outlined" :disabled="!!busy||writeHeld" @click="cancelCatalog(id)">放弃这份目录草稿</v-btn>
      </v-card>
      <v-card v-for="provider in data.providers" :key="provider.id" class="pa-5 provider-card">
        ...
        <div class="actions"><v-btn variant="outlined" :disabled="!!busy||loading||writeHeld" @click="editProvider(provider)">编辑接口</v-btn><v-btn color="error" variant="text" :disabled="!!busy||loading||writeHeld||inUse(provider.id)||(!!conflicts.entries[`delete:${provider.id}`]&&!conflicts.entries[`delete:${provider.id}`].snapshot)" @click="deleteProvider(provider)">删除</v-btn></div>
        ...
        <template v-if="catalogs[provider.id]">
          ...
          <ConfigConflictBanner v-else :conflict="conflicts.entries[`models:${provider.id}`]?.problem" :current="conflicts.entries[`models:${provider.id}`]?.snapshot" :path-label="conflicts.entries[`models:${provider.id}`]?.problem.path.join(' → ')" :read-at="conflicts.entries[`models:${provider.id}`]?.readAt" :read-error="conflicts.entries[`models:${provider.id}`]?.readError" :busy="!!busy||loading||writeHeld" @keep="resolveConflict(`models:${provider.id}`,true)" @take="resolveConflict(`models:${provider.id}`,false)" @reload="refresh">
          ...
          <v-autocomplete :disabled="!!busy||loading||writeHeld||!!catalogInvalid[provider.id]" v-model="selectedModels[provider.id]" :items="[...new Set([...catalogs[provider.id],...provider.models,...selectedModels[provider.id]])]" label="搜索并选择常用模型" multiple chips closable-chips clearable />
          <div class="actions"><v-btn color="primary" :loading="busy===`models:${provider.id}`" :disabled="!!busy||loading||writeHeld||!!catalogInvalid[provider.id]||!!conflicts.entries[`models:${provider.id}`]" @click="saveModels(provider)">保存常用模型</v-btn><v-btn variant="text" :disabled="!!busy||writeHeld" @click="cancelCatalog(provider.id)">取消选择</v-btn></div>
        </template>
        <template v-else>
          ...
          <v-btn class="mt-4" variant="tonal" :loading="busy===`catalog:${provider.id}`" :disabled="!!busy||loading||writeHeld||!!catalogUnavailable(provider)" @click="fetchModels(provider)">获取接口模型目录</v-btn>
        </template>
      </v-card>
    ...
    <v-dialog :model-value="providerOpen" max-width="650" scrollable :persistent="!!busy" @update:model-value="value=>!value&&closeProvider()">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-form v-if="providerForm" :disabled="!!busy||loading||writeHeld" class="config-form" @submit.prevent="saveProvider">
            <v-alert v-if="error" type="error" variant="tonal">{{ error }}</v-alert>
            <v-text-field v-model="providerForm.id" label="供应商名称" :disabled="!!editingProvider||!!busy||loading||writeHeld" required />
            ...
            <v-btn type="submit" color="primary" :loading="busy==='provider'" :disabled="!!busy||loading||writeHeld||!!providerConflict||!canSaveProvider">保存供应商</v-btn>
          </v-form>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog :model-value="routesOpen" max-width="900" scrollable :persistent="!!busy" @update:model-value="value=>!value&&closeRouting()">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-alert v-if="readbackPending" type="warning" variant="tonal" class="mb-4">已写入配置，待读回真实值。<v-btn variant="text" :disabled="!!busy" :loading="loading" @click="refresh">重新读取保存值</v-btn></v-alert>
          <ConfigConflictBanner :conflict="routingConflict?.problem" :current="routingConflict?.snapshot" :path-label="routingConflict?.problem.path.join(' → ')" :read-at="routingConflict?.readAt" :read-error="routingConflict?.readError" :busy="!!busy||loading||writeHeld" @keep="resolveConflict('routing',true)" @take="resolveConflict('routing',false)" @reload="refresh">
          ...
          <v-form v-if="routingForm" :disabled="!!busy||loading||writeHeld" @submit.prevent="saveRouting">
            <section v-for="role in roles" :key="role.key" class="routing-section">
              ...
              <template v-if="roleEnabled[role.key]">
                ...
                <v-btn variant="text" color="primary" :disabled="!!busy||writeHeld||!providerById(routingForm[role.key].provider_id)?.enabled||!routingForm[role.key].model?.trim()" @click="testConfirm={name:role.name,profile:profile(role.key)}">检查当前选择（不保存）</v-btn>
              </template>
            ...
            <v-btn type="submit" color="primary" :loading="busy==='routing'" :disabled="!!busy||loading||writeHeld||!!routingConflict||!canSaveRouting||!routingDirty">保存职责配置</v-btn>
          </v-form>
        </v-card-text>
      </v-card>
    </v-dialog>
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-form :disabled="!!busy||loading||writeHeld" class="config-form" @submit.prevent="saveRetrieval">
            ...
            <v-btn type="submit" color="primary" :loading="busy==='retrieval'" :disabled="!!busy||loading||writeHeld||!!retrievalConflict||!!retrievalIssue||!retrievalDirty">保存语义检索配置</v-btn>
          </v-form>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog :model-value="!!testConfirm" max-width="560" :persistent="busy==='test'" @update:model-value="value=>!value&&(testConfirm=null)"><v-card v-if="testConfirm" title="主动能力检查"><v-card-text><p>检查 {{ testConfirm.name }}：{{ testConfirm.profile.provider_id }} / {{ testConfirm.profile.model }}。</p><p class="mt-3">将进行最多 2 次真实模型请求，检查原图、指定工具与原生续接，可能产生供应商费用。检查使用合成资料，不向群聊发送消息。</p><v-alert v-if="error" type="error" variant="tonal" class="mt-3">{{ error }}</v-alert></v-card-text><v-card-actions><v-spacer /><v-btn :disabled="busy==='test'" @click="testConfirm=null">取消</v-btn><v-btn color="primary" :loading="busy==='test'" :disabled="writeHeld" @click="testRoute">确认发起检查</v-btn></v-card-actions></v-card></v-dialog>
  </div>
</template>
<style scoped>
