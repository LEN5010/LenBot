...
const reservations = ref(null), reservationError = ref(''), reservationLoading = ref(false), reservationReadAt = ref(null)
const selection = () => `${route.name}:${tab.value}`
const readGuard = useRequestGuard(selection), operationGuard = useRequestGuard(selection), reservationGuard = useRequestGuard(selection)
...
async function loadReservations() {
  ...
  reservationError.value = ''; reservationLoading.value = true
  try {
    const result = await api('/api/models/reservations')
    if (fresh()) { reservations.value = result; reservationReadAt.value = Date.now()/1000 }
  }
  catch (e) { if (fresh()) reservationError.value = e.message }
  finally { if (fresh()) reservationLoading.value = false }
}
...
async function saveFailure(problem, kind, fresh) {
  if (!fresh()) return
  if (problem.details?.config_saved === true) {
    error.value = problem.message; readbackPending.value = kind
  } else if (conflicts.mark(kind, problem)) await load({ accept: fresh })
  else {
    if (kind.startsWith('models:') && problem.status === 404) {
      catalogInvalid.value[kind.slice(7)] = problem.message
      await load({ accept: fresh })
    }
...
function adoptProvider(provider) {
  editingProvider.value = provider.id; providerBaseline.value = clone(provider); providerRemoved.value = false
  writeProvider(provider); providerOriginal.value = JSON.stringify(providerForm.value)
}
...
function clearCatalog(id) {
  delete catalogs.value[id]; delete selectedModels.value[id]; delete modelOriginal.value[id]
  delete catalogProviders.value[id]; delete catalogInvalid.value[id]; conflicts.clear(`models:${id}`)
}
function cancelCatalog(id) {
  if (busy.value || readbackPending.value) return
...
function initialiseRouting(routing = data.value.routing) {
  routingBaseline.value=clone(routing); writeRouting(routing)
...
function changeRoutingBinding(key, field, value) {
  ...
  routingForm.value[key].reasoning_effort = ''; routingForm.value[key].supports_vision = false
}
...
function adoptRetrieval(retrieval) {
  retrievalBaseline.value=clone(retrieval); writeRetrieval(retrieval)
...
function changeRetrievalBinding(key, field, value) {
  ...
  retrievalForm.value[key].dimension = null; retrievalForm.value[key].protocol = null
}
function retrievalValues(checkProviders = true) {
  return Object.fromEntries(['embedding', 'rerank'].map(key => {
    const value = retrievalForm.value[key], provider = value.provider_id?.trim() || '', model = value.model?.trim() || ''
...
const retrievalIssue = computed(() => { try { retrievalValues(); return '' } catch(problem) { return problem.message } })
...
function resolveConflict(kind, keep) {
  ...
  if (busy.value || loading.value || readbackPending.value || !snapshot) return
  if (!keep && !window.confirm('放弃此项未保存的修改，采用本次读取的保存值？其他配置草稿不变。')) return
  try {
    if (kind === 'provider') {
      if (keep && providerKeepBlocked.value) return
      const key = {api_key:providerForm.value.api_key, api_key_action:providerForm.value.api_key_action}
      ...
      if (snapshot.provider) {
        ...
        providerOpen.value=false; providerForm.value=null; providerBaseline.value=null; editingProvider.value=''
      }
    } else if (kind === 'routing' || kind === 'retrieval') {
      const current = snapshot.values, original = kind === 'routing' ? routingBaseline.value : retrievalBaseline.value
      ...
      if (kind === 'routing') { initialiseRouting(current); if(keep)writeRouting(next) }
      else { adoptRetrieval(current); if(keep)writeRetrieval(next) }
      ...
      selectedModels.value[id]=next;modelOriginal.value[id]=JSON.stringify(current)
    }
    conflicts.clear(kind); error.value=''
...
function editRetrieval() {
  if (busy.value || loading.value || readbackPending.value || !loaded.value) return
  conflicts.clear('retrieval'); error.value=''; message.value=''
  adoptRetrieval(data.value.retrieval); retrievalOpen.value = true
}
function closeRetrieval() { if (busy.value) return; if (retrievalDirty.value && !window.confirm('放弃尚未保存的语义检索配置？')) return; retrievalOpen.value = false; conflicts.clear('retrieval') }
async function saveRetrieval() {
  if (busy.value || loading.value || readbackPending.value || retrievalConflict.value || !retrievalForm.value || retrievalIssue.value) return
  const fresh = beginOperation('retrieval')
  try { const result = await api('/api/models/retrieval',{method:'POST',body:JSON.stringify({baseline:retrievalBaseline.value,values:retrievalValues()})}); await readSaved(result, 'retrieval', fresh) }
  catch(e){ await saveFailure(e, 'retrieval', fresh) } finally { if(fresh())busy.value='' }
}
async function load({ accept = () => true } = {}) {
  ...
  try {
    ...
    if (readbackPending.value === 'provider') { providerOpen.value = false; providerForm.value = null; providerBaseline.value = null; editingProvider.value = '' }
...
async function refresh() { if (!busy.value) await Promise.all([load(), loadReservations()]) }
function editProvider(provider = null) {
  if (busy.value || loading.value || readbackPending.value || !loaded.value) return
  conflicts.clear('provider'); error.value=''; message.value=''
...
function closeProvider() {
  ...
  providerOpen.value = false; providerForm.value = null; providerBaseline.value = null; editingProvider.value = ''; conflicts.clear('provider')
}
function editRouting() {
  if (busy.value || loading.value || readbackPending.value || !loaded.value) return
  conflicts.clear('routing'); error.value=''; message.value=''; initialiseRouting(); routesOpen.value = true
}
function closeRouting() {
  if (busy.value) return
  if (routingDirty.value && !window.confirm('放弃尚未保存的职责配置？')) return
  routesOpen.value = false; conflicts.clear('routing'); initialiseRouting()
}
async function saveProvider() {
  if (busy.value || loading.value || readbackPending.value || providerConflict.value || !canSaveProvider.value) return
  ...
  try {
    ...
    await saveFailure(e, 'provider', fresh)
  }
  finally { if (fresh()) busy.value = '' }
}
async function deleteProvider(provider) {
  const conflict = conflicts.entries[`delete:${provider.id}`]
  if (busy.value || loading.value || readbackPending.value || (conflict && !conflict.snapshot)
  ...
  } catch (e) { await saveFailure(e, `delete:${provider.id}`, fresh) }
  finally { if (fresh()) busy.value = '' }
}
async function fetchModels(provider) {
  if (busy.value || loading.value || readbackPending.value || catalogs.value[provider.id] || catalogUnavailable(provider)) return
...
async function saveModels(provider) {
  if (busy.value || loading.value || readbackPending.value || !catalogs.value[provider.id]
  ...
  try {
    const result = await api(`/api/models/providers/${encodeURIComponent(provider.id)}/models`, { method: 'POST', body: JSON.stringify({baseline:JSON.parse(modelOriginal.value[provider.id]),values:{models:modelNames(selectedModels.value[provider.id])}}) })
    await readSaved(result, `models:${provider.id}`, fresh)
  } catch (e) { await saveFailure(e, `models:${provider.id}`, fresh) }
  finally { if (fresh()) busy.value = '' }
}
function profile(key) {
  const value = routingForm.value[key]
  return { provider_id: value.provider_id?.trim() || '', model: value.model?.trim() || '', reasoning_effort: value.reasoning_effort?.trim() || null, supports_vision: !!value.supports_vision }
}
...
async function saveRouting() {
  if (busy.value || loading.value || readbackPending.value || routingConflict.value || !canSaveRouting.value) return
  ...
  } catch (e) { await saveFailure(e, 'routing', fresh) }
  finally { if (fresh()) busy.value = '' }
}
async function testRoute() {
  if (busy.value || loading.value || readbackPending.value || !testConfirm.value) return
  const { name, profile } = testConfirm.value
  const fresh = beginOperation('test'); testResult.value = null
  try { const result = await api('/api/models/test', { method: 'POST', body: JSON.stringify(profile) }); if(fresh()){testResult.value = { name, ...result }; testConfirm.value = null} }
  catch (e) { if(fresh())error.value = e.message }
  finally { if(fresh())busy.value = '' }
}
watch(tab, () => {
  operationGuard(); busy.value = ''; message.value = ''
  providerOpen.value = false; providerForm.value = null; routesOpen.value = false; retrievalOpen.value = false; testConfirm.value = null
  providerBaseline.value = null; editingProvider.value = ''
...
watch(() => providerForm.value?.id.trim(), () => {
  ...
  conflicts.clear('provider'); providerBaseline.value = null; error.value = ''; message.value = ''
}, { flush:'sync' })
</script>
<template>
  <div class="page-stack">
    <PageHeader title="模型设置" description="三个职责显式配置，每次运行固定提供商、模型和推理强度。"><v-btn variant="outlined" :loading="loading||reservationLoading" :disabled="!!busy" @click="refresh">刷新</v-btn><v-btn v-if="tab==='roles'" color="primary" :disabled="!loaded||loading||!!busy||!!readbackPending" @click="editRouting">编辑职责配置</v-btn><v-btn v-else color="primary" :disabled="!loaded||loading||!!busy||!!readbackPending" @click="editProvider()">添加供应商</v-btn></PageHeader>
    ...
    <v-alert v-for="[kind,conflict] in deleteConflicts" :key="kind" type="warning" variant="tonal">
      ...
      <p v-if="conflict.snapshot" class="mt-2">已重读保存值（{{ fmtTime(conflict.readAt) }}）。{{ conflict.snapshot.provider ? '核对后可回到该供应商再次点击删除，仍需确认；不会重试原删除。' : '该供应商当前已不存在，没有再次删除。' }}</p>
      <p v-else class="mt-2">先重读当前供应商，再决定是否提出新的删除操作。</p>
      <p v-if="conflict.readError" role="alert">重读失败：{{ conflict.readError }}</p>
      <details v-if="conflict.snapshot?.provider" class="mt-2"><summary>查看本次读取的供应商</summary><ResourceViewer :content="conflict.snapshot.provider" title="已保存供应商（不含密钥）" /></details>
      <div class="actions mt-3"><v-btn variant="text" :disabled="!!busy||loading" @click="refresh">重读供应商</v-btn><v-btn variant="text" :disabled="!!busy" @click="conflicts.clear(kind)">放弃本次删除</v-btn></div>
    </v-alert>
    <v-card v-if="loaded" class="pa-4 retrieval-card"><div class="role-title"><h2>语义检索</h2><div class="actions"><v-chip size="small" :color="data.retrieval?.embedding ? 'primary' : 'default'">{{ data.retrieval?.embedding ? '已绑定' : '未启用' }}</v-chip><v-btn variant="outlined" size="small" :disabled="!!busy||!!readbackPending" @click="editRetrieval">编辑</v-btn></div></div><p class="muted">只在 Agent 主动查询认识时使用；索引失败不会回滚已提交认识。Embedding 与 rerank 请求单独计量。</p><p v-if="data.retrieval?.embedding" class="auxiliary">Embedding：{{ data.retrieval.embedding.provider_id }} · {{ data.retrieval.embedding.model }}<span v-if="data.retrieval.embedding.dimension"> · {{ data.retrieval.embedding.dimension }} 维</span></p><p v-if="data.retrieval?.rerank" class="auxiliary">Rerank：{{ data.retrieval.rerank.provider_id }} · {{ data.retrieval.rerank.model }}</p></v-card>
    <v-card class="pa-4 usage-card">
      <div class="role-title"><h2>工作额度预占</h2><v-btn size="small" variant="text" :loading="reservationLoading" @click="loadReservations">刷新额度账</v-btn><v-chip size="small" :color="reservationAccounts.length ? 'primary' : 'default'">{{ reservations?.day_key || '未读取' }}</v-chip></div>
      ...
      <v-progress-linear v-if="reservationLoading" indeterminate /><p v-if="reservationReadAt" class="muted">额度读取于 {{ fmtTime(reservationReadAt) }}{{ reservationError ? '；当前刷新失败，保留该次样本' : '' }}</p>
    ...
    <template v-else>
      ...
      <v-card v-for="id in orphanCatalogs" :key="`missing:${id}`" class="pa-5 provider-card">
        <h2>{{ id }} · 目录草稿失效</h2><p class="my-3">{{ catalogInvalid[id] }}</p>
        <ResourceViewer :content="selectedModels[id]" title="未保存的目录选择（仅供核对）" />
        <v-btn class="mt-3" variant="outlined" :disabled="!!busy||!!readbackPending" @click="cancelCatalog(id)">放弃这份目录草稿</v-btn>
      </v-card>
      <v-card v-for="provider in data.providers" :key="provider.id" class="pa-5 provider-card">
        <div class="provider-heading"><div class="provider-name"><h2>{{ provider.id }}</h2><p class="provider-url muted">{{ provider.base_url }}</p></div><StatusBadge domain="provider" :status="provider.enabled?'enabled':'disabled'" /></div>
        <div class="provider-meta"><span>密钥 {{ provider.api_key_masked || '未设置' }}</span><span>超时 {{ provider.timeout_seconds }} 秒</span><span v-if="inUse(provider.id)">已保存的职责或检索绑定引用</span></div>
        <div class="actions"><v-btn variant="outlined" :disabled="!!busy||loading||!!readbackPending" @click="editProvider(provider)">编辑接口</v-btn><v-btn color="error" variant="text" :disabled="!!busy||loading||!!readbackPending||inUse(provider.id)||(!!conflicts.entries[`delete:${provider.id}`]&&!conflicts.entries[`delete:${provider.id}`].snapshot)" @click="deleteProvider(provider)">删除</v-btn></div>
        <v-divider class="my-4" /><h3 class="mb-3">常用模型目录</h3>
        <template v-if="catalogs[provider.id]">
          <v-alert v-if="catalogInvalid[provider.id]" type="warning" variant="tonal" class="mb-3">{{ catalogInvalid[provider.id] }}<p v-if="conflicts.entries[`models:${provider.id}`]">{{ conflicts.entries[`models:${provider.id}`].problem.message }}</p></v-alert>
          <ConfigConflictBanner v-else :conflict="conflicts.entries[`models:${provider.id}`]?.problem" :current="conflicts.entries[`models:${provider.id}`]?.snapshot" :path-label="conflicts.entries[`models:${provider.id}`]?.problem.path.join(' → ')" :read-at="conflicts.entries[`models:${provider.id}`]?.readAt" :read-error="conflicts.entries[`models:${provider.id}`]?.readError" :busy="!!busy||loading||!!readbackPending" @keep="resolveConflict(`models:${provider.id}`,true)" @take="resolveConflict(`models:${provider.id}`,false)" @reload="refresh">
            <template #current><ResourceViewer :content="conflicts.entries[`models:${provider.id}`]?.snapshot?.provider?.models" title="本次保存的常用模型" /></template>
          </ConfigConflictBanner>
          <v-autocomplete :disabled="!!busy||loading||!!readbackPending||!!catalogInvalid[provider.id]" v-model="selectedModels[provider.id]" :items="[...new Set([...catalogs[provider.id],...provider.models,...selectedModels[provider.id]])]" label="搜索并选择常用模型" multiple chips closable-chips clearable />
          <div class="actions"><v-btn color="primary" :loading="busy===`models:${provider.id}`" :disabled="!!busy||loading||!!readbackPending||!!catalogInvalid[provider.id]||!!conflicts.entries[`models:${provider.id}`]" @click="saveModels(provider)">保存常用模型</v-btn><v-btn variant="text" :disabled="!!busy||!!readbackPending" @click="cancelCatalog(provider.id)">取消选择</v-btn></div>
        </template>
        <template v-else>
          <div class="model-tags"><v-chip v-for="model in provider.models" :key="model" size="small">{{ model }}</v-chip><span v-if="!provider.models.length" class="muted">尚未保存常用模型</span></div>
          <p v-if="catalogUnavailable(provider)" class="muted mt-3">{{ catalogUnavailable(provider) }}</p>
          <v-btn class="mt-4" variant="tonal" :loading="busy===`catalog:${provider.id}`" :disabled="!!busy||loading||!!readbackPending||!!catalogUnavailable(provider)" @click="fetchModels(provider)">获取接口模型目录</v-btn>
        </template>
      </v-card>
      <v-card v-if="loaded&&!error&&!data.providers.length" class="pa-8 text-center muted">还没有供应商，点击“添加供应商”开始配置。</v-card>
    </template>
    <v-dialog :model-value="providerOpen" max-width="650" scrollable :persistent="!!busy" @update:model-value="value=>!value&&closeProvider()">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-alert v-if="providerRemoved&&!providerConflict" type="warning" variant="tonal" class="mb-4">原供应商已从保存列表移除，旧草稿不能继续保存。请核对后关闭此编辑；确需新建时明确添加，后来出现的同名供应商也须重新进入编辑。</v-alert>
          <v-alert v-if="readbackPending" type="warning" variant="tonal" class="mb-4">已写入配置，待读回真实值。<v-btn variant="text" :disabled="!!busy" :loading="loading" @click="refresh">重新读取保存值</v-btn></v-alert>
          <ConfigConflictBanner :conflict="providerConflict?.problem" :current="providerConflict?.snapshot" :path-label="providerConflict?.problem.path.join(' → ')" :read-at="providerConflict?.readAt" :read-error="providerConflict?.readError" :keep-disabled="providerKeepBlocked" :busy="!!busy||loading||!!readbackPending" @keep="resolveConflict('provider',true)" @take="resolveConflict('provider',false)" @reload="refresh">
            <template #current><ResourceViewer v-if="providerConflict?.snapshot?.provider" :content="providerConflict.snapshot.provider" title="本次保存的供应商（不含密钥）" /><p v-else>这个名称当前没有供应商。</p></template>
          </ConfigConflictBanner>
          <p v-if="providerKeepBlocked" class="muted mb-4">{{ editingProvider ? '原供应商已从保存列表移除，旧编辑不能重建或接到后来出现的同名供应商。请采用现值重新编辑，或关闭后明确添加。' : '此名称已有供应商。请改用新名称，或采用现值进入该供应商编辑；不会用新建草稿直接覆盖已有接口。' }}</p>
          <v-form v-if="providerForm" :disabled="!!busy||loading||!!readbackPending" class="config-form" @submit.prevent="saveProvider">
            <v-alert v-if="error" type="error" variant="tonal">{{ error }}</v-alert>
            <v-text-field v-model="providerForm.id" label="供应商名称" :disabled="!!editingProvider||!!busy||loading||!!readbackPending" required />
            ...
            <v-select v-model="providerForm.api_key_action" label="密钥操作" :items="[{title:'保留当前密钥',value:'keep'},{title:'替换密钥',value:'replace'},{title:'清除密钥',value:'clear'}]" /><v-text-field v-if="providerForm.api_key_action==='replace'" v-model="providerForm.api_key" label="接口密钥" type="password" autocomplete="new-password" placeholder="替换时必须填写新密钥；留空不会保存" />
            ...
            <v-btn type="submit" color="primary" :loading="busy==='provider'" :disabled="!!busy||loading||!!readbackPending||!!providerConflict||!canSaveProvider">保存供应商</v-btn>
          </v-form>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog :model-value="routesOpen" max-width="900" scrollable :persistent="!!busy" @update:model-value="value=>!value&&closeRouting()">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-alert v-if="readbackPending" type="warning" variant="tonal" class="mb-4">已写入配置，待读回真实值。<v-btn variant="text" :disabled="!!busy" :loading="loading" @click="refresh">重新读取保存值</v-btn></v-alert>
          <ConfigConflictBanner :conflict="routingConflict?.problem" :current="routingConflict?.snapshot" :path-label="routingConflict?.problem.path.join(' → ')" :read-at="routingConflict?.readAt" :read-error="routingConflict?.readError" :busy="!!busy||loading||!!readbackPending" @keep="resolveConflict('routing',true)" @take="resolveConflict('routing',false)" @reload="refresh">
            <template #current><ResourceViewer v-if="routingConflict?.snapshot?.values" :content="routingConflict.snapshot.values" title="本次保存的职责绑定" /><p v-else>当前没有配置职责路由。</p></template>
          </ConfigConflictBanner>
          <p class="muted mb-3">切换供应商或模型会清除旧绑定的推理强度和视觉确认。冲突后不会将旧参数拼到他人刚更换的模型；明确改选绑定时，该项按整项保留。</p>
          <v-form v-if="routingForm" :disabled="!!busy||loading||!!readbackPending" @submit.prevent="saveRouting">
            <section v-for="role in roles" :key="role.key" class="routing-section">
              ...
              <template v-if="roleEnabled[role.key]">
                <div class="route-fields">
                  <v-select :model-value="routingForm[role.key].provider_id" :items="data.providers.map(p=>({title:`${p.id}${p.enabled?'':'（停用）'}`,value:p.id}))" label="供应商" @update:model-value="value=>changeRoutingBinding(role.key,'provider_id',value)" />
                  <v-combobox :model-value="routingForm[role.key].model" :items="choicesFor(routingForm[role.key].provider_id)" label="模型名称" @update:model-value="value=>changeRoutingBinding(role.key,'model',value)" />
                ...
                <p v-if="routingForm[role.key].provider_id&&!providerById(routingForm[role.key].provider_id)" class="text-error mb-3" role="alert">原供应商 {{ routingForm[role.key].provider_id }} 已不在当前保存列表中；请明确选择，不自动迁移到其他接口。</p>
                <v-btn variant="text" color="primary" :disabled="!!busy||!!readbackPending||!providerById(routingForm[role.key].provider_id)?.enabled||!routingForm[role.key].model?.trim()" @click="testConfirm={name:role.name,profile:profile(role.key)}">检查当前选择（不保存）</v-btn>
              </template>
            ...
            <v-btn type="submit" color="primary" :loading="busy==='routing'" :disabled="!!busy||loading||!!readbackPending||!!routingConflict||!canSaveRouting||!routingDirty">保存职责配置</v-btn>
          </v-form>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog :model-value="retrievalOpen" max-width="700" scrollable :persistent="!!busy" @update:model-value="value=>!value&&closeRetrieval()">
      <v-card>
        <v-card-title class="dialog-title">编辑语义检索配置<v-btn variant="text" :disabled="!!busy" @click="closeRetrieval">关闭</v-btn></v-card-title>
        <v-card-text>
          <v-alert v-if="message" type="info" variant="tonal" class="mb-4">{{ message }}</v-alert>
          <v-alert v-if="readbackPending" type="warning" variant="tonal" class="mb-4">已写入配置，待读回真实值。<v-btn variant="text" :disabled="!!busy" :loading="loading" @click="refresh">重新读取保存值</v-btn></v-alert>
          <v-alert v-if="error" type="error" variant="tonal" class="mb-4">{{ error }}</v-alert>
          <ConfigConflictBanner :conflict="retrievalConflict?.problem" :current="retrievalConflict?.snapshot" :path-label="retrievalConflict?.problem.path.join(' → ')" :read-at="retrievalConflict?.readAt" :read-error="retrievalConflict?.readError" :busy="!!busy||loading||!!readbackPending" @keep="resolveConflict('retrieval',true)" @take="resolveConflict('retrieval',false)" @reload="refresh">
            <template #current><ResourceViewer :content="retrievalConflict?.snapshot?.values" title="本次保存的检索绑定" /></template>
          </ConfigConflictBanner>
          <v-form :disabled="!!busy||loading||!!readbackPending" class="config-form" @submit.prevent="saveRetrieval">
            ...
            <section v-for="key in ['embedding','rerank']" :key="key">
              <v-divider class="my-3" /><h3>{{ key==='embedding'?'Embedding':'Rerank（可选）' }}</h3>
              <div class="route-fields">
                <v-select :model-value="retrievalForm[key].provider_id" :items="data.providers.map(p=>({title:`${p.id}${p.enabled?'':'（停用）'}`,value:p.id}))" label="供应商" clearable @update:model-value="value=>changeRetrievalBinding(key,'provider_id',value)" />
                <v-combobox :model-value="retrievalForm[key].model" :items="choicesFor(retrievalForm[key].provider_id)" label="模型名称" clearable @update:model-value="value=>changeRetrievalBinding(key,'model',value)" />
                <v-text-field v-if="key==='embedding'" v-model.number="retrievalForm[key].dimension" type="number" min="1" label="已确认维度（可留空）" />
                <v-select v-else v-model="retrievalForm[key].protocol" :items="[{title:'Cohere v1（需确认供应商兼容）',value:'cohere_v1'}]" label="已确认协议" clearable />
              </div>
            </section>
            <p v-if="retrievalIssue" class="text-error" role="alert">{{ retrievalIssue }}</p>
            <v-btn type="submit" color="primary" :loading="busy==='retrieval'" :disabled="!!busy||loading||!!readbackPending||!!retrievalConflict||!!retrievalIssue||!retrievalDirty">保存语义检索配置</v-btn>
          </v-form>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog :model-value="!!testConfirm" max-width="560" :persistent="busy==='test'" @update:model-value="value=>!value&&(testConfirm=null)"><v-card v-if="testConfirm" title="主动能力检查"><v-card-text><p>检查 {{ testConfirm.name }}：{{ testConfirm.profile.provider_id }} / {{ testConfirm.profile.model }}。</p><p class="mt-3">将进行最多 2 次真实模型请求，检查原图、指定工具与原生续接，可能产生供应商费用。检查使用合成资料，不向群聊发送消息。</p><v-alert v-if="error" type="error" variant="tonal" class="mt-3">{{ error }}</v-alert></v-card-text><v-card-actions><v-spacer /><v-btn :disabled="busy==='test'" @click="testConfirm=null">取消</v-btn><v-btn color="primary" :loading="busy==='test'" :disabled="!!readbackPending" @click="testRoute">确认发起检查</v-btn></v-card-actions></v-card></v-dialog>
  </div>
</template>
<style scoped>
