const purposes={
  audio_transcription:'音频片段转写',
  interest_share:'公共兴趣表达',
  heartbeat:'心跳公共研究',
  action_review:'动作审查',
  conversation:'对话',
  plugin_agent:'插件 Agent',
  announcement:'旧开播邀请',
  work:'后台工作',
  history_maintenance:'历史维护',
  work_compression:'工作压缩',
  skill_maintenance:'技能整理',
  capability_probe:'能力检查',
  embedding_query:'语义查询向量',
  embedding_document:'索引向量',
  rerank:'候选重排'
}
const events={
  ...
  SCENE_WAKE_CONFIRMED:'本群叫醒已确认',
  ACTION_REQUESTED:'动作已登记',
  ACTION_REVIEW_STARTED:'动作审查已开始',
  ...
  AGENT_JOB_CHECKPOINT:'工作检查点',
  TOOL_OBSERVATION_RECORDED:'工具资料',
  HISTORY_COMPACTION_RECORDED:'历史摘要',
  REFLECTION_RECORDED:'维护认识',
  MEDIA_UPDATED:'媒体更新',
  OPERATOR_ACTION:'运营操作',
  LIVE_STARTED:'直播开始',
  LIVE_ENDED:'直播结束',
  TOOL_COMPLETED:'感知工具返回',
  USER_JOINED:'成员加入'
}
...
export function eventLabel(value){
  return value==='PLUGIN_EVENT'?'插件事件':events[value] || value
}
const traces={
  interest_share:'公共兴趣分享机会',
  heartbeat:'心跳周期',
  action_review:'动作审查',
  browser_cleanup:'浏览器回收',
  history_projection_error:'维护提交后的投影失败',
  conversation:'对话',
  ...
  agent_job:'信息工作',
  agent_job_wait:'工作槽位等待',
  agent_job_error:'工作失败',
  history_maintenance:'历史维护',
  history_maintenance_error:'历史维护失败',
  skill_maintenance_wait:'方法维护槽位等待',
  work_compression:'工作压缩',
  skill_maintenance:'技能整理',
  memory_retrieval:'认识召回'
}
Object.assign(traces, {
  memory_index_rebuild:'认识索引显式重建',
  memory_index_error:'认识索引错误',
  memory_index_cancelled:'认识索引已停止'
})
export const traceOptions=Object.entries(traces).map(([value,title])=>({value,title}))
export function traceLabel(value){
  return traces[value] || value
}
export function formatDurationMs(value){
  return Number.isFinite(value) && value >= 0 ? `${value.toLocaleString()} ms` : '未记录'
}
...
export function interactionReason(value){
  return {
    ...
    chat_closed_for_requester:'普通聊天关闭，此 QQ 账号不在回复白名单',
    calendar_command:'旧日程命令记录',
    calendar_comment:'旧日程引用评论',
    calendar_response:'旧日程响应记录',
    announcement:'旧开播公告记录',
    chat_eligible:'具备普通聊天资格，仍由注意力与对话决定是否参与'
  }[value] || value
}
