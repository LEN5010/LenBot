...
class EgressRules:
    ...
    def resolve(self, host: str, port: int) -> tuple[str, ...]:
        ...
        The synchronous form is for in-container pre-checks.  The proxy uses
        ``resolve_async`` so DNS does not block its event loop.
        """
        try:
            answers = socket.getaddrinfo(host, port, type=socket.SOCK_STREAM)
        except socket.gaierror as error:
            raise EgressBlocked(f'目的主机 {host} 无法解析：{error}') from None
        return self._admit_answers(host, answers)

    async def resolve_async(self, host: str, port: int) -> tuple[str, ...]:
        """The same admission as ``resolve``, without blocking the event loop."""
    ...
    def _admit_answers(self, host: str, answers) -> tuple[str, ...]:
        addresses: list[str] = []
