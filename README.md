...
如果只是群里使用 Bot，先看[群内使用指南](docs/usage.md)；不需要先安装项目或阅读架构。其他读者按[四类入口](docs/README.md#按读者开始)选择。以下是安装者入口。
...
- 后台工作：建立工作后分步检索、计算、归纳，产出带来源的结果；工作可查看进度、继续、修订和取消；执行中断与控制请求另看当前记录。
...
支持组合、实验性后端和打包范围见[支持与信任边界](docs/support.md)。私下安全报告邮箱与提交范围见[安全问题报告](SECURITY.md)。

## 文档

完整目录见[文档入口](docs/README.md)。核心开发接手时先读[贡献流程](CONTRIBUTING.md)、[当前任务](docs/iteration.md)和[路线状态](docs/plan/README.md)，再按模块读当前合同；历史文档不作为新能力的启用依据。
