...
12. 实发群名单与全局 Shadow 是唯一运营发送控制。默认开启 Shadow，初始实发群名单为 group:126300994；名单外不实发。源码、模型或人格变化不触发重新验收、清空名单或批量隔离任务。内部模拟不能伪装真实群聊。
13. 账号昵称、群名片是事件事实；偏好称呼和反馈是有证据的增量理解。列表添加/移除，省略不清空，同名不合并。
